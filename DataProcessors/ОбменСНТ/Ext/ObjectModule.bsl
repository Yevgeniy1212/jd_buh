...
Процедура ДокументСНТ_ОбработкаЗаполнения(ОбъектСНТ, ДанныеЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если ТипЗнч(МассивДанныхДляЗаполнения) = Тип("Массив") И МассивДанныхДляЗаполнения.Количество() > 0 Тогда
		...
		Для каждого СтруктураДокумента Из МассивДанныхДляЗаполнения Цикл
			...
			ЗаполнятьВесНетто = СНТСервер.ПроверкаЗаполнятьВесНетто(НовыйДокументСНТ);
			...
			Для Каждого СтрокаТЧ Из СтруктураДокумента.Товары Цикл
				...
				Если ЗаполнятьВесНетто Тогда
					ЗаполнитьЗначенияСвойств(НоваяСтрокаТЧ,СтрокаТЧ);
				Иначе
					ЗаполнитьЗначенияСвойств(НоваяСтрокаТЧ,СтрокаТЧ,,"ВесНетто");
				КонецЕсли;
...
Процедура ДокументСНТ_ОбработкаПроведения(ОбъектСНТ, Отказ, РежимПроведения) Экспорт
	...
	Если ОбъектСНТ.Направление = Перечисления.НаправленияСНТ.Исходящий 
		И (ОбъектСНТ.ВидОперации = Перечисления.ВидыОперацийСНТ.Ввоз И НЕ(ОбъектСНТ.ПоставщикФизическоеЛицо И ОбъектСНТ.ТипСНТ = Перечисления.ТипыСНТ.ВозвратТоваров)
			ИЛИ ОбъектСНТ.ПоставщикМалаяТорговаяТочка И ОбъектСНТ.ПредоставленВБумажномВиде)
	...
	Если (((ОбъектСНТ.Направление = Перечисления.НаправленияСНТ.Входящий И НЕ СНТСервер.ПроверитьСтатусНеТребуетПодтверждения(ОбъектСНТ) 
				И НЕ ОбъектСНТ.ПричинаВыпискиНаБумажномНосителе = Перечисления.ПричиныВыпискиСФНаБумаге.ОтсутствиеТребованиеПоВыписке)
			ИЛИ (ОбъектСНТ.Направление = Перечисления.НаправленияСНТ.Исходящий И ОбъектСНТ.ВидОперации = Перечисления.ВидыОперацийСНТ.Ввоз
	...
	Если ОбъектСНТ.Направление = Перечисления.НаправленияСНТ.Исходящий 
		И ОбъектСНТ.ПредоставленВБумажномВиде 
		И НЕ ОбъектСНТ.ПричинаВыпискиНаБумажномНосителе = Перечисления.ПричиныВыпискиСФНаБумаге.ОтсутствиеТребованиеПоВыписке
		И НЕ ОбъектСНТ.ВидОперации = Перечисления.ВидыОперацийСНТ.Ввоз Тогда
		
		Если СНТСервер.ПроверитьСтатусНеТребуетПодтверждения(ОбъектСНТ) Тогда //Обработка по бумажному СНТ с условиями, когда не требуется подтверждение
			
			
			Если ОбъектСНТ.Статус = Перечисления.СтатусыСНТ.Черновик Тогда
			...
			Если НЕ ОбъектСНТ.Статус = Перечисления.СтатусыСНТ.Черновик Тогда
				
				//резервы были в статусе Черновика
				СНТСервер.СформироватьДвиженияТоваровНаВСВРезерве(ОбъектСНТ.ДатаВыпискиНаБумажномНосителе, ПараметрыПроведения.Реквизиты, ПараметрыПроведения.ТаблицаТоварыНаВСВРезерве, ОбъектСНТ.Движения,ВидДвиженияНакопления.Приход, Отказ, Перечисления.СпособВыпискиСНТ.ВБумажномВиде);
				...
				//сделать расход по товарам на ВС
				СНТСервер.СформироватьДвиженияТоваровНаВС(ПараметрыПроведения.Реквизиты, ПараметрыПроведения.ТаблицаТоварыНаВС, ОбъектСНТ.Движения, Отказ);
		...
		ИначеЕсли ОбъектСНТ.Статус = Перечисления.СтатусыСНТ.Созданный 
			ИЛИ ОбъектСНТ.Статус = Перечисления.СтатусыСНТ.Черновик 
			ИЛИ ОбъектСНТ.Статус = Перечисления.СтатусыСНТ.Доставленный 
...
Процедура ДокументСНТ_ПередЗаписью(ОбъектСНТ, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	Если НЕ Отказ И РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		Если ОбъектСНТ.Направление = Перечисления.НаправленияСНТ.Исходящий
			И (НЕ(ОбъектСНТ.ВидОперации = Перечисления.ВидыОперацийСНТ.Ввоз И ОбъектСНТ.ПредоставленВБумажномВиде) 
					ИЛИ (ОбъектСНТ.ВидОперации <> Перечисления.ВидыОперацийСНТ.Ввоз И ОбъектСНТ.ПредоставленВБумажномВиде))
		...
		ИначеЕсли ОбъектСНТ.Направление = Перечисления.НаправленияСНТ.Исходящий И 
			((ОбъектСНТ.ВидОперации = Перечисления.ВидыОперацийСНТ.Ввоз)
...
Процедура ДокументСНТ_ПриКопировании(ОбъектСНТ, ОбъектКопирования) Экспорт
	...
	// Раздел К. очистить, если в силу вступил уже новый формат раздела. Устаревший формат раздела действовал до 26.12.24.
	Если СНТСерверПереопределяемый.ВведенНовыйРазделК() Тогда
		
		// Раздел К. Данные о грузе, перевозимом на автомобильном транспорте
		
		Если ЗначениеЗаполнено(ОбъектКопирования.НомерОттискаПломбы) Тогда
			ОбъектСНТ.НомерОттискаПломбы = Неопределено;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ОбъектКопирования.ФИОВодителя) Тогда
			ОбъектСНТ.ФИОВодителя = Неопределено;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ОбъектКопирования.ИИНВодителя) Тогда
			ОбъектСНТ.ИИНВодителя = Неопределено;
		КонецЕсли;
		
		//Раздел K1. Сведение о грузе
		Если ОбъектКопирования.СведенияОГрузе.Количество() > 0 Тогда
			ОбъектСНТ.СведенияОГрузе.Очистить();
		КонецЕсли;
		
		//Раздел K2. Погрузочно-разгрузочные операции
		Если ОбъектКопирования.ПогрузочноРазгрузочныеОперации.Количество() > 0 Тогда
			ОбъектСНТ.ПогрузочноРазгрузочныеОперации.Очистить();
		КонецЕсли;
		
		//Раздел K3. Прочие сведения (заполняется перевозчиком)
		Если ОбъектКопирования.ПрочиеСведения.Количество() > 0 Тогда
			ОбъектСНТ.ПрочиеСведения.Очистить();
		КонецЕсли;
		
		//Раздел К4. Таксировка
		Если ОбъектКопирования.Таксировка.Количество() > 0 Тогда
			ОбъектСНТ.Таксировка.Очистить();
		КонецЕсли;
...
Функция ЗаполнитьSNT_ДобавитьБазовыеСведенияОТоваре(products, ТоварыСтрока, Фабрика, ИмяТабличнойЧасти, СНТ)
	...
	Если СНТСерверПереопределяемый.ВесНеттоИспользуется() Тогда
		Если СНТ.Дата>=СНТСерверПереопределяемый.ДатаВводаВесаНетто() Тогда
			Если ЗначениеЗаполнено(ТоварыСтрока.ВесНетто) Тогда
				product.netWeight = ТоварыСтрока.ВесНетто;     
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ЗаполнитьСНТ_ДобавитьБазовыеСведенияОТоваре(ТоварXDTO, ТоварСтрока)
	...
	Если СНТСерверПереопределяемый.ВесНеттоИспользуется() Тогда
		Если СНТСервер.ЕстьСвойство(ТоварXDTO, "netWeight") Тогда
			ТоварСтрока.ВесНетто = СНТСервер.ПолучитьСвойство(ТоварXDTO, "netWeight", "");
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьSNT_Версия1(Знач СНТ, Знач Фабрика, SNT)// Отправка СНТ
	...
	Если ЗначениеЗаполнено(СНТ.УникальныйНомерВалютногоКонтроля) Тогда
		contract.accountNumber = СНТ.УникальныйНомерВалютногоКонтроля; // Учетный номер (F 42.2)
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СНТ.НомерОттискаПломбы) 
		Или ЗначениеЗаполнено(СНТ.ФИОВодителя) 
		Или ЗначениеЗаполнено(СНТ.ИИНВодителя)
		Или СНТ.СведенияОГрузе.Количество() > 0
		Или СНТ.ПогрузочноРазгрузочныеОперации.Количество() > 0
		Или СНТ.ПрочиеСведения.Количество() > 0
		Или СНТ.Таксировка.Количество() > 0   Тогда	
	...
	//Раздел К. Объединенный раздел по сведениям о грузе	
	
	Если СНТСерверПереопределяемый.ВведенНовыйРазделК() Тогда	
		
		Если СНТ.ДанныеОГрузе1_2.Количество() > 0 Тогда
			
			carCargoInfoV1_2 = ЭСФСервер.НовоеСвойствоОбъектаXDTO(SNT, "carCargoInfoV1_2", Фабрика); 			
			
			Если СНТ.ДанныеОГрузе1_2.Количество() > 0 Тогда
				
				sntCarCargoInfoItems = ЭСФСервер.НовоеСвойствоОбъектаXDTO(carCargoInfoV1_2, "sntCarCargoInfoItems", Фабрика);
				
				Для Каждого ДанныеОГрузеCтрока Из СНТ.ДанныеОГрузе1_2 Цикл
					sntCarCargoInfoItem = ЭСФСервер.НовыйЭлементСпискаXDTO(sntCarCargoInfoItems.sntCarCargoInfoItem, Фабрика);
					
					Если НЕ ПустаяСтрока(ДанныеОГрузеCтрока.НомерПутевогоЛиста) Тогда
						sntCarCargoInfoItem.tripNumber = ДанныеОГрузеCтрока.НомерПутевогоЛиста;
					КонецЕсли;
					
					Если НЕ ПустаяСтрока(ДанныеОГрузеCтрока.ДатаОформленияПутевогоЛиста) Тогда
						sntCarCargoInfoItem.tripDate = Формат(ДанныеОГрузеCтрока.ДатаОформленияПутевогоЛиста, "ДФ=dd.MM.yyyy");
					КонецЕсли;
					
					Если НЕ ПустаяСтрока(ДанныеОГрузеCтрока.ФИОВодителя) Тогда
						sntCarCargoInfoItem.driverName = ДанныеОГрузеCтрока.ФИОВодителя;
					КонецЕсли;
					
					Если НЕ ПустаяСтрока(ДанныеОГрузеCтрока.ТехническаяХарактеристикаАТС) Тогда
						sntCarCargoInfoItem.vehicleSpecs = ДанныеОГрузеCтрока.ТехническаяХарактеристикаАТС;
					КонецЕсли;
					
					Если НЕ ПустаяСтрока(ДанныеОГрузеCтрока.МаршрутДвиженияАТС) Тогда
						sntCarCargoInfoItem.route = ДанныеОГрузеCтрока.МаршрутДвиженияАТС;
					КонецЕсли;
					
					Если НЕ ПустаяСтрока(ДанныеОГрузеCтрока.МедОсвидетельствование) Тогда
						sntCarCargoInfoItem.driverMedCheck = ДанныеОГрузеCтрока.МедОсвидетельствование;
					КонецЕсли;
					
					Если НЕ ПустаяСтрока(ДанныеОГрузеCтрока.МестоИВремяВыездаАТСИзНачальногоПункта) Тогда
						sntCarCargoInfoItem.departurePoint = ДанныеОГрузеCтрока.МестоИВремяВыездаАТСИзНачальногоПункта;
					КонецЕсли;
					
					Если НЕ ПустаяСтрока(ДанныеОГрузеCтрока.НомерТТН) Тогда
						sntCarCargoInfoItem.cmrNumber = ДанныеОГрузеCтрока.НомерТТН;
					КонецЕсли;
					
					Если НЕ ПустаяСтрока(ДанныеОГрузеCтрока.ДатаОформленияТТН) Тогда
						sntCarCargoInfoItem.cmrDate = Формат(ДанныеОГрузеCтрока.ДатаОформленияТТН, "ДФ=dd.MM.yyyy");
					КонецЕсли;
					
					Если НЕ ПустаяСтрока(ДанныеОГрузеCтрока.НаименованиеГруза) Тогда
						sntCarCargoInfoItem.cargoName = ДанныеОГрузеCтрока.НаименованиеГруза;
					КонецЕсли;
					
					Если НЕ ПустаяСтрока(ДанныеОГрузеCтрока.ВесИРазмерыГруза) Тогда
						sntCarCargoInfoItem.cargoDimensions = ДанныеОГрузеCтрока.ВесИРазмерыГруза;
					КонецЕсли;
					
					Если НЕ ПустаяСтрока(ДанныеОГрузеCтрока.НаименованиеИКлассОпасностиГруза) Тогда
						sntCarCargoInfoItem.cargoClass = ДанныеОГрузеCтрока.НаименованиеИКлассОпасностиГруза;
					КонецЕсли;
					
					Если НЕ ПустаяСтрока(ДанныеОГрузеCтрока.НомерВидСрокРД) Тогда
						sntCarCargoInfoItem.permitNumber = ДанныеОГрузеCтрока.НомерВидСрокРД;
					КонецЕсли;
					
					Если НЕ ПустаяСтрока(ДанныеОГрузеCтрока.НомерРазрешения) Тогда
						sntCarCargoInfoItem.domesticPermit = ДанныеОГрузеCтрока.НомерРазрешения;
					КонецЕсли;
					
					Если НЕ ПустаяСтрока(ДанныеОГрузеCтрока.ВремяПрибытияАТС) Тогда
						sntCarCargoInfoItem.arrivalTime = Формат(ДанныеОГрузеCтрока.ВремяПрибытияАТС, "ДФ=dd.MM.yyyy HH.mm.ss");
					КонецЕсли;
					
					Если НЕ ПустаяСтрока(ДанныеОГрузеCтрока.ВремяУбытияАТС) Тогда
						sntCarCargoInfoItem.departureTime = Формат(ДанныеОГрузеCтрока.ВремяУбытияАТС, "ДФ=dd.MM.yyyy HH.mm.ss");
					КонецЕсли;
					
					Если НЕ ПустаяСтрока(ДанныеОГрузеCтрока.ВремяДатаПриездаАТСВКонечныйПункт) Тогда
						sntCarCargoInfoItem.endTripTime = Формат(ДанныеОГрузеCтрока.ВремяДатаПриездаАТСВКонечныйПункт, "ДФ=dd.MM.yyyy HH.mm.ss");
					КонецЕсли;
...
Процедура ПроверитьРазличныеДанныеСНТ(ОбъектСНТ, РезультатПроверки) Экспорт
	...
	ТребуетсяПроверкаВесаНетто = Ложь;
	Если СНТСерверПереопределяемый.ВесНеттоИспользуется() Тогда
		// проверка импорта
		Если ОбъектСНТ.Дата >= СНТСерверПереопределяемый.ДатаВесНеттоОбязательноеЗаполнение() И ((ОбъектСНТ.ВидВвоза = ПредопределенноеЗначение("Перечисление.ВидыВвозаСНТ.Импорт")
			ИЛИ ОбъектСНТ.ВидВвоза = ПредопределенноеЗначение("Перечисление.ВидыВвозаСНТ.ВвозНаПереработку") 
			ИЛИ ОбъектСНТ.ВидВвоза = ПредопределенноеЗначение("Перечисление.ВидыВвозаСНТ.ВременныйВвоз") 
			ИЛИ ОбъектСНТ.ВидВвоза = ПредопределенноеЗначение("Перечисление.ВидыВвозаСНТ.ВвозВременноВывезенногоТовара") 
			ИЛИ (ОбъектСНТ.ВидВвоза = ПредопределенноеЗначение("Перечисление.ВидыВвозаСНТ.ВвозТоваровНаТерриториюСЭЗ") И (ОбъектСНТ.ПоставщикКодСтраныОтправки = "RU" ИЛИ ОбъектСНТ.ПоставщикКодСтраныОтправки = "AM" ИЛИ ОбъектСНТ.ПоставщикКодСтраныОтправки = "KG" ИЛИ ОбъектСНТ.ПоставщикКодСтраныОтправки = "BY")))) Тогда
			ТребуетсяПроверкаВесаНетто = Истина;
		КонецЕсли;
		// проверка экспорта
		Если ОбъектСНТ.Дата >= СНТСерверПереопределяемый.ДатаВесНеттоОбязательноеЗаполнение() И ((ОбъектСНТ.ВидВывоза = ПредопределенноеЗначение("Перечисление.ВидыВывозаСНТ.ВывозНаПереработку")
			ИЛИ ОбъектСНТ.ВидВывоза = ПредопределенноеЗначение("Перечисление.ВидыВывозаСНТ.Экспорт") 
			ИЛИ ОбъектСНТ.ВидВывоза = ПредопределенноеЗначение("Перечисление.ВидыВывозаСНТ.ВременныйВывоз") 
			ИЛИ ОбъектСНТ.ВидВывоза = ПредопределенноеЗначение("Перечисление.ВидыВывозаСНТ.ВывозВременноВвезенногоТовара") 
			ИЛИ (ОбъектСНТ.ВидВывоза = ПредопределенноеЗначение("Перечисление.ВидыВывозаСНТ.ВывозТоваровСТерриторииСЭЗ") И (ОбъектСНТ.ПолучательКодСтраныДоставки = "RU" ИЛИ ОбъектСНТ.ПолучательКодСтраныДоставки = "AM" ИЛИ ОбъектСНТ.ПолучательКодСтраныДоставки = "KG" ИЛИ ОбъектСНТ.ПолучательКодСтраныДоставки = "BY")))) Тогда
			ТребуетсяПроверкаВесаНетто = Истина;
		КонецЕсли;
	КонецЕсли;
	...
	Если (ОбъектСНТ.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийСНТ.Ввоз") И ОбъектСНТ.ТипСНТ = ПредопределенноеЗначение("Перечисление.ТипыСНТ.ВозвратТоваров"))
		И ОбъектСНТ.ГрузоотправительНерезидент И НЕ ЗначениеЗаполнено(ОбъектСНТ.ГрузоотправительИдентификатор) 
		...
		ТекстСообщения = НСтр("ru = 'D. Поле ИИН/БИН грузоотправителя является обязательным для заполнения при ввозе товаров из стран ЕАЭС.'"); 
		РезультатПроверки.УведомительныеОшибки.Добавить(ТекстСообщения);
	КонецЕсли;
	
	Если (ОбъектСНТ.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийСНТ.Ввоз") И ОбъектСНТ.ТипСНТ = ПредопределенноеЗначение("Перечисление.ТипыСНТ.ВозвратТоваров"))
		И ОбъектСНТ.ГрузоотправительНерезидент И ЗначениеЗаполнено(ОбъектСНТ.ГрузоотправительИдентификатор) 
		И (ОбъектСНТ.ГрузоотправительКодСтраныОтправки = "AM" ИЛИ ОбъектСНТ.ГрузоотправительКодСтраныОтправки = "KG"
			ИЛИ ОбъектСНТ.ГрузоотправительКодСтраныОтправки = "RU" ИЛИ ОбъектСНТ.ГрузоотправительКодСтраныОтправки = "BY") Тогда
	...
	Если ОбъектСНТ.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийСНТ.Вывоз")
		И ОбъектСНТ.ГрузополучательНерезидент И НЕ ЗначениеЗаполнено(ОбъектСНТ.ГрузополучательИдентификатор) 
	...
	Если ОбъектСНТ.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийСНТ.Вывоз")
		И ОбъектСНТ.ГрузополучательНерезидент И ЗначениеЗаполнено(ОбъектСНТ.ГрузополучательИдентификатор) 
		И (ОбъектСНТ.ГрузополучательКодСтраныОтправки = "AM" ИЛИ ОбъектСНТ.ГрузополучательКодСтраныОтправки = "KG"
			ИЛИ ОбъектСНТ.ГрузополучательКодСтраныОтправки = "RU" ИЛИ ОбъектСНТ.ГрузополучательКодСтраныОтправки = "BY") Тогда
	...
	Если (ОбъектСНТ.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийСНТ.Ввоз") 
		...
		И НЕ ОбъектСНТ.ПрочийТранспорт Тогда
	...
	Если ЗначениеЗаполнено(ОбъектСНТ.УникальныйНомерВалютногоКонтроля) Тогда
		
		СтрокаСоответствуетФасету = СНТКлиентСерверПереопределяемый.ВерноУказанУчетныйНомер(ОбъектСНТ.УникальныйНомерВалютногоКонтроля);
		
		Если ((ОбъектСНТ.ВидВвоза <> ПредопределенноеЗначение("Перечисление.ВидыВвозаСНТ.Импорт")
			И ОбъектСНТ.ВидВвоза <> ПредопределенноеЗначение("Перечисление.ВидыВвозаСНТ.ВвозТоваровНаТерриториюСЭЗ")
			И ОбъектСНТ.ВидВывоза <> ПредопределенноеЗначение("Перечисление.ВидыВывозаСНТ.Экспорт")
			И ОбъектСНТ.ВидВывоза <> ПредопределенноеЗначение("Перечисление.ВидыВывозаСНТ.ВывозТоваровСТерриторииСЭЗ"))) Тогда
			
			ТекстСообщения = НСтр("ru = 'F. Поле ""Учетный номер"" должно быть не заполнено, т.к. сделка не является операцией ввоза/вывоза.'");
			РезультатПроверки.УведомительныеОшибки.Добавить(ТекстСообщения);
				
		ИначеЕсли НЕ СтрокаСоответствуетФасету Тогда
			ТекстСообщения = НСтр("ru = 'F. Формат заполнения поля ""Учетный номер"" - X/XXX/XXXX/XXXXXXXXXX, где X - цифра, первый фасет должен быть 1 или 2, последний фасет должен содержать от 1 до 20 цифр.'");
			РезультатПроверки.УведомительныеОшибки.Добавить(ТекстСообщения);
			
		Иначе
			
			ЗначениеПервогоФасета = Лев(ОбъектСНТ.УникальныйНомерВалютногоКонтроля, 1);
			
			Если (ОбъектСНТ.ВидВывоза = ПредопределенноеЗначение("Перечисление.ВидыВывозаСНТ.Экспорт") ИЛИ ОбъектСНТ.ВидВывоза = ПредопределенноеЗначение("Перечисление.ВидыВывозаСНТ.ВывозТоваровСТерриторииСЭЗ"))
				И ОбъектСНТ.ПолучательКодСтраныДоставки <> "KZ"
				И ЗначениеПервогоФасета <> "1" 
				И ОбъектСНТ.ТипСНТ <> Перечисления.ТипыСНТ.ВозвратТоваров Тогда
				
				ТекстСообщения = НСтр("ru = 'F. Первый фасет учетного номера валютного договора должен содержать значение ""1"" - при вывозе товаров с территории РК (Экспорт).'");
				РезультатПроверки.УведомительныеОшибки.Добавить(ТекстСообщения);
				
			КонецЕсли;
			
			Если (ОбъектСНТ.ВидВвоза = ПредопределенноеЗначение("Перечисление.ВидыВвозаСНТ.Импорт") ИЛИ ОбъектСНТ.ВидВвоза = ПредопределенноеЗначение("Перечисление.ВидыВвозаСНТ.ВвозТоваровНаТерриториюСЭЗ"))
				И ОбъектСНТ.ПоставщикКодСтраныОтправки <> "KZ" 
				И ЗначениеПервогоФасета <> "2" 
				И ОбъектСНТ.ТипСНТ <> Перечисления.ТипыСНТ.ВозвратТоваров Тогда
				
				ТекстСообщения = НСтр("ru = 'F. Первый фасет учетного номера валютного договора должен содержать значение ""2"" - при ввозе товаров на территорию РК (Импорт).'");
				РезультатПроверки.УведомительныеОшибки.Добавить(ТекстСообщения);
	...
	|КоличествоВКилограммах, КоличествоВМиллилитрах, НомерЛицензии, ДатаВыдачиЛицензии, ДатаОкончанияЛицензии, КоличествоБутылок, ВесНетто", 
	"", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "");
	...
	Если ОбъектСНТ.ТоварыВС.Количество() > 0 Тогда
		...
		Если ЗначениеЗаполнено(ОбъектСНТ.СвязанныйСНТ) Тогда
			Если ОбъектСНТ.СвязанныйСНТ.ТоварыВС.Количество() > 0 Тогда
				масСтрокСВесом = ОбъектСНТ.СвязанныйСНТ.ТоварыВС.НайтиСтроки(Новый Структура("ВесНетто", 0));
				Если масСтрокСВесом.Количество()>0 Тогда
					ТребуетсяПроверкаВесаНетто = Ложь;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		
		Если ТребуетсяПроверкаВесаНетто Тогда
			НовыйСтруктураРеквизитов.ВесНетто = " СНТТовары.ВесНетто, ";
		КонецЕсли;
	...
	Если ОбъектСНТ.ЭтиловыйСпирт.Количество() > 0 Тогда
		...
		Если ЗначениеЗаполнено(ОбъектСНТ.СвязанныйСНТ) Тогда
			Если ОбъектСНТ.СвязанныйСНТ.ЭтиловыйСпирт.Количество() > 0 Тогда
				масСтрокСВесом = ОбъектСНТ.СвязанныйСНТ.ЭтиловыйСпирт.НайтиСтроки(Новый Структура("ВесНетто", 0));
				Если масСтрокСВесом.Количество()>0 Тогда
					ТребуетсяПроверкаВесаНетто = Ложь;
				КонецЕсли;
			КонецЕсли;
	...
	Если ОбъектСНТ.Виноматериал.Количество() > 0 Тогда
		...
		Если ЗначениеЗаполнено(ОбъектСНТ.СвязанныйСНТ) Тогда
			Если ОбъектСНТ.СвязанныйСНТ.Виноматериал.Количество() > 0 Тогда
				масСтрокСВесом = ОбъектСНТ.СвязанныйСНТ.Виноматериал.НайтиСтроки(Новый Структура("ВесНетто", 0));
				Если масСтрокСВесом.Количество()>0 Тогда
					ТребуетсяПроверкаВесаНетто = Ложь;
				КонецЕсли;
			КонецЕсли;
	...
	Если ОбъектСНТ.ПивоПивныеНапитки.Количество() > 0 Тогда
		...
		Если ЗначениеЗаполнено(ОбъектСНТ.СвязанныйСНТ) Тогда
			Если ОбъектСНТ.СвязанныйСНТ.ПивоПивныеНапитки.Количество() > 0 Тогда
				масСтрокСВесом = ОбъектСНТ.СвязанныйСНТ.ПивоПивныеНапитки.НайтиСтроки(Новый Структура("ВесНетто", 0));
				Если масСтрокСВесом.Количество()>0 Тогда
					ТребуетсяПроверкаВесаНетто = Ложь;
				КонецЕсли;
			КонецЕсли;
	...
	Если ОбъектСНТ.АлкогольнаяпродукцияКромеПива.Количество() > 0 Тогда
		...
		Если ЗначениеЗаполнено(ОбъектСНТ.СвязанныйСНТ) Тогда
			Если ОбъектСНТ.СвязанныйСНТ.АлкогольнаяпродукцияКромеПива.Количество() > 0 Тогда
				масСтрокСВесом = ОбъектСНТ.СвязанныйСНТ.АлкогольнаяпродукцияКромеПива.НайтиСтроки(Новый Структура("ВесНетто", 0));
				Если масСтрокСВесом.Количество()>0 Тогда
					ТребуетсяПроверкаВесаНетто = Ложь;
				КонецЕсли;
			КонецЕсли;
	...
	Если ОбъектСНТ.ДанныеПоНефтепродуктам.Количество() > 0 Тогда
		...
		Если ЗначениеЗаполнено(ОбъектСНТ.СвязанныйСНТ) Тогда
			Если ОбъектСНТ.СвязанныйСНТ.ДанныеПоНефтепродуктам.Количество() > 0 Тогда
				масСтрокСВесом = ОбъектСНТ.СвязанныйСНТ.ДанныеПоНефтепродуктам.НайтиСтроки(Новый Структура("ВесНетто", 0));
				Если масСтрокСВесом.Количество()>0 Тогда
					ТребуетсяПроверкаВесаНетто = Ложь;
				КонецЕсли;
			КонецЕсли;
	...
	Если ОбъектСНТ.ДанныеПоБиотопливу.Количество() > 0 Тогда
		...
		Если ЗначениеЗаполнено(ОбъектСНТ.СвязанныйСНТ) Тогда
			Если ОбъектСНТ.СвязанныйСНТ.ДанныеПоБиотопливу.Количество() > 0 Тогда
				масСтрокСВесом = ОбъектСНТ.СвязанныйСНТ.ДанныеПоБиотопливу.НайтиСтроки(Новый Структура("ВесНетто", 0));
				Если масСтрокСВесом.Количество()>0 Тогда
					ТребуетсяПроверкаВесаНетто = Ложь;
				КонецЕсли;
			КонецЕсли;
	...
	Если ОбъектСНТ.ТабачнаяПродукция.Количество() > 0 Тогда
		...
		Если ЗначениеЗаполнено(ОбъектСНТ.СвязанныйСНТ) Тогда
			Если ОбъектСНТ.СвязанныйСНТ.ТабачнаяПродукция.Количество() > 0 Тогда
				масСтрокСВесом = ОбъектСНТ.СвязанныйСНТ.ТабачнаяПродукция.НайтиСтроки(Новый Структура("ВесНетто", 0));
				Если масСтрокСВесом.Количество()>0 Тогда
					ТребуетсяПроверкаВесаНетто = Ложь;
				КонецЕсли;
			КонецЕсли;
	...
	Если ОбъектСНТ.ДругиеТоварыЦифроваяМаркировка.Количество() > 0 Тогда
		...
		Если ЗначениеЗаполнено(ОбъектСНТ.СвязанныйСНТ) Тогда
			Если ОбъектСНТ.СвязанныйСНТ.ДругиеТоварыЦифроваяМаркировка.Количество() > 0 Тогда
				масСтрокСВесом = ОбъектСНТ.СвязанныйСНТ.ДругиеТоварыЦифроваяМаркировка.НайтиСтроки(Новый Структура("ВесНетто", 0));
				Если масСтрокСВесом.Количество()>0 Тогда
					ТребуетсяПроверкаВесаНетто = Ложь;
				КонецЕсли;
			КонецЕсли;
	...
	Если ОбъектСНТ.ТоварыЭкспортныйКонтроль.Количество() > 0 Тогда
		...
		Если ЗначениеЗаполнено(ОбъектСНТ.СвязанныйСНТ) Тогда
			Если ОбъектСНТ.СвязанныйСНТ.ТоварыЭкспортныйКонтроль.Количество() > 0 Тогда				
				масСтрокСВесом = ОбъектСНТ.СвязанныйСНТ.ТоварыЭкспортныйКонтроль.НайтиСтроки(Новый Структура("ВесНетто", 0));
				Если масСтрокСВесом.Количество()>0 Тогда
					ТребуетсяПроверкаВесаНетто = Ложь;
				КонецЕсли;
			КонецЕсли;
...
Процедура ПроверитьТаблицуТовары(ОбъектСНТ, ТаблицаТоваров, НаименованиеТабличнойЧасти, ПредставлениеТабличнойЧасти, СтруктураРеквизитов, РезультатПроверки) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Выборка, "ВесНетто") Тогда
			Если не ЗначениеЗаполнено(Выборка.ВесНетто) Тогда
				ТекстСообщения = НСтр("ru = '%2. Не заполнен ""Вес нетто"" в строке №%1.'"); 
				ТекстСообщения = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(
				ТекстСообщения, Выборка.НомерСтроки, ПредставлениеТабличнойЧасти);
				Если ОбъектСНТ.Дата >= СНТСерверПереопределяемый.ДатаВесНеттоОбязательноеЗаполнение() Тогда
					РезультатПроверки.УведомительныеОшибки.Добавить(ТекстСообщения);
				КонецЕсли;
			ИначеЕсли Выборка.ВесНетто < 0 Тогда
				ТекстСообщения = НСтр("ru = '%2. ""Вес нетто"" не может быть отрицательным в строке №%1.'"); 
				ТекстСообщения = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(
				ТекстСообщения, Выборка.НомерСтроки, ПредставлениеТабличнойЧасти);
				Если ОбъектСНТ.Дата >= СНТСерверПереопределяемый.ДатаВесНеттоОбязательноеЗаполнение() Тогда
					РезультатПроверки.УведомительныеОшибки.Добавить(ТекстСообщения);
...
Функция ПолучитьНовыеСНТ(ПараметрыВыгрузки) Экспорт
	...
	Текст = СтрЗаменить(Текст, "%ТекущаяДата%", ТекущаяДатаСеанса());	
	СообщениеПользователя.Текст = Текст;
	СообщениеПользователя.Сообщить();
	...
	Для Каждого СтруктураПрофиляСинхронизации Из МассивДанныхПрофилейИСЭСФ Цикл
		...
		ПараметрыМетодаВходящихСНТ = ЭСФСервер.ПолучитьПараметрыМетода(ДанныеПрофиля.СтруктурнаяЕдиница.Ссылка, ВРег("querySntUpdates"), Входящий);
		ПараметрыМетодаИсходящихСНТ = ЭСФСервер.ПолучитьПараметрыМетода(ДанныеПрофиля.СтруктурнаяЕдиница.Ссылка, ВРег("querySntUpdates"), Исходящий);
		...
		Пока НЕ ЭтоПоследняяПорция Цикл
			...
			ЭтоПоследняяПорция = ПолучитьДанныеНовыхСНТ(Входящий, СтруктураПрофиляСинхронизации, Ложь, Ложь, ИдентификаторСессии, ВерсияВС, ПараметрыМетодаВходящихСНТ, МассивИдентификаторовДляОбновленияСтатусаПриема, МассивСНТДляПолученияИдентификаторовТоваров);
		КонецЦикла;
		...
		Пока НЕ ЭтоПоследняяПорция Цикл
			...
			ЭтоПоследняяПорция = ПолучитьДанныеНовыхСНТ(Исходящий, СтруктураПрофиляСинхронизации, Ложь, Ложь, ИдентификаторСессии, ВерсияВС, ПараметрыМетодаИсходящихСНТ, МассивИдентификаторовДляОбновленияСтатусаПриема, МассивСНТДляПолученияИдентификаторовТоваров);
		КонецЦикла;
...
Функция ПолучитьДанныеНовыхСНТ(Знач НаправлениеСНТ, Знач ПрофильИСЭСФ, Знач ПолучитьОднуПорцию, Знач ТолькоСоздатьОбновитьФайлы, ИдентификаторСессии, ВерсияИСЭСФ, ПараметрыМетода, МассивИдентификаторовДляОбновленияСтатусаПриема = Неопределено, МассивЭСФДляПолученияИдентификаторовТоваров = Неопределено) Экспорт
	...
	Если НаправлениеСНТ = Перечисления.НаправленияЭСФ.Входящий Тогда
		Направление = Перечисления.НаправленияСНТ.Входящий;
	ИначеЕсли НаправлениеСНТ = Перечисления.НаправленияЭСФ.Исходящий Тогда
	...
	Пока НЕ ПоследнийБлок И (LastEventDate<>ПредыдущийLastEventDate ИЛИ lastSntId<>ПредыдущийlastSntId) Цикл
		...
		ЗапросXML = СтрЗаменить(ЗапросXML, "[direction]", ЭСФКлиентСервер.НаправлениеИСЭСФ(НаправлениеСНТ));
		...
		Если НЕ ЭтоМодульнаяОшибка(ОтветXML, ИнформацияОбОшибке) Тогда
			...
			Иначе
				...
				СоздатьОбновитьПолучаемыеСНТ(sntInfoList, Направление, ДанныеПрофиляИСЭСФ.СтруктурнаяЕдиница.Ссылка, Ложь, МассивИсключаемыхИД, Истина, МассивЭСФДляПолученияИдентификаторовТоваров, МассивСНТДляПодтвержденияПолученияВходящихСНТ, ДанныеПрофиляИСЭСФ.ПользовательИСЭСФ.Пользователь); 
...
Процедура ЗаполнитьСНТ_Версия1(Знач SntInfo, Знач СтруктурнаяЕдиница, Знач НаправлениеСНТ, СНТ) Экспорт //Получение СНТ
	...
	Если sntExport <> Неопределено Тогда
		...
		СНТ.ИдентификаторТерриторииСЭЗ = СНТСервер.ПолучитьСвойство(sntExport, "sezCode", 0);
	КонецЕсли;
	...
	Если sntImport <> Неопределено Тогда
		...
		СНТ.ИдентификаторТерриторииСЭЗ = СНТСервер.ПолучитьСвойство(sntImport, "sezCode", 0);
	КонецЕсли;
	...
	Если contract <> Неопределено Тогда
		...
		СНТ.УникальныйНомерВалютногоКонтроля = СНТСервер.ПолучитьСвойство(contract, "accountNumber",""); // Учетный номер (F 42.2)
	Конецесли;
	...
	Если СНТСерверПереопределяемый.ВведенНовыйРазделК() Тогда		
		carCargoInfoV1_2 = СНТСервер.ПолучитьСвойство(Snt, "carCargoInfoV1_2", Неопределено);		
		Если carCargoInfoV1_2 <> Неопределено Тогда  			
			sntCarCargoInfoItems = СНТСервер.ПолучитьСвойство(carCargoInfoV1_2, "sntCarCargoInfoItems", Неопределено);			
			Если sntCarCargoInfoItems <> Неопределено Тогда  				
				sntCarCargoInfoItem = СНТСервер.ПолучитьСвойство(sntCarCargoInfoItems, "sntCarCargoInfoItem", Неопределено);				
				Если sntCarCargoInfoItem <> Неопределено Тогда  					
					ДанныеОГрузе = ЭСФСервер.СписокXDTOИзОбъектXDTO(sntCarCargoInfoItem);						
					Для Каждого ДанныеОГрузеCтрокаXDTO Из ДанныеОГрузе Цикл						
						ДанныеОГрузе1_2 = СНТ.ДанныеОГрузе1_2.Добавить();
						ДанныеОГрузе1_2.НомерПутевогоЛиста = СНТСервер.ПолучитьСвойство(ДанныеОГрузеCтрокаXDTO, "tripNumber", "");
						ДанныеОГрузе1_2.ДатаОформленияПутевогоЛиста = СНТСервер.ДатаИБ(СНТСервер.ПолучитьСвойство(ДанныеОГрузеCтрокаXDTO, "tripDate", "01.01.0001"));
						ДанныеОГрузе1_2.ФИОВодителя = СНТСервер.ПолучитьСвойство(ДанныеОГрузеCтрокаXDTO, "driverName", "");
						ДанныеОГрузе1_2.ТехническаяХарактеристикаАТС = СНТСервер.ПолучитьСвойство(ДанныеОГрузеCтрокаXDTO, "vehicleSpecs", "");
						ДанныеОГрузе1_2.МаршрутДвиженияАТС = СНТСервер.ПолучитьСвойство(ДанныеОГрузеCтрокаXDTO, "route", "");
						ДанныеОГрузе1_2.МедОсвидетельствование = СНТСервер.ПолучитьСвойство(ДанныеОГрузеCтрокаXDTO, "driverMedCheck", "");
						ДанныеОГрузе1_2.МестоИВремяВыездаАТСИзНачальногоПункта = СНТСервер.ПолучитьСвойство(ДанныеОГрузеCтрокаXDTO, "departurePoint", "");
						ДанныеОГрузе1_2.НомерТТН = СНТСервер.ПолучитьСвойство(ДанныеОГрузеCтрокаXDTO, "cmrNumber", "");
						ДанныеОГрузе1_2.ДатаОформленияТТН = СНТСервер.ДатаИБ(СНТСервер.ПолучитьСвойство(ДанныеОГрузеCтрокаXDTO, "cmrDate", "01.01.0001"));
						ДанныеОГрузе1_2.НаименованиеГруза = СНТСервер.ПолучитьСвойство(ДанныеОГрузеCтрокаXDTO, "cargoName", "");
						ДанныеОГрузе1_2.ВесИРазмерыГруза  = СНТСервер.ПолучитьСвойство(ДанныеОГрузеCтрокаXDTO, "cargoDimensions", "");  
						ДанныеОГрузе1_2.НаименованиеИКлассОпасностиГруза = СНТСервер.ПолучитьСвойство(ДанныеОГрузеCтрокаXDTO, "cargoClass", "");
						ДанныеОГрузе1_2.НомерВидСрокРД = СНТСервер.ПолучитьСвойство(ДанныеОГрузеCтрокаXDTO, "permitNumber", "");
						ДанныеОГрузе1_2.НомерРазрешения = СНТСервер.ПолучитьСвойство(ДанныеОГрузеCтрокаXDTO, "domesticPermit", "");
						ДанныеОГрузе1_2.ВремяПрибытияАТС = СНТСервер.ПолучитьСвойство(ДанныеОГрузеCтрокаXDTO, "arrivalTime", "01.01.0001");
						ДанныеОГрузе1_2.ВремяУбытияАТС = СНТСервер.ПолучитьСвойство(ДанныеОГрузеCтрокаXDTO, "departureTime", "01.01.0001");
						ДанныеОГрузе1_2.ВремяДатаПриездаАТСВКонечныйПункт =  СНТСервер.ПолучитьСвойство(ДанныеОГрузеCтрокаXDTO, "endTripTime", "01.01.0001");							
					КонецЦикла;					
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;	
	...
	Если ogdMarksInfo <> Неопределено Тогда
		...
		ogdMarkType = СНТСервер.ПолучитьСвойство(ogdMarksInfo, "ogdMarkType", "");
		
		Если ogdMarkType <> Неопределено Тогда
			
			Если ogdMarkType = "AIR" Тогда //Воздушный
				СНТ.ПунктАвтомобильногоПропуска = СНТСервер.ПолучитьСвойство(ogdMarks, "checkpointId", ""); // Пункт автомобильного пропуска/КПП/аэропорт (N 87)
				СНТ.ГосНомерАТСФакт = СНТСервер.ПолучитьСвойство(ogdMarks, "planeNumber", ""); // Гос. номер АТС/номер борта (N 90.2)
			КонецЕсли;
			
			Если ogdMarkType = "AUTO" Тогда //Авто
				СНТ.ПунктАвтомобильногоПропуска = СНТСервер.ПолучитьСвойство(ogdMarks, "automotiveCheckpointId", ""); // Пункт автомобильного пропуска/КПП/аэропорт (N 87)
				СНТ.МаркаАТСФакт = СНТСервер.ПолучитьСвойство(ogdMarks, "carModel", ""); // Марка АТС (N 90.1)
				СНТ.ГосНомерАТСФакт = СНТСервер.ПолучитьСвойство(ogdMarks, "carStateNumber", ""); // Гос. номер АТС/номер борта (N 90.2)
				СНТ.ФИОВодителяФакт = СНТСервер.ПолучитьСвойство(ogdMarks, "driverFullName", "");// Ф.И.О. водителя/Наименование перевозчика (N 90.3)
				СНТ.ИИНВодителяФакт = СНТСервер.ПолучитьСвойство(ogdMarks, "driverIin", "");// ИИН/БИН водителя/перевозчика (N 90.4)
			КонецЕсли;
		...
		Если НовыеДанныеСтатусовСНТ <> Неопределено Тогда
			...
			Если ОбновитьСНТ Тогда
				...
				Если СоответствиеПодписейСНТ <> Неопределено 
					...
					ОбъектСНТ.ЭЦПЮрЛицаПрием = ?(СоответствиеПодписейСНТ[СтарыеДанныеСтатусовСНТ.Идентификатор].ТипПодписи = Перечисления.ТипыПодписиЭСФ.ЮридическоеЛицо,
													СоответствиеПодписейСНТ[СтарыеДанныеСтатусовСНТ.Идентификатор].Подпись,"");
					ОбъектСНТ.ЭЦПУполномоченногоПодтверждатьСНТ = ?(СоответствиеПодписейСНТ[СтарыеДанныеСтатусовСНТ.Идентификатор].ТипПодписи = Перечисления.ТипыПодписиЭСФ.УполномоченноеЛицо,
																	СоответствиеПодписейСНТ[СтарыеДанныеСтатусовСНТ.Идентификатор].Подпись,"");
					ОбъектСНТ.ФИОПодтвердившегоСНТ = ФИОСубъектаДляПодтверждения;
					ОбъектСНТ.ПриемПроизвел = НовыеДанныеСтатусовСНТ.БинПолучателя;
...
Функция СоздатьЗапросНаИзменениеСтатусовСНТ(Знач Действие, Знач КоллецияДляИзмененияСтатусов, Знач ДанныеКлючаЭЦП, Знач ТипПодписиЭСФ) Экспорт
	...
	Для Каждого ЭлементКоллеции Из КоллецияДляИзмененияСтатусов Цикл
		...
		Если Действие = СНТКлиентСервер.ДействиеПодтверждение() Тогда
			...
			acceptanceGoodsBodyТекущегоДокумента = СтрЗаменить(acceptanceGoodsBodyТекущегоДокумента, "[ДатаПодтверждения]", ТекущаяДатаСеанса());
...
Функция ВыполнитьЗапросНаИзменениеСтатусовСНТ(Знач Действие, Знач ЗапросXML, Знач ПрофильИСЭСФ, ИдентификаторСессии = Неопределено, СоответствиеПодписейСНТ = Неопределено, ДанныеКлючаЭЦП = Неопределено) Экспорт
	...
	Если ДанныеПрофиляИСЭСФ.Свойство("ОписаниеСертификата") Тогда
		...
		ФИОСубъектаДляПодтверждения = ЭСФКлиентСервер.ПолучитьФИОИзОписанияСертификата(ОписаниеСертификата);
	Иначе
		ДанныеСертификата = ЭСФКлиентСервер.СвойстваСертификата(ДанныеКлючаЭЦП.КлючBase64, ДанныеКлючаЭЦП.Пароль);
		ФИОСубъектаДляПодтверждения = СокрЛП(ДанныеСертификата.ИмяФамилияСубъекта) + " " + СокрЛП(ДанныеСертификата.ОтчествоСубъекта);
	КонецЕсли;
	...
	Если НЕ ЭтоМодульнаяОшибка(ОтветXML, ИнформацияОбОшибке) Тогда
		...
		Если changeStatusResultList <> Неопределено Тогда
			...
			ОтразитьИзмененияСтатусовСНТ(КоллекцияСтатусовСНТ, СтруктурнаяЕдиница, СоответствиеПодписейСНТ, КоллекцияСсылокСНТ,,,, Действие, ФИОСубъектаДляПодтверждения);  //, СоответствиеПодписейСНТ, Действие
			ОбновитьСтатусыЭСФПриИзмененииСНТ(КоллекцияСсылокСНТ, ИдентификаторСессии, ПрофильИСЭСФ);
...
Функция ПечатьСНТ(МассивСНТ, ОбъектыПечати = Неопределено) Экспорт
	...
	|	СНТ.УникальныйНомерВалютногоКонтроля,
	|	СНТ.ОтпускПроизвел,
	|	СНТ.ЭЦПЮрЛицаОтпуск,
	...
	|	СНТТоварыВС.GTIN,
	|	СНТТоварыВС.ВесНетто КАК ВесНетто,
	...
	|	СНТЭтиловыйСпирт.ВесНетто КАК ВесНетто,
	...
	|	СНТВиноматериал.ВесНетто КАК ВесНетто,
	...
	|	СНТПивоПивныеНапитки.ВесНетто КАК ВесНетто,
	...
	|	СНТАлкогольнаяпродукцияКромеПива.ВесНетто КАК ВесНетто,
	...
	|	СНТДанныеПоНефтепродуктам.ВесНетто КАК ВесНетто,
	...
	|	СНТДанныеПоБиотопливу.ВесНетто КАК ВесНетто,
	...
	|	СНТТабачнаяПродукция.ВесНетто КАК ВесНетто,
	...
	|	СНТДругиеТоварыЦифроваяМаркировка.ВесНетто КАК ВесНетто,
	...
	|	СНТТоварыЭкспортныйКонтроль.ВесНетто КАК ВесНетто,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СНТДанныеОГрузе1_2.НомерПутевогоЛиста КАК НомерПутевогоЛиста,
	|	СНТДанныеОГрузе1_2.ДатаОформленияПутевогоЛиста КАК ДатаОформленияПутевогоЛиста,
	|	СНТДанныеОГрузе1_2.ФИОВодителя КАК ФИОВодителя,
	|	СНТДанныеОГрузе1_2.ТехническаяХарактеристикаАТС КАК ТехническаяХарактеристикаАТС,
	|	СНТДанныеОГрузе1_2.МаршрутДвиженияАТС КАК МаршрутДвиженияАТС,
	|	СНТДанныеОГрузе1_2.МедОсвидетельствование КАК МедОсвидетельствование,
	|	СНТДанныеОГрузе1_2.МестоИВремяВыездаАТСИзНачальногоПункта КАК МестоИВремяВыездаАТСИзНачальногоПункта,
	|	СНТДанныеОГрузе1_2.НомерТТН КАК НомерТТН,
	|	СНТДанныеОГрузе1_2.ДатаОформленияТТН КАК ДатаОформленияТТН,
	|	СНТДанныеОГрузе1_2.НаименованиеГруза КАК НаименованиеГруза,
	|	СНТДанныеОГрузе1_2.ВесИРазмерыГруза КАК ВесИРазмерыГруза,
	|	СНТДанныеОГрузе1_2.НаименованиеИКлассОпасностиГруза КАК НаименованиеИКлассОпасностиГруза,
	|	СНТДанныеОГрузе1_2.НомерВидСрокРД КАК НомерВидСрокРД,
	|	СНТДанныеОГрузе1_2.НомерРазрешения КАК НомерРазрешения,
	|	СНТДанныеОГрузе1_2.ВремяПрибытияАТС КАК ВремяПрибытияАТС,
	|	СНТДанныеОГрузе1_2.ВремяУбытияАТС КАК ВремяУбытияАТС,
	|	СНТДанныеОГрузе1_2.ВремяДатаПриездаАТСВКонечныйПункт КАК ВремяДатаПриездаАТСВКонечныйПункт,
	|	СНТДанныеОГрузе1_2.НомерСтроки КАК НомерСтроки
	|ИЗ
	|	ВТ_СНТ КАК СНТ
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СНТ.ДанныеОГрузе1_2 КАК СНТДанныеОГрузе1_2
	|		ПО (СНТДанныеОГрузе1_2.Ссылка = СНТ.Ссылка)
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки";
	...
	ДанныеСведенияОГрузе_2 = Результат[16].Выбрать();
	...
	Если СНТСерверПереопределяемый.ВведенНовыйРазделК() Тогда
		СтруктураОбластей.Вставить("ОбластьРазделKПустаяСтрока", Макет.ПолучитьОбласть("РазделКПустаяСтрока"));
		СтруктураОбластей.Вставить("ОбластьРазделKНовыйСтрока", Макет.ПолучитьОбласть("РазделКНовыйСтрока"));
		СтруктураОбластей.Вставить("ОбластьРазделKНовыйШапка", Макет.ПолучитьОбласть("РазделКНовыйШапка"));
		СтруктураОбластей.Вставить("ОбластьРазделKНовыйПустаяСтрока", Макет.ПолучитьОбласть("РазделКНовыйПустаяСтрока"));
	КонецЕсли;
	...
	Пока ДанныеПоДокументам.Следующий() Цикл
		...
		ДанныеСведенияОГрузе_2.Сбросить();
		...
		Если СНТСерверПереопределяемый.ВведенНовыйРазделК() И ДанныеПоДокументам.Дата >= СНТСерверПереопределяемый.ДатаВводаРазделаКВерсии1_2() Тогда
			Если ДанныеСведенияОГрузе_2.Количество() <> 0 Тогда
				ТабДокумент.Вывести(ОбластиМакета.ОбластьРазделKНовыйШапка);
				Пока ДанныеСведенияОГрузе_2.Следующий() Цикл
					ЗаполнитьЗначенияСвойств(ОбластиМакета.ОбластьРазделKНовыйСтрока.Параметры, ДанныеСведенияОГрузе_2); 
					ТабДокумент.Вывести(ОбластиМакета.ОбластьРазделKНовыйСтрока);
				КонецЦикла;	
				ТабДокумент.Вывести(ОбластиМакета.ОбластьРазделKНовыйПустаяСтрока);
			КонецЕсли;			
		Иначе	
			// Старая Печать документа
...
Процедура ЗаполнитьИсходящийЭСФИзСНТ(ПараметрыЗаполнения, ОбъектЭСФ) Экспорт
	...
	Пока ВыборкаРеквизиты.Следующий() Цикл
		...
		ОбъектЭСФ.Дата = ТекущаяДатаСеанса();
		...
		Для Каждого Товар Из Товары Цикл
			...
			ТипПошлины = ВСКлиентСервер.ОпределитьТипПошлины(СтрокаТовары.НомерЗаявленияВРамкахТС);
			ЭСФСерверПереопределяемый.ЗаполнитьДанныеПоКоличественнойЕдинице(СтрокаТовары, ТипПошлины);
			...
			Если ВыборкаРеквизиты.ТипСНТ = Перечисления.ТипыСНТ.ВозвратТоваров Тогда
				...
				СтрокаТовары.КоличествоВКоличественнойЕдИзм = - СтрокаТовары.КоличествоВКоличественнойЕдИзм;
			КонецЕсли;
		...
		Если ЗначениеЗаполнено(ОбъектЭСФ.ГосучреждениеБИК) Тогда
			БИКОтличается = Ложь;
			Если ОбъектЭСФ.ГосучреждениеБИК <>  ОбработкаОбменЭСФ.ГосучреждениеБИК() Тогда
				Если ОбъектЭСФ.ГосучреждениеБИК <> ОбработкаОбменЭСФ.ГосучреждениеБИК2() Тогда
					БИКОтличается = Истина;
				КонецЕсли;	
			КонецЕсли;		
			
			Если БИКОтличается Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'БИК госучреждения отличается от ""%1"" или ""%2"".'"),ОбработкаОбменЭСФ.ГосучреждениеБИК(),ОбработкаОбменЭСФ.ГосучреждениеБИК2());
...
Функция РезультатЗапросаПоРеквизитамСНТ(ПараметрыЗаполнения) Экспорт
	...
	|	СНТ.УникальныйНомерВалютногоКонтроля КАК УникальныйНомерВалютногоКонтроля,	
	...
	|	СНТ.Контрагент.ОсновнойБанковскийСчет КАК КонтрагентОсновнойБанковскийСчет 
	|ПОМЕСТИТЬ ДанныеСНТ
	|ИЗ
	|	Документ.СНТ КАК СНТ
	...
	|	ДанныеСНТ.УникальныйНомерВалютногоКонтроля КАК УникальныйНомерВалютногоКонтроля,
	...
	|			ТОГДА ДанныеПоПолучателю.ГосУчреждениеБИК
	|		ИНАЧЕ """" 
	|	КОНЕЦ КАК ГосучреждениеБИК, 
	|	ВЫБОР 
	|		КОГДА ДанныеПоПолучателю.ГосударственноеУчреждение 
	|			ТОГДА ДанныеПоПолучателю.ГосУчреждениеИИК
    |		ИНАЧЕ """" 
	|	КОНЕЦ КАК ГосУчреждениеИИК, 
	|	ВЫБОР 
	|		КОГДА ДанныеПоПолучателю.ГосударственноеУчреждение 
	|			ТОГДА Истина
    |		ИНАЧЕ Ложь 
	|	КОНЕЦ КАК Получатель_ГосударственноеУчреждение, 
...
Функция РезультатЗапросаПоТоварам(СНТ)
	...
	|	АлкогольнаяпродукцияКромеПива.GTIN,
	|	ВЫБОР
	...
	|	ДругиеТоварыЦифроваяМаркировка.GTIN,
	|	ВЫБОР
				...
				Если НайденнаяСтрока.НомерСтроки = СтрокаТЧ.НомерСтроки Тогда
					...
					Для каждого Строка Из РеквизитТабличнойЧасти Цикл
						Если СопоставляемыйОбъектСНТ.ТипСНТ = Перечисления.ТипыСНТ.ВозвратТоваров
							...
							Продолжить;
						КонецЕсли;
						Если (ТЧ = "ДанныеПоНефтепродуктам" ИЛИ ТЧ = "ДанныеПоБиотопливу" ИЛИ ТЧ = "ТоварыЭкспортныйКонтроль")
							И (Строка.ИмяРеквизитаЭСФ = "GTIN") Тогда
							Продолжить;
						КонецЕсли;
...
Процедура ЗаполнитьТаблицуТоварыПоПолучателямПоТаблицеУчастниковСД(ТаблицаУчастниковСД, ОбъектЭСФ) Экспорт
	...
	Для Каждого СтрокаТовары Из ОбъектЭСФ.Товары Цикл
		...
		Для Каждого СтрокаТаблицаУчастниковСД Из ТаблицаУчастниковСД Цикл
			...
			СтрокаТоварыПоПолучателям.КоличествоВКоличественнойЕдИзм = СтрокаТовары.КоличествоВКоличественнойЕдИзм * КоэффициентУчастникаСД;
...
Процедура ПодтвердитьДоставкуВходящихСНТ(МассивИдентификаторовСНТ, Знач ПрофильИСЭСФ, ИдентификаторСессии) Экспорт
	...
	// Уведомить ИС ЭСФ о том, что отправленные СНТ были приняты (доставлены).
...
Процедура ОбновитьДокументыСНТПоИдентификатору(Знач МассивСНТ, Знач ПрофильИСЭСФ, ВерсияИСЭСФ, ИдентификаторСессии) Экспорт
	...
	|	ДокументыСНТ.Идентификатор КАК Идентификатор,
	|	ВЫБОР
	|		КОГДА ДокументыСНТ.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСНТ.Ввоз)
	...
	|	ДокументыСНТ.ТипСНТ КАК ТипСНТ
	|ИЗ
	...
	Пока ВыборкаНаправление.Следующий() Цикл
		...
		/////////////////////////////////////////////////////////////////////////////
		// Обновить состояния исправляемых СНТ.
	...
	Для Каждого СтрокаТовар Из ОбъектСНТ[ИмяТабличнойЧасти] Цикл
		...
		Если ТипЗнч(ОбъектСНТ) = Тип("ДокументОбъект.СНТ") Тогда
			...
			Если ИмяТабличнойЧасти <> "Товары" И (ОбъектСНТ.Направление = Перечисления.НаправленияСНТ.Входящий 
				ИЛИ ((ОбъектСНТ.ВидВвоза = Перечисления.ВидыВвозаСНТ.Импорт 
						ИЛИ ОбъектСНТ.ВидВвоза = Перечисления.ВидыВвозаСНТ.ВвозТоваровНаТерриториюСЭЗ
						ИЛИ ОбъектСНТ.ВидВвоза = Перечисления.ВидыВвозаСНТ.ВременныйВвоз
						ИЛИ ОбъектСНТ.ВидВвоза = Перечисления.ВидыВвозаСНТ.ВвозНаПереработку) 
							И СтрокаТовар.ДополнительныйИдентификатор <> 1)
...
Процедура СравнитьКлючевыеРеквизитыИсправленногоИОсновногоСНТ(ТаблицаИсточниковПроисхождения, ОбъектКорректировочныйСНТ,ИмяТабличнойЧасти)
	...
	|//%Возвр%			И ИсточникиПроисхожденияИсправленного.СсылкаИсточникПроисхождения = ИсточникиПроисхожденияОсновного.ИсточникПроисхождения //на тот слчай когда одна и та же номеклатура
	...
	ИначеЕсли ТипСНТ = Перечисления.ТипыСНТ.ВозвратТоваров Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "//%Возвр%", "");
	КонецЕсли;
...
Процедура ЗаполнитьТоварыСУчетомИсточниковПроисхождения(Объект, Отказ = Ложь, ИмяТабличнойЧасти = "Товары") Экспорт
	...
	ПараметрыФормы.Вставить("Дата"		 , ?(Объект.ДатаОтгрузкиТовара = Дата(1,1,1), НачалоДня(ТекущаяДатаСеанса()), Объект.ДатаОтгрузкиТовара));
...
Процедура ЗаполнитьИсходящийСНТ(Знач ДокументОснованиеДляЗаполнения, ОбъектСНТ, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ТипЗнч(МассивДанныхДляЗаполнения) = Тип("Массив") И МассивДанныхДляЗаполнения.Количество() > 0 Тогда
		...
		ЗаполнятьВесНетто = СНТСервер.ПроверкаЗаполнятьВесНетто(ОбъектСНТ);
