...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого СтрокаСписокСНТ Из СписокСНТ Цикл
		...
		Если ПустаяСтрока(СтрокаСписокСНТ.Идентификатор) Тогда
			...
			Сообщение.Текст = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'В строке %1 не заполнено поле ""Идентификатор"".'"),Формат(НомерСтроки, "ЧГ=")); 
		...
		Иначе
			...
			Если НЕ СНТКлиентСервер.ИдентификаторКорректен(СтрокаСписокСНТ.Идентификатор, ТекстСообщения) Тогда 
				...
				Сообщение.Текст = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'В строке %1 поле ""Идентификатор"" заполнено некорректно. Причина: %2'"),Формат(НомерСтроки, "ЧГ="),ТекстСообщения); 
...
Функция НаправлениеДляДействия(Знач Действие, Знач НаправлениеСНТ = Неопределено, ВидОперации = Неопределено, ПоставщикМалаяТорговаяТочка = Ложь, ПоставщикРозничныйРеализатор = Ложь, ТипСНТ = Неопределено, ЗаполненаТЧТоварыВС = Ложь, ДатаСвязанногоСНТ)
	...
	ИначеЕсли Действие = СНТКлиентСервер.ДействиеПодтверждение() Тогда
		...
		Если ВидОперации = Перечисления.ВидыОперацийСНТ.Ввоз ИЛИ ВидОперации = Перечисления.ВидыОперацийСНТ.Перемещение ИЛИ ВидОперации = Перечисления.ВидыОперацийСНТ.ПеремещениеМеждуФилиалами
			...
			ИЛИ (ПоставщикРозничныйРеализатор И ТипСНТ = Перечисления.ТипыСНТ.ВозвратТоваров И ЗаполненаТЧТоварыВС И ДатаСвязанногоСНТ < СНТСерверПереопределяемый.ДатаВводаМТТ())
			ИЛИ (ВидОперации = Перечисления.ВидыОперацийСНТ.Вывоз И ТипСНТ = Перечисления.ТипыСНТ.ВозвратТоваров) Тогда
			Направление = Перечисления.НаправленияСНТ.Исходящий;
		КонецЕсли;	
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Направление <> Направление Тогда
			...
			Если ПоказыватьСообщения Тогда 
				ТекстСообщения = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Невозможно выполнить действие для документа ""%1"", так как он %2.'"),Выборка.СНТ, НРег(Направление));
				ЭСФКлиентСервер.СообщитьПользователю(ТекстСообщения);
			КонецЕсли;
