...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь(, Истина) Тогда
		...
		Элементы.ПодсказкаГдеНайтиЭтуФорму.Заголовок = 
			НСтр("ru = 'Ход обработки данных версии программы можно также проконтролировать из раздела'")  + Символы.ПС +
		               НСтр("ru = '""Информация"" на рабочем столе, команда ""Описание изменений программы"".'");
	...
	ПодсказкаПроблемСДанными = НСтр("ru = 'Проблемы с данными могут мешать их обработке при переходе на новую версию.'") + Символы.ПС +
		 НСтр("ru = 'Если выполнение дополнительных процедур обработки данных завершилось с ошибкой, то необходимо:'") + Символы.ПС +
		 НСтр("ru = '  • перейти к списку проблем и исправить их согласно рекомендации в отчете;'") + Символы.ПС +
		 НСтр("ru = '  • продолжить выполнение дополнительных процедур обработки данных, для чего необходимо перейти по гиперссылке <b>Не все процедуры удалось выполнить ...</b> и нажать на кнопку <b>Запустить</b> внизу формы.'");
...
Процедура ИзменитьРасписаниеПослеУстановкиРасписания(НовоеРасписание, ДополнительныеПараметры) Экспорт
	...
	Если НовоеРасписание <> Неопределено Тогда
		Если НовоеРасписание.ПериодПовтораВТечениеДня = 0 Тогда
			...
			ТекстСообщения = НСтр("ru = 'Дополнительные процедуры обработки данных выполняются небольшими порциями,'") + Символы.ПС +
				НСтр("ru = 'поэтому для их корректной работы необходимо обязательно задать интервал повтора после завершения.'")  + Символы.ПС +
				НСтр("ru = 'Для этого в окне настройки расписания необходимо перейти на вкладку ""Дневное""'") + Символы.ПС +
				НСтр("ru = 'и заполнить поле ""Повторять через"".'");
...
Процедура ПроверитьВыполнениеОтложенногоОбновления(СведенияОбОбновлении)
	...
	Если Задание.Использование Тогда
		...
		ТекстСообщения = НСтр("ru = 'Регламентное задание <b>Отложенное обновление</b> включено, но'") + Символы.ПС +
			НСтр("ru = 'не выполняется. Вероятно включена блокировка выполнения регламентных заданий.'");
		...
		КонецЕсли;
	Иначе
		ТекстСообщения = НСтр("ru = 'Дополнительные процедуры обработки данных не выполняются,'") + Символы.ПС +
			НСтр("ru = 'т.к. отключено регламентное задание <b>Отложенное обновление</b>.'");
		Сообщения.Добавить(СтрСоединить(СтрРазделить(ТекстСообщения, Символы.ПС), " "));
