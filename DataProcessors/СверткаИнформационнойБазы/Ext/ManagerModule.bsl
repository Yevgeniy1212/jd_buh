	...
	Пока ВыборкаПоОрганизации.Следующий() Цикл
		...
		Пока ВыборкаПоСчетам.Следующий() Цикл
			...
			Если (НЕ ВыборкаПоСчетам.СуммаОстаток = 0) ИЛИ (НЕ ВыборкаПоСчетам.ВалютнаяСуммаОстаток = 0) ИЛИ (НЕ ВыборкаПоСчетам.КоличествоОстаток = 0) 
				...
				ДокументОбъект.Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("*Сформирован обработкой свертки базы. %1", Формат(ТекущаяДатаСеанса(),"ДФ=dd.MM.yyyy"));
				ДокументОбъект.Содержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ввод остатков по регистрам бухгалтерии: Типовой. Счет: %1'", ОбщегоНазначения.КодОсновногоЯзыка()), ВыборкаПоСчетам.Счет.Код);
	...
	Пока ВыборкаПоОрганизации.Следующий() Цикл
		...
		Пока ВыборкаПоСчетам.Следующий() Цикл
			...
			Если (НЕ ВыборкаПоСчетам.СуммаОстаток = 0) ИЛИ (НЕ ВыборкаПоСчетам.КоличествоОстаток = 0) 
				...
				ДокументОбъект.СпособЗаполнения = "Вручную"; 
				ДокументОбъект.Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("*Сформирован обработкой свертки базы. %1", Формат(ТекущаяДатаСеанса(),"ДФ=dd.MM.yyyy"));
				ДокументОбъект.Содержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ввод остатков по регистрам бухгалтерии: Налоговый. Счет: %1'", ОбщегоНазначения.КодОсновногоЯзыка()), ВыборкаПоСчетам.Счет.Код);
...
Функция СформироватьШапкуДокументаВводаОстатков(ТекущийРегистр, ДатаСвертки, ТипРегистра, Организация = Неопределено)
	...
	ДокументОстатков.Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("*Сформирован обработкой свертки базы. %1", Формат(ТекущаяДатаСеанса(),"ДФ=dd.MM.yyyy"));
	ДокументОстатков.Содержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ввод остатков по регистрам %1 : %2'",ОбщегоНазначения.КодОсновногоЯзыка()), ТипРегистра, ТекущийРегистр.Имя);
...
Процедура ПередКомпоновкойОтчетаПоРегиструБухгалтерии(Настройки,ПараметрыОтчета)
	...
	ГруппаОстаткиДоСвертки.Заголовок     = НСтр("ru = 'Остатки до свертки'");
	...
	ГруппаОстаткиПослеСвертки.Заголовок     = НСтр("ru = 'Остатки после свертки'");
	...
	ГруппаПроверка.Заголовок     = НСтр("ru = 'Проверка'");
...
Функция ОбновитьТекстЗаголовкаНаСервере(ПериодСвертки) Экспорт
	...
	ЗаголовокОтчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Проверка созданных документов %1'"), БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(ПериодСвертки, КонецГода(ПериодСвертки)));
