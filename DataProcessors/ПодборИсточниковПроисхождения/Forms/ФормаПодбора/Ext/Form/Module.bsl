...
Процедура ОбновитьИнформационнуюНадписьПараметрыВыбора(Форма)
	...
	Для Каждого Элемент Из ЭлементыОтбора Цикл
		...
		ИначеЕсли Элемент.Использование И Элемент.Параметр = Новый ПараметрКомпоновкиДанных("Организация") Тогда
			ТекстНадписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 <%2> - не указана;'"), ТекстНадписи, Элемент.Параметр); 
		ИначеЕсли Элемент.Использование Тогда
			ТекстНадписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 <%2> - не указан;'"), ТекстНадписи, Элемент.Параметр);
		КонецЕсли;
	КонецЦикла;
