...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого Напоминание Из Напоминания Цикл
		...
		Если Напоминание.ТипСообщения = Перечисления.СообщенияДляРегОтчетов.УплатаНалога Тогда
			События[Индекс].ТипСообщения = ТипСообщенияУплатаНалога();
		...
		ИначеЕсли Напоминание.ТипСообщения = Перечисления.СообщенияДляРегОтчетов.СдачаОтчета Тогда
			События[Индекс].ТипСообщения = ТипСообщенияСдачаОтчета();
	...
	ЭтаФорма.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Напоминания календаря бухгалтера: %1 г., %2'"),
		Формат(ДатаНапоминания, "ДФ='dd ММММ yyyy';ДЛФ=ДД"),
		Формат(ДатаНапоминания, "ДФ='дддд'"));
...
Процедура ОткрытьПоСтроке(ВыбраннаяСтрока)
	...
	Если ВыбраннаяСтрока.ТипСообщения = ТипСообщенияУплатаНалога() Тогда
	...
	Если Найти(ВыбраннаяСтрока.Периодичность, ПериодичностьМесячная()) Тогда
		Периодичность = ПеречислениеПериодичностьМесяц;
	ИначеЕсли Найти(ВыбраннаяСтрока.Периодичность, ПериодичностьКвартальная()) Тогда
...
Процедура ОткрытьСобытие(Команда)
	...
	Если Элементы.События.ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Нет выбранного события'"));
		Возврат;
	КонецЕсли;
...
Процедура ОтменитьСобытие(Команда)
	...
	РежимПлатежа = Элементы.События.ТекущиеДанные.ТипСообщения = ТипСообщенияУплатаНалога();
...
Функция ТипСообщенияУплатаНалога()
	Возврат НСтр("ru = 'Уплата налога'")
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ТипСообщенияСдачаОтчета()
	Возврат НСтр("ru = 'Сдача отчета'")
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ПериодичностьМесячная()
	Возврат НСтр("ru = 'Месячная'")
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ПериодичностьКвартальная()
	Возврат НСтр("ru = 'Квартальная'")
КонецФункции
