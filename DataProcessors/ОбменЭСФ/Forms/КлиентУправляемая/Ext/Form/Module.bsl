...
//
&НаКлиенте
Функция ТипКлюча(Знач КлючBase64, Знач ПарольОтКлюча, Знач ДанныеКлючаЭЦП = Неопределено) Экспорт
	...
	Если ЭСФВызовСервера.ИспользоватьПодписьНовойКомпоненты() И ЭСФВызовСервера.ИспользоватьВнешнююКриптографиюДляКомпоненты() Тогда
		...
		Если ДанныеКлючаЭЦП = Неопределено ИЛИ ТипЗнч(ДанныеКлючаЭЦП) <> Тип("Структура") Тогда
		...
		Если ЭСФВызовСервераПовтИсп.ВыполнятьКриптографическиеОперацииНаКлиенте() Тогда
			ОтветСтруктура = ЭСФКлиентСервер.ПолучитьДанныеСертификатаВнешняяКриптография("000000000000", "1CService", ДополнительныеПараметры, ПараметрыКлюча);
		Иначе
...
Процедура ОбновитьМодульКриптографии(ПараметрыАвторизации, ВыполняемоеОповещение = Неопределено, ТихийРежим = Ложь, Организация = Неопределено) Экспорт
	...
	Если ЭлектронноеВзаимодействиеССервисамиКлиент.ПроверитьНаличиеОбновлений(СвойстваМодуляДляТекущегоРелиза, ШапкаОшибки) = Ложь Тогда
...
Функция ЗагрузитьСтрокуXMLВДеревоЗначений(СтрокаXML, ОписаниеОшибки = Неопределено, ЧтениеXML = Неопределено, Знач ТекУзел = Неопределено) Экспорт
	...
	Попытка
		...
		КонецЦикла;
	Исключение
		ОписаниеОшибки = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка разбора XML: %1.'"), ИнформацияОбОшибке().Описание);
		Возврат Неопределено;
	КонецПопытки;
...
Процедура ПерезаполнитьЭСФ(Знач Параметр) Экспорт
	...
	Если СоответствиеДляОбновленияАктуальныхЭСФ.Количество() = МассивСчетовФактур.Количество() Тогда
		...
		ИначеЕсли СоответствиеДляОбновленияАктуальныхЭСФ.Количество() = 1 Тогда
			ТекстВопроса = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Перезаполнить ""%1"" на основании счета-фактуры?'"),СоответствиеДляОбновленияАктуальныхЭСФ[МассивСчетовФактур[0]]);
	...
	ИначеЕсли СоответствиеДляОбновленияАктуальныхЭСФ.Количество() <> 0 Тогда
		ТекстВопроса = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Невозможно перезаполнить все электронные счета-фактуры.
             |
             |Всего счетов-фактур: %1 шт.
             |Возможно перезаполнить счетов-фактур: %2 шт.
             |
             |Перезаполнить электронные счета-фактуры, которые возможно перезаполнить?'"),МассивСчетовФактур.Количество(),СоответствиеДляОбновленияАктуальныхЭСФ.Количество());
		ПоказатьВопрос(ПерезаполнитьЭСФЗавершение, ТекстВопроса, РежимДиалогаВопрос.ОКОтмена);		
	КонецЕсли;
...
Процедура ОбновитьЭСФ(Знач Параметр, Знач ЗапускатьФоновоеЗадание) Экспорт
	...
	Для Каждого ЭлементСоответствия Из СоответствиеАктуальныхЭСФ Цикл
		...
		Иначе
			...
			ТекстСообщения = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Документ ""%1"" не имеет связанного электронного счета-фактуры.'"),ЭлементСоответствия.Ключ);
...
Процедура ОткрытьЭСФ(Знач Параметр) Экспорт
	...
	ИначеЕсли МассивСчетовФактур.Количество() > 10 Тогда
		ТекстВопроса = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Будет открыто форм электронных счетов-фактур: %1.
			|Вы действительно хотите продолжить?'"),МассивСчетовФактур.Количество());
		ОписаниеОповещения = Новый ОписаниеОповещения("ОбновитьМодульКриптографииЗавершение", ЭтаФорма, ДополнительныеПараметры);
...
Процедура ОткрытьЭСФЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Тогда
		...
		Для Каждого ЭлементКоллекции Из КоллецияАктуальныхЭСФ Цикл
			...
			Иначе
				ТекстСообщения = ТекстСообщения + ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Документ ""%1"" не имеет актуального электронного счета-фактуры.'"),ЭлементКоллекции.Ключ) + Символы.ПС;
			КонецЕсли;
