...
Функция ЗаполнитьИтогиПоОчередиЭСФ()
	...
	Если ИспользуютсяФоновыеЗадания И ИнформационнаяБазаСерверная Тогда
		...
		Если ИтоговыеДанныеФоновыхЗаданий.Следующий() Тогда
			...
			ИнформационнаяНадпись = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Выполняется отправка %1 ЭСФ, %2 пользователем(ми) (%3 фоновых заданий):
											| - %4 ЭСФ в процессе подписи ЭЦП пользователем(ми);
											| - %5 ЭСФ в процессе отправки на сервер ИС ЭСФ; 
											| - отправка %6 ЭСФ завершилась сбоем.
											|При попытке повторной отправки:
											...
											КоличествоЭСФ,КоличествоФоновыхПользователей,КоличествоФоновыхЗаданий,КоличествоНаПодписи,КоличествоАктивных,КоличествоЗавершенныхПоОшибке);
