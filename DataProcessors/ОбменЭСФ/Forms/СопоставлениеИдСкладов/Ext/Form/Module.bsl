...
Процедура ВопросПередСопоставлением(ПропускатьЗаполненные = Ложь)
	...
	Иначе
		...
		ПоказатьВопрос(ПродолжениеСопоставления, НСтр("ru='Вы собираетесь произвести сопоставление для всех складов, включая те, для которых уже выбраны виртуальные склады. Продолжить?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
...
Функция ДанныеПрофилейДляПолученияСкладовНаСервереПоРезультатуВыбораПрофилей(Знач Результат)
	...
	Для Каждого Элемент Из Результат Цикл
		...
		Если МетодKalkan Тогда
			...
			ДанныеПрофиляИСЭСФ.Вставить("АдресКлюча", Элемент.Значение.АдресКлючаВхода);
		КонецЕсли;
...
Процедура ТаблицаСкладовПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	Если ЗначениеЗаполнено(Элемент.ТекущиеДанные.Склад) И ЗначениеЗаполнено(Элемент.ТекущиеДанные.Организация) Тогда
		...
		Если НайденныеСтроки.Количество() > 1 И Не ОтменаРедактирования Тогда
			СообщениеПользователю = НСтр("'ru = 'Указанное сочетание Организация - Склад уже присутствует в таблице. Пожалуйста, выберите другую организацию или склад.'");
...
Процедура Готово(Команда)
	...
	Если ЕстьОшибки Тогда
		...
		ПоказатьВопрос(ПродолжениеЗавершения, НСтр("ru='В таблице присутствуют строки с незаполненными полями. Строки с незаполненными полями не будут записаны. Продолжить?'"), РежимДиалогаВопрос.ДаНет);	
	Иначе
...
Процедура Далее(Команда)
	...
	Если Элементы.ГруппаШапка.ТекущаяСтраница = Элементы.ПолучениеСкладов Тогда
		...
		Если ОрганизацииСОтсутствующимиСкладамиВС.Количество() > 0 Тогда
			...
			СообщениеПользователю = НСтр("ru = 'У организаций %1 отсутствуют виртуальные склады в учетной системе, произведите получение виртуальных складов нажав на кнопку ""Получить виртуальные склады"" либо создайте их самостоятельно.'");
