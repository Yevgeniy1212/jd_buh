...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого СтрокаСписокЭСФ Из СписокЭСФ Цикл
		...
		Если ПустаяСтрока(СтрокаСписокЭСФ.Идентификатор) Тогда
			...
			Сообщение.Текст = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'В строке %1 не заполнено поле ""Идентификатор"".'"),
		...
		Иначе
			...
			Если НЕ ЭСФКлиентСервер.ИдентификаторКорректен(СтрокаСписокЭСФ.Идентификатор, ТекстСообщения) Тогда 
				...
				Сообщение.Текст = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'В строке %1 поле ""Идентификатор"" заполнено некорректно. Причина: %2'"),
				Формат(НомерСтроки, "ЧГ="), ТекстСообщения); 
		...
		Если Действие = ЭСФКлиентСервер.ДействиеОтклонение() Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				Если Выборка.Вид <> Перечисления.ВидыЭСФ.Дополнительный
					...
					ТекстСообщения = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Невозможно выполнить действие, так как ""%1"" не является дополнительным, исправленным или отозванным.'"),
						Выборка.ЭСФ);		
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Направление <> Направление Тогда
			...
			Если ПоказыватьСообщения Тогда 
				ТекстСообщения = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Невозможно выполнить действие для документа ""%1"", так как он %2.'"),
				Выборка.ЭСФ, НРег(Выборка.Направление));
				ЭСФКлиентСервер.СообщитьПользователю(ТекстСообщения);
			КонецЕсли;
