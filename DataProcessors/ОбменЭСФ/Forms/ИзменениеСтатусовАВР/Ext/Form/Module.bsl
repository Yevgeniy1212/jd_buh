	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Направление <> Направление Тогда
			...
			Если ПоказыватьСообщения Тогда 
				ТекстСообщения = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Невозможно выполнить действие для документа ""%1"", так как он %2.'"),Выборка.АВР, НРег(Направление));
				ЭСФКлиентСервер.СообщитьПользователю(ТекстСообщения);
			КонецЕсли;
