...
Функция ТекстИнструкцииИзМакета(ИмяМакета, Домен)
	...
	Если НЕ Домен = 0 Тогда			
		ЛокализованноеИмяМакета = ИмяМакета + "_EU";
		Макет = Обработки.МониторПортала1СИТС.ПолучитьМакет(ЛокализованноеИмяМакета); 		
	Иначе		
		ЛокализованноеИмяМакета = ОбщегоНазначенияБК.ПолучитьИмяМакетаНаТекущемЯзыкеПользователя("Обработка.МониторПортала1СИТС", ИмяМакета);
	...
	ТекстИнструкции = Макет.ПолучитьТекст();
	
	#Если НЕ (ТолстыйКлиентОбычноеПриложение ИЛИ ТолстыйКлиентУправляемоеПриложение ИЛИ ВебКлиент) Тогда
	
	Если ПустаяСтрока(ТекстИнструкции) Тогда
		Макет.КодЯзыкаМакета = ОбщегоНазначения.КодОсновногоЯзыка();
		ТекстИнструкции = Макет.ПолучитьТекст();
	КонецЕсли;
	...
	ФДок = Новый ФорматированныйДокумент;
	ФДок.УстановитьHTML(ТекстИнструкции, Новый Структура);
	Возврат ФДок.ПолучитьФорматированнуюСтроку();
