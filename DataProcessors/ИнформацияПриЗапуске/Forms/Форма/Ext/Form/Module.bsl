...
Функция ПодготовитьДанныеФормы()
	...
	Если ИспользоватьКэшРегистра Тогда
		...
		#Область Локализация_РК		
		Если НЕ ПакетыСтраниц = Неопределено И НЕ ПакетыСтраниц.Колонки.Найти("КодЯзыка") = Неопределено Тогда
			Если ПакетыСтраниц.Количество() > 0 Тогда
				КодЯзыкаМакета = ПакетыСтраниц[0].КодЯзыка;	
				
				Если НЕ КодЯзыкаМакета = ТекущийЯзык().КодЯзыка Тогда
					ИспользоватьКэшРегистра = Ложь;		
				КонецЕсли; 			
			КонецЕсли;
		КонецЕсли;
