...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если НужноПечататьМакет Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			Киб_ПечатьДА4(МассивОбъектов, ОбъектыПечати),
			//Евгений-
			,
...
Функция ПечатьНакладная(МассивОбъектов, ОбъектыПечати, КодЯзыка=Неопределено) Экспорт
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ПеремещениеОС.ПФ_MXL_Накладная", КодЯзыка);
	...
	Пока ШапкаДокумента.Следующий() Цикл
		...
		ТекстНакладнаяНаПеремещениеОС 							 = НСтр("ru = 'Накладная на перемещение ОС'", КодЯзыка);
		ОбластьМакетаЗаголовок.Параметры.ТекстЗаголовок 		 = РаботаСДиалогами.СформироватьЗаголовокДокумента(ШапкаДокумента.Ссылка, ТекстНакладнаяНаПеремещениеОС,,,,КодЯзыка);
...
Функция ПечатьДА4(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Пока ШапкаДокумента.Следующий() Цикл
		...
		Шапка.Параметры.ДатаДокумента = ?(ЗначениеЗаполнено(ШапкаДокумента.ДатаДокумента),
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 года'", КодЯзыка), Формат(ШапкаДокумента.ДатаДокумента, "Л=" + КодЯзыка + "; ДФ='дд ММММ гггг'")),
			НСтр("ru = '""_____""________________________ 20_____года'", КодЯзыка));
		...
		Пока ВыборкаСтрокОС.Следующий() Цикл
			...
			СтрокаТаблицы.Параметры.ЕдиницаИзмерения = НСтр("ru = 'шт'", КодЯзыка);
		...
		Если КоличествоСтрокОС <> 0 Тогда
			Итого.Параметры.КоличествоНаименований = СтрЗаменить(ЧислоПрописью(КоличествоСтрокОС, "L=" + КодЯзыка, ",,,,,,,,0"), "00", "");
		...
		Для Каждого СтрокаМОЛ Из ДанныеМОЛ Цикл
			МОЛ.Параметры.ЗаголовокМОЛ		 = ?(ВыводитьЗаголовок, НСтр("ru = 'Сдал:'", КодЯзыка), "");
		...
		Для Каждого СтрокаМОЛ Из ДанныеМОЛ Цикл
			МОЛ.Параметры.ЗаголовокМОЛ		 = ?(ВыводитьЗаголовок, НСтр("ru = 'Принял:'", КодЯзыка), "");
			МОЛ.Параметры.Должность			 = СтрокаМОЛ.Должность;
			МОЛ.Параметры.РасшифровкаПодписи = СтрокаМОЛ.Представление;
...
// Евгений+
Функция Киб_ПечатьДА4(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ПеремещениеОС.СтруктурноеПодразделениеОтправитель КАК СтруктурноеПодразделение,
	               |	ПеремещениеОС.СтруктурноеПодразделениеПолучатель КАК СтруктурноеПодразделениеПолучатель,
	               |	ПеремещениеОС.Ответственный.ФизЛицо КАК Ответственный,
	               |	ПеремещениеОС.Ссылка КАК Ссылка,
	               |	ПеремещениеОС.НачальноеЗаполнениеАналитикиНаСчетахУчетаОС КАК НачальноеЗаполнениеАналитикиНаСчетахУчетаОС,
	               |	ПеремещениеОС.Подписант КАК Подписант,
	               |	ПеремещениеОС.ДолжностьПодписанта КАК ДолжностьПодписанта
	               |ИЗ
	               |	Документ.ПеремещениеОС КАК ПеремещениеОС
	               |ГДЕ
	               |	ПеремещениеОС.Ссылка В(&Ссылка)
	...
	МОЛ				 		  = Макет.ПолучитьОбласть("МОЛ");
	ПодписьГлавногоБухгалтера = Макет.ПолучитьОбласть("ПодписьГлавногоБухгалтера");
    киб_Солидарный            = Макет.ПолучитьОбласть("киб_Солидарный");
	Пока ШапкаДокумента.Следующий() Цикл
		...
		Шапка.Параметры.ДатаДокумента = ?(ЗначениеЗаполнено(ШапкаДокумента.ДатаДокумента), Формат(ШапкаДокумента.ДатаДокумента, "ДФ=dd.MM.yyyy"), Стр);
		...
		Для Каждого СтрокаМОЛ Из ДанныеМОЛ Цикл
			...
			МОЛ.Параметры.Должность			 = СтрокаМОЛ.Должность;
			МОЛ.Параметры.РасшифровкаПодписи = СтрокаМОЛ.Представление;
			МОЛ.параметры.ДатаДокумента      = Формат(ШапкаДокумента.ДатаДокумента,"ДФ=""дд ММММ гггг 'года'""");
			ТабДокумент.Вывести(МОЛ);
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ПеремещениеОССолидарныеМол.Ссылка КАК Ссылка,
		|	ПеремещениеОССолидарныеМол.ФизЛицо КАК ФизЛицо,
		|	ПеремещениеОССолидарныеМол.Ссылка.СолидарноеМолДата КАК ДатаСолМол,
		|	ПеремещениеОССолидарныеМол.Ссылка.Дата КАК Дата
		|ИЗ
		|	Документ.ПеремещениеОС.СолидарныеМол КАК ПеремещениеОССолидарныеМол
		|ГДЕ
		|	ПеремещениеОССолидарныеМол.Ссылка В(&Ссылка)
		|АВТОУПОРЯДОЧИВАНИЕ";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		ДанныеМол = ПроцедурыУправленияПерсоналомВызовСервера.ДанныеФизЛица(ШапкаДокумента.Организация, ВыборкаДетальныеЗаписи.ФизЛицо, ШапкаДокумента.ДатаДокумента);
		киб_Солидарный.Параметры.СолМол = ВыборкаДетальныеЗаписи.ФизЛицо;
		киб_Солидарный.Параметры.ДолжностьСолМол = ДанныеМол.Должность; 
		киб_Солидарный.Параметры.ДатаДокумента = Формат(ВыборкаДетальныеЗаписи.Дата,"ДФ=""дд ММММ гггг 'года'""");   
		киб_Солидарный.Параметры.ДатаСолМол = Формат(ВыборкаДетальныеЗаписи.ДатаСолМол,"ДФ=""дд ММММ гггг 'года'"""); 
		ТабДокумент.Вывести(киб_Солидарный);
	КонецЦикла;
...
КонецЕсли;
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ПеремещениеОССолидарныеМол1.Ссылка КАК Ссылка,
		|	ПеремещениеОССолидарныеМол1.ФизЛицо КАК ФизЛицо,
		|	ПеремещениеОССолидарныеМол1.Ссылка.СолидарноеМолДата1 КАК ДатаСолМол,
		|	ПеремещениеОССолидарныеМол1.Ссылка.Дата КАК Дата
		|ИЗ
		|	Документ.ПеремещениеОС.СолидарныеМол1 КАК ПеремещениеОССолидарныеМол1
		|ГДЕ
		|	ПеремещениеОССолидарныеМол1.Ссылка В(&Ссылка)
		|АВТОУПОРЯДОЧИВАНИЕ";
...
КонецЕсли;
		...
		НужнаяДолжность = Перечисления.ОтветственныеЛицаОрганизаций.БухгалтерПоОС;
		
		Если НЕ ЗначениеЗаполнено(ШапкаДокумента.Подписант) Тогда
			Если НЕ ЗначениеЗаполнено(ШапкаДокумента.ДолжностьПодписанта) Тогда
				ПодписьГлавногоБухгалтера.Параметры.ДолжностьПодписанта = Перечисления.ОтветственныеЛицаОрганизаций.БухгалтерПоОС;
			Иначе
				ПодписьГлавногоБухгалтера.Параметры.ДолжностьПодписанта = ШапкаДокумента.ДолжностьПодписанта;
				НужнаяДолжность = ШапкаДокумента.ДолжностьПодписанта;
			КонецЕсли;
			ПодписьГлавногоБухгалтера.Параметры.ДолжностьПодписанта = НужнаяДолжность;
			
			ЗапросОтветЛица = Новый Запрос;
			ЗапросОтветЛица.Текст = "ВЫБРАТЬ
			|	ОтветственныеЛицаОрганизацийСрезПоследних.ФизическоеЛицо
			|ИЗ
			|	РегистрСведений.ОтветственныеЛицаОрганизаций.СрезПоследних(&Дата, ОтветственноеЛицо = &НужнаяДолжность) КАК ОтветственныеЛицаОрганизацийСрезПоследних";
			
			ЗапросОтветЛица.УстановитьПараметр("НужнаяДолжность", НужнаяДолжность);
			ЗапросОтветЛица.УстановитьПараметр("Дата", ШапкаДокумента.ДатаДокумента);
			
			РезультатОтветЛица = ЗапросОтветЛица.Выполнить().Выбрать();
			
			Если РезультатОтветЛица.Количество() > 0 Тогда
				РезультатОтветЛица.Следующий();
				...
				|ИЗ
				|	РегистрСведений.ФИОФизЛиц.СрезПоследних(&Дата, ФизЛицо = &Подписант) КАК ФИОФизЛицСрезПоследних";
				
				ЗапросФИО.УстановитьПараметр("Подписант", РезультатОтветЛица.ФизическоеЛицо);
				ЗапросФИО.УстановитьПараметр("Дата", ШапкаДокумента.ДатаДокумента);
				
				РезультатФИО = ЗапросФИО.Выполнить().Выбрать();
				РезультатФИО.Следующий();
				
				ИмяПодписанта = РезультатФИО.Фамилия + " " + Лев(РезультатФИО.Имя, 1) + "." + Лев(РезультатФИО.Отчество, 1) + ".";
				ПодписьГлавногоБухгалтера.Параметры.РасшифровкаПодписи = ИмяПодписанта;
			КонецЕсли;
		Иначе
			ЗапросФИО = Новый Запрос;
			ЗапросФИО.Текст = "ВЫБРАТЬ
			...
			ЗапросФИО.УстановитьПараметр("Подписант", ШапкаДокумента.Подписант);
			ЗапросФИО.УстановитьПараметр("Дата", ШапкаДокумента.ДатаДокумента);
			
			РезультатФИО = ЗапросФИО.Выполнить().Выбрать();
			
			
			Если РезультатФИО.Следующий() Тогда
				ИмяПодписанта = РезультатФИО.Фамилия + " " + Лев(РезультатФИО.Имя, 1) + "." + Лев(РезультатФИО.Отчество, 1) + ".";
			Иначе
				ИмяПодписанта = ОбщегоНазначенияБК.ФамилияИнициалыФизЛица( ШапкаДокумента.Подписант);
			КонецЕсли; ;
		...
		ПодписьГлавногоБухгалтера.Параметры.ДатаПодписи  = Формат(ШапкаДокумента.ДатаДокумента, "ДФ=""дд ММММ гггг 'года'""");
