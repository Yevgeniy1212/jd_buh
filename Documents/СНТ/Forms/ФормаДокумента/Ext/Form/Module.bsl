...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбновитьПредставлениеКодаКлассификатора();
...
ПоддержкаРаботыСоСтруктурнымиПодразделениями = ВССерверПереопределяемый.ПолучитьПараметрРаботыСоСтруктурнымиПодразделениями();
	...
	СНТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаРедактироватьНедоступныеРеквизиты", "Доступность", ?(ЭтаФорма.ТолькоПросмотр, Ложь, Истина));
	...
	Если Объект.Направление = ПредопределенноеЗначение("Перечисление.НаправленияСНТ.Входящий") Тогда
		Элементы.НомерСНТ.Заголовок = НСтр("ru='Номер СНТ учетной системы (поставщика)'");
	КонецЕсли;
	...
	Для Каждого СтрокаСоответствия Из СоответствиеНомеровТаблицИНазванийСНТ Цикл
		...
		Если ЭлементНомерСтрокиОсновнойСНТ <> Неопределено Тогда
			ЭлементНомерСтрокиОсновнойСНТ.Заголовок = ?(Объект.ТипСНТ = Перечисления.ТипыСНТ.ВозвратТоваров, НСтр("ru='№ п/п осн. СНТ'"), НСтр("ru='№ п/п СНТ'"));
		КонецЕсли;
	КонецЦикла;
	...
	Если СНТСерверПереопределяемый.ВведенНовыйРазделК() Тогда
		Если Объект.Дата < СНТСерверПереопределяемый.ДатаВводаРазделаКВерсии1_2() Тогда
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаГруз", "Видимость", Истина);
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаСведенияОГрузе1_2", "Видимость", Ложь);
		Иначе
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаГруз", "Видимость", Ложь);
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаСведенияОГрузе1_2", "Видимость", Истина);
		КонецЕсли;	
		
		Если ЗначениеЗаполнено(Объект.СвязанныйСНТ) Тогда
			Если Объект.СвязанныйСНТ.Дата < СНТСерверПереопределяемый.ДатаВводаРазделаКВерсии1_2() Тогда
				ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаГруз", "Видимость", Истина);
				ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаСведенияОГрузе1_2", "Видимость", Ложь);
			Иначе
				ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаСведенияОГрузе1_2", "Видимость", Истина);
	...
	Если СНТСерверПереопределяемый.ВесНеттоИспользуется() Тогда		
		Если Объект.Дата < СНТСерверПереопределяемый.ДатаВводаВесаНетто() Тогда
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыВСВесНетто", "Видимость", Ложь);
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыВесНетто", "Видимость", Ложь);
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЭтиловыйСпиртВесНетто", "Видимость", Ложь);
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ВиноматериалВесНетто", "Видимость", Ложь);
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПивоПивныеНапиткиВесНетто", "Видимость", Ложь);
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "АлкогольнаяПродукцияКромеПиваВесНетто", "Видимость", Ложь);
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДанныеПоНефтепродуктамВесНетто", "Видимость", Ложь);
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДанныеПоБиотопливуВесНетто", "Видимость", Ложь);
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТабачнаяПродукцияВесНетто", "Видимость", Ложь);
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДругиеТоварыЦифроваяМаркировкаВесНетто", "Видимость", Ложь);
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыЭкспортныйКонтрольВесНетто", "Видимость", Ложь); 
		Иначе
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыВСВесНетто", "Видимость", Истина);
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыВесНетто", "Видимость", Истина);
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЭтиловыйСпиртВесНетто", "Видимость", Истина);
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ВиноматериалВесНетто", "Видимость", Истина);
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПивоПивныеНапиткиВесНетто", "Видимость", Истина);
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "АлкогольнаяПродукцияКромеПиваВесНетто", "Видимость", Истина);
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДанныеПоНефтепродуктамВесНетто", "Видимость", Истина);
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДанныеПоБиотопливуВесНетто", "Видимость", Истина);
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТабачнаяПродукцияВесНетто", "Видимость", Истина);
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДругиеТоварыЦифроваяМаркировкаВесНетто", "Видимость", Истина);
			ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыЭкспортныйКонтрольВесНетто", "Видимость", Истина);
		КонецЕсли;
	Иначе
		ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыВСВесНетто", "Видимость", Ложь);
		ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыВесНетто", "Видимость", Ложь);
...
Процедура ИзменитьКоманднуюПанельДокумента()
	...
	ИначеЕсли Объект.Направление = Перечисления.НаправленияСНТ.Исходящий Тогда
		...
		Если Объект.ТипСНТ = Перечисления.ТипыСНТ.ВозвратТоваров И ЗначениеЗаполнено(Объект.СвязанныйСНТ) Тогда
			Если Объект.СвязанныйСНТ.ВидОперации = Перечисления.ВидыОперацийСНТ.Ввоз Тогда
				СНТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаСоздатьЭСФИСФ", "Видимость", Ложь);
			КонецЕсли;
		КонецЕсли;
...
Процедура ВидВвозаПриИзменении(Элемент)
	...
	Если Объект.ВидВвоза <> ПредопределенноеЗначение("Перечисление.ВидыВвозаСНТ.Импорт")
		И Объект.ВидВвоза <> ПредопределенноеЗначение("Перечисление.ВидыВвозаСНТ.ВвозТоваровНаТерриториюСЭЗ") Тогда
...
Процедура ВидВывозаПриИзменении(Элемент)
	...
	Если Объект.ВидВывоза <> ПредопределенноеЗначение("Перечисление.ВидыВывозаСНТ.Экспорт")
		И Объект.ВидВывоза <> ПредопределенноеЗначение("Перечисление.ВидыВывозаСНТ.ВывозТоваровСТерриторииСЭЗ") Тогда
		Объект.УникальныйНомерВалютногоКонтроля = "";
	КонецЕсли;
...
Функция ДанныеДляСозданияОбновленияДоговораЗаполнены(ИмяРеквизитаКонтрагента)
	...
	Если НЕ ЗначениеЗаполнено(ИмяРеквизитаКонтрагента) Тогда
		...
		ТекстСообщения = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Невозможно выполнить действие, так как раздел "" %1"" является пустым.'"),СинонимРаздела);
	...
	Если НЕ ЗначениеЗаполнено(ИмяРеквизитаКонтрагента) Тогда
		...
		ТекстСообщения = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Невозможно выполнить действие, так как в разделе ""%1"" не заполнено поле %2.'"),СинонимРаздела, ИмяРеквизитаКонтрагента);
		Сообщить(ТекстСообщения);
...
Функция СтрокаКонтрагент(Знач Объект)
	...
	ИначеЕсли Объект.Направление = ПредопределенноеЗначение("Перечисление.НаправленияСНТ.Исходящий") 
		...
		И (Объект.ВидВвоза = ПредопределенноеЗначение("Перечисление.ВидыВвозаСНТ.Импорт") ИЛИ Объект.ВидВвоза = ПредопределенноеЗначение("Перечисление.ВидыВвозаСНТ.ВвозТоваровНаТерриториюСЭЗ")) Тогда
		ИмяКонтрагента = "Поставщик";
	Иначе
...
&НаСервере
Процедура ОбновитьПредставлениеКодаКлассификатора() Экспорт
...
Процедура ЗаполнитьРеквизитДоговорПоставкиПоДаннымСНТ()
	...
	Если ПоказатьСообщение Тогда
		...
		Если ЗначениеЗаполнено(Контрагент) Тогда
			ТекстСообщения = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Поле ""Договор поставки"" в разделе ""F. Договор (контракт)"" было очищено, так как указанный договор не принадлежал контрагенту ""%1"".'"),Контрагент);
		Иначе
...
&НаКлиенте
Процедура ТоварыСоздатьТоварыG10(Команда)
	
	ТекущиеДанные = Элементы.ТоварыЭкспортныйКонтроль.ТекущиеДанные;
...
&НаКлиенте
Процедура ТоварыСоздатьТоварыG9(Команда)
	
	ТекущиеДанные = Элементы.ДругиеТоварыЦифроваяМаркировка.ТекущиеДанные;
...
Процедура ПеренестиТоварВТаблицу(Команда)
	...
	ТабличнаяЧасть = ТабличныеЧасти.ВыбратьЭлемент(НСтр("ru='Перенести в таблицу'"), ТекущийЭлемент);
	...
	Если ТипЗнч(ВыбранноеЗначение) <> Тип("Структура") Тогда
		...
		Если ИзменитьДокументОснование Тогда
			...
			Иначе
				...
				ТекстВопросаВыборОснования = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр(
				"ru = 'На основании документа ""%1"" уже создан СНТ. 
				|Привязать текущий СНТ к выбранному документу без отсоединения ранее связанного СНТ?'"),ВыбранноеЗначение);
...
Процедура УстановитьВидимостьЭлементовДоговораПоставки()
	...
	Элементы.УникальныйНомерВалютногоКонтроля.Видимость = Не Объект.БезДоговора;
...
Процедура ЗаполнитьДанныеПоДоговору() Экспорт
	...
	Если Элементы.ФормаРедактироватьНедоступныеРеквизиты.Пометка Тогда
		...
		Если Объект.ВидОперации = Перечисления.ВидыОперацийСНТ.Ввоз ИЛИ Объект.ВидОперации = Перечисления.ВидыОперацийСНТ.Вывоз Тогда
...
Процедура ПоставщикПриИзмененииНаСервере()
	...
	Если Элементы.ФормаРедактироватьНедоступныеРеквизиты.Пометка Тогда
		...
		Если Объект.ВидОперации = Перечисления.ВидыОперацийСНТ.Ввоз Тогда
			ЗаполнитьЗначенияСвойств(Объект, СтруктураДанных, "ПоставщикНаименование, ПоставщикНерезидент, АдресОтправки");
		Иначе	
...
Процедура ДоговорПоставкиПриИзмененииНаСервере()
	...
	Если Элементы.ФормаРедактироватьНедоступныеРеквизиты.Пометка Тогда
		...
		Если Объект.ВидОперации = Перечисления.ВидыОперацийСНТ.Ввоз ИЛИ Объект.ВидОперации = Перечисления.ВидыОперацийСНТ.Вывоз Тогда
			...
			"ДоговорПоставкиНомер, ДоговорПоставкиДата, ДоговорПоставкиУсловияОплаты, ДоговорПоставкиУсловияПоставки, УникальныйНомерВалютногоКонтроля");
		Иначе
			ЗаполнитьЗначенияСвойств(Объект, СтруктураДанных, 
...
Процедура ОбработкаЗаписиНового(НовыйОбъект, Источник, СтандартнаяОбработка)
	...
	Если ТипЗнч(НовыйОбъект) = Тип("СправочникСсылка.Контрагенты") Тогда
		...
		Если ТипЗнч(НовыйОбъект) = Тип("СправочникСсылка.Контрагенты") Тогда
			Объект.Поставщик = НовыйОбъект;
		Иначе // ТипЗнч(НовыйОбъект) = Тип("СправочникОбъект.Контрагенты")
			Объект.Поставщик = НовыйОбъект.Ссылка;
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(НовыйОбъект) = Тип("СправочникСсылка.Номенклатура") Тогда
		...
		ТекНаимГруппы = Элементы.ГруппаТоварыСтраницы.ТекущаяСтраница.Имя;
		ТЧСНТ = ОпределитьТЧПоГруппе(ТекНаимГруппы);//определяются все кроме G2-G5(Группа5- "Данные по алкогольной продукции")
		ТекущиеДанные = Элементы[ТЧСНТ].ТекущиеДанные;
		
		Если ТипЗнч(НовыйОбъект) = Тип("СправочникСсылка.Номенклатура") Тогда
			ТекущиеДанные.Товар = НовыйОбъект;
		Иначе // ТипЗнч(НовыйОбъект) = Тип("СправочникОбъект.Номенклатура")
