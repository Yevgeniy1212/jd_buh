...
/// Пересчитываем срок службы в табличной части	
	Для Каждого СтрокаТаб Из Объект.Товары Цикл
		СтрокаТаб.СрокСлужбы = Месяц(СтрокаТаб.ДатаОкончания) - Месяц(Объект.Дата) + (Год(СтрокаТаб.ДатаОкончания) - Год(Объект.Дата)) * 12; /// + 1;
	КонецЦикла;
...
Процедура ОрганизацияПриИзмененииНаСервере()
	...
	ПроцедурыНалоговогоУчета.ПриИзмененииПризнакаОтраженияВНалоговомУчете(Объект.Организация, Объект.Дата, Объект.УчитыватьКПН, Ложь);
...
Процедура ЗаполнитьОстаткамиНаСервере()
	...
	естьИ="";
	естьИ2="";
	если ЗначениеЗаполнено(Объект.МОЛОтправитель) тогда
		естьИ2="И";
	Конецесли;
	Запрос = новый Запрос;
	Запрос.Текст="ВЫБРАТЬ
	             |	СрокСлужбыСООстатки.Номенклатура КАК Номенклатура,
	             |	СУММА(СрокСлужбыСООстатки.КоличествоОстаток) КАК Количество
	             |ИЗ
	             |	РегистрНакопления.СрокСлужбыСО.Остатки(&дата2, "+?(ЗначениеЗаполнено(Объект.МОЛОтправитель),"МОЛПолучатель = &МОЛПолучатель","")+"
	             |				"+?(ЗначениеЗаполнено(Объект.СкладОтправитель),?(ЗначениеЗаполнено(Объект.МОЛОтправитель),"И","")+" СкладПолучатель = &СкладПолучатель","")+") КАК СрокСлужбыСООстатки
	             |ГДЕ 
	...
	Пока выборка.Следующий() цикл
		...
		НовСтрока.ЕдиницаИзмерения=выборка.Номенклатура.БазоваяЕдиницаИзмерения;
	Конеццикла;
