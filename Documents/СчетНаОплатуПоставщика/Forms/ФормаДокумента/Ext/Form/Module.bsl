...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	КлючеваяОперация = "Документ ""счет на оплату от поставщика"" (запись)";
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	ДанныеСтрокаТаблицы = Новый Структура("Номенклатура, ЕдиницаИзмерения, Коэффициент, Количество, Цена, Сумма, СтавкаНДС, СуммаНДС, Всего");
...
&НаСервере
Процедура ЗаполнитьРассчитатьСуммыВСтроке(Строка, ТаблицаЦенНоменклатуры, СтруктураЦеныИВалюта, ВалютаПередИзменением, СтруктураКурса, ПерезаполнитьЦены, ПересчитатьЦены, ПересчитатьНДС)
...
&НаСервере
Процедура ЗаполнитьРассчитатьСуммы(Знач ВалютаДоИзменения, КурсДоИзменения, КратностьДоИзменения, СтруктураЦеныИВалюта, ПересчитатьНДС = Неопределено)
...
Процедура ДобавитьНоменклатуруНаСервере(Результат, УникальныйИдентификатор) Экспорт
	...
	ДанныеОбъекта = Новый Структура("Организация, Дата, Ссылка, ТипЦен, УчитыватьНДС, СуммаВключаетНДС,
	|ВалютаДокумента, КурсВзаиморасчетов, КратностьВзаиморасчетов, Склад, ДоговорКонтрагента");   
...
Процедура ЗагрузитьИзФайла(Команда)
	...
	ПараметрыЗагрузки.ПолноеИмяТабличнойЧасти = "СчетНаОплатуПоставщика.Товары";
...
Процедура ЗагрузитьИзФайлаУслуги(Команда)
	...
	ПараметрыЗагрузки.ПолноеИмяТабличнойЧасти = "СчетНаОплатуПоставщика.Услуги";
...
Процедура ЗаполнитьИнформациюОбОплатеСчета()
	...
	Если ЗначениеЗаполнено(Объект.Организация)
		...
		ОплаченнаяСуммаСчета = Документы.СчетНаОплатуПоставщика.ОплаченнаяСуммаСчета(Объект.Организация, Объект.Ссылка);
		...
		Если ОжидаетсяОплата <> 0 Тогда 
			...
			ТекстОжидаетсяОплата = НСтр("ru = 'Необходимо оплатить: '");
