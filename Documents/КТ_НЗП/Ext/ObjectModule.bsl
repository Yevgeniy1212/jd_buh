...
Процедура ОбработкаПроведения(Отказ)
	...
	Для Каждого ТекСтрокатбДанные Из Данные Цикл
		...
		Если СчетКт = ПланыСчетов.Типовой.НезавершенноеПроизводство_ОсновноеПроизводство Тогда //1341 
			Движение.КоличествоКт = ТекСтрокатбДанные.Количество;
		ИначеЕсли СчетДт = ПланыСчетов.Типовой.НезавершенноеПроизводство_ОсновноеПроизводство Тогда //1341 
			Движение.КоличествоДт = ТекСтрокатбДанные.Количество;
		КонецЕсли;
	...
	//-------Обратные проводки --------------
	Для Каждого ТекСтрокатбДанные Из Данные Цикл
		Если ТекСтрокатбДанные.СчетДт = ПланыСчетов.Типовой.НезавершенноеПроизводство_ОсновноеПроизводство Тогда
			СчетКт = ТекСтрокатбДанные.СчетДт;
			СчетДт = ТекСтрокатбДанные.СчетКт;		
			
			массивСубКт = новый Массив;
			массивСубКт.Добавить(ТекСтрокатбДанные.СубконтоДт1);
			массивСубКт.Добавить(ТекСтрокатбДанные.СубконтоДт2);
			массивСубКт.Добавить(ТекСтрокатбДанные.СубконтоДт3);
			
			массивСубДт = новый Массив;
			массивСубДт.Добавить(ТекСтрокатбДанные.СубконтоКт1);
			массивСубДт.Добавить(ТекСтрокатбДанные.СубконтоКт2);
			массивСубДт.Добавить(ТекСтрокатбДанные.СубконтоКт3);
			
			Движение = Движения.Типовой.Добавить();
			
			Движение.Период = НачалоМесяца(ДобавитьМесяц(Дата, 1));
			Движение.СчетДт = СчетДт;
			Для каждого видС Из СчетДт.ВидыСубконто Цикл
				Движение.СубконтоДт[видС.ВидСубконто] = массивСубДт[видС.НомерСтроки - 1];
			КонецЦикла;
