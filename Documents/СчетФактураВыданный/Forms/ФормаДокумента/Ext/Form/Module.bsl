...
Процедура ДокументыОснованияПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	ПроверитьОтложенноеНачислениеНДСДокументовОснований(СтрокаТабличнойЧасти.ДокументОснование, Отказ);
...
&НаСервереБезКонтекста
Процедура ПроверитьОтложенноеНачислениеНДСДокументовОснований(ДокументОснование, Отказ)

	МетаданныеДокумента = ДокументОснование.Метаданные();

	Если ОбщегоНазначенияБК.ЕстьРеквизитДокумента("ОтложитьНачислениеНДС",МетаданныеДокумента) И ДокументОснование.ОтложитьНачислениеНДС Тогда		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='У документа ""%1"" установлен признак отложенного начисления НДС, счет-фактуру следует вводить на основании документа Регистрация прочих операций по реализованным товарам.'"), 
				ДокументОснование),,,,Отказ);
		ДокументОснование = Неопределено;
	КонецЕсли;
