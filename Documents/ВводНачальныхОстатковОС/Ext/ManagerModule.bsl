...
Процедура СформироватьДвиженияПроводокБух(ТаблицаОС, Движения, Отказ) Экспорт
	...
	Для Каждого СтрокаТаблицы Из Параметры.ТаблицаОС  Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы.СчетУчетаБУ) И НЕ (СтрокаТаблицы.ТекущаяСтоимостьБУ = 0 )Тогда
			...
			Проводка.Период      = СтрокаТаблицы.Период;
			Проводка.Организация = СтрокаТаблицы.Организация;
			Проводка.Содержание  = НСтр("ru = 'Ввод остатков по ОС (текущая стоимость)'", КодОсновногоЯзыка);
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы.СчетНачисленияАмортизацииБУ) И НЕ (СтрокаТаблицы.НакопленнаяАмортизацияБУ = 0) Тогда
			...
			Проводка.Период      = СтрокаТаблицы.Период;
			Проводка.Организация = СтрокаТаблицы.Организация;
			Проводка.Содержание  = НСтр("ru = 'Ввод остатков по ОС (начисл. амортизация)'", КодОсновногоЯзыка);
...
Процедура СформироватьДвиженияПроводокНал(ТаблицаОС, Движения, Отказ) Экспорт
	...
	Для Каждого СтрокаОС Из ТаблицаОС Цикл
		...
		Если НЕ (СтрокаОС.Стоимость = 0) Тогда			
			...
			Проводка.Содержание  = НСтр("ru = 'Ввод остатков ОС'", КодОсновногоЯзыка);
		...
		Если НЕ (СтрокаОС.ТекущаяСтоимостьПР = 0) Тогда				
			...
			Проводка.Период      = СтрокаОС.Период;
			Проводка.Организация = СтрокаОС.Организация;
			Проводка.Содержание  = НСтр("ru = 'Ввод остатков по ОС (постоянная разница)'", КодОсновногоЯзыка);
		...
		Если НЕ (СтрокаОС.ТекущаяСтоимостьВР = 0) Тогда
			...
			Проводка.Период      = СтрокаОС.Период;
			Проводка.Организация = СтрокаОС.Организация;
			Проводка.Содержание  = НСтр("ru = 'Ввод остатков по ОС (временная разница)'", КодОсновногоЯзыка);
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если НужноПечататьМакет Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			ПечатьОпись(МассивОбъектов, ОбъектыПечати, ПараметрыПечати, ПараметрыВывода.КодЯзыка),
			,
			"Документ.ВводНачальныхОстатковОС.ПФ_MXL_ОписьОС");
		
		ОписаниеПечатнойФормы = КоллекцияПечатныхФорм.Найти(ВРег("ПечатьОпись"), "ИмяВРЕГ");
...
//
Функция ПечатьОпись(МассивОбъектов, ОбъектыПечати, ПараметрыПечати, КодЯзыка = Неопределено) Экспорт
	...
	Макет  = УправлениеПечатью.МакетПечатнойФормы("Документ.ВводНачальныхОстатковОС.ПФ_MXL_ОписьОС", КодЯзыка);
	...
	Для Каждого Ссылка Из МассивОбъектов Цикл 
		...
		Область.Параметры.Заголовок = РаботаСДиалогами.СформироватьЗаголовокДокумента(Ссылка, НСтр("ru = 'Ввод начальных остатков по ОС'", КодЯзыка));
			...
			Для Каждого Колонка Из СтруктРекв Цикл
				Если Не Колонка.Ключ = "ОсновноеСредство" Тогда
					...
					Если ТипЗнч(СтрокаТЧ[Колонка.Ключ]) = Тип("Булево") Тогда
						Область.ТекущаяОбласть.Формат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("БЛ=%1; БИ=%2", НСтр("ru = 'Нет'", КодЯзыка), НСтр("ru = 'Да'", КодЯзыка));
					ИначеЕсли ТипЗнч(СтрокаТЧ[Колонка.Ключ]) = Тип("Дата") Тогда
						Область.ТекущаяОбласть.Формат = "Л=" + КодЯзыка + "; ДЛФ=DD";
