...
Функция ПолучитьОстатки()
	...
	зпОстатки.Текст = 
	"ВЫБРАТЬ
	|	ПроизводственнаяСправкаИнформацияПоПродукции.Продукция КАК Продукция
	|ИЗ
	|	Документ.ПроизводственнаяСправка.ИнформацияПоПродукции КАК ПроизводственнаяСправкаИнформацияПоПродукции
	|ГДЕ
	|	ПроизводственнаяСправкаИнформацияПоПродукции.Ссылка = &Ссылка";
	...
	зпОстатки.УстановитьПараметр("Ссылка", МСсылка);

	тбСкважины.Загрузить(зпОстатки.Выполнить().Выгрузить());
