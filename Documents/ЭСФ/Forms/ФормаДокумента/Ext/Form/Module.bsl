...
Процедура ОбработкаВыбораСчетаФактуры(СчетФактура)
	...
	Если ЗначениеЗаполнено(Объект.СчетФактура)
		...
		ТекстВопроса = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр(
			...
			|Отразить документ в учете с помощью выбранного счета-фактуры?'"), Объект.СчетФактура);
...
Процедура ОбработкаВыбораСНТ(СНТ)
	...
	Если ЗначениеЗаполнено(Объект.СНТ)
		...
		ТекстВопроса = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Документ уже отражен в учете с помощью ""%1"".
			|Отразить документ в учете с помощью выбранной СНТ?'"), Объект.СНТ);
...
Процедура ТоварыИсточникПроисхожденияПриИзменении(Элемент)
	...
	ЗаполнитьЗначенияСвойств(СтрокаТабличнойЧасти, СтруктураЗначенийИП,"НомерЗаявленияВРамкахТС, НомерПозицииВДекларацииИлиЗаявлении,
																	|ПризнакПроисхождения, ТоварНаименованиеВРамкахТС, ИдентификаторТовара,
																	|КодТНВЭД, ДополнительныйИдентификатор, ПризнакУчетаНаВиртуальномСкладе, GTIN");
	
	ЭтоЭСФсСНТ = ?((СтрНайти(Объект.ДокументПодтверждающийПоставкуНомер, "KZ-SNT-") > 0 ИЛИ СтрНайти(Объект.ДокументПодтверждающийПоставкуАВРНомер, "KZ-SNT-") > 0), Истина, Ложь);
	
	Если (СтрНайти(Объект.ДокументПодтверждающийПоставкуНомер, "NON SNT") > 0 ИЛИ СтрНайти(Объект.ДокументПодтверждающийПоставкуАВРНомер, "KZ-SNT-") > 0) 
		 ИЛИ (НЕ ЭтоЭСФсСНТ И НЕ (Объект.Получатели.Количество() > 0 И Объект.Получатели[0].РозничнаяРеализация)) Тогда
		СтрокаТабличнойЧасти.ИдентификаторТовара = "1";
	КонецЕсли;
	
	ДанныеСтрокиТаблицы = Новый Структура("КодТНВЭД, Количество, ЕдиницаИзмерения, ЕдиницаИзмеренияКод, КоличественнаяЕдИзмКод, КоличественнаяЕдИзм, КоличествоВКоличественнойЕдИзм");
	ЗаполнитьЗначенияСвойств(ДанныеСтрокиТаблицы, СтрокаТабличнойЧасти);

	ТипПошлины = ВСКлиентСервер.ОпределитьТипПошлины(СтруктураЗначенийИП.НомерЗаявленияВРамкахТС);
	ЗаполнитьДанныеПоКоличественнойЕдинице(ДанныеСтрокиТаблицы, ТипПошлины);
...
Процедура ПерезаполнитьПоСчетуФактуреНаСервере()
	...
	Если НЕ (Объект.Состояние = Перечисления.СостоянияЭСФ.Сформирован
		...
		ТекстСообщения = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Невозможно перезаполнить, так как электронный счет-фактура %1.'"),НРег(Объект.Состояние));
	...
	Если РезультатВопроса = КодВозвратаДиалога.ОК Тогда
		...
		Если ЗначениеЗаполнено(Объект.СчетФактура) Тогда
			...
			ТекстВопроса = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр(
				...
				|Создать новый счет-фактуру?'"), Объект.СчетФактура);
...
Процедура ОбновитьСчетФактуруПослеЗаписи(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Тогда
		Если ЭСФКлиент.ОбъектЗаписан(ЭтаФорма) Тогда
			ТекстВопроса = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Обновить ""%1"" на основании данного документа?'"), Объект.СчетФактура);
...
&НаКлиенте
Процедура ЗаполнитьДанныеКоличественнойЕИ(СтруктураРеквизитовЕдиницыИзмерения)
	
	Если Объект.Направление = ПредопределенноеЗначение("Перечисление.НаправленияЭСФ.Исходящий") Тогда
		Элементы.Товары.ТекущиеДанные.КоличественнаяЕдИзмКод = ?(ЗначениеЗаполнено(СтруктураРеквизитовЕдиницыИзмерения.Код),СтруктураРеквизитовЕдиницыИзмерения.Код,"") ;
	КонецЕсли;
...
Функция НовыйМассивИменНедоступныхЭлементов()
	...
	Массив.Добавить("ТоварыКоличественнаяЕдИзм");
	Массив.Добавить("ТоварыКоличествоВКоличественнойЕдИзм");
	Массив.Добавить("ТоварыGTIN");
...
Процедура СформироватьПредставлениеСостояния()
	...
	ИначеЕсли Объект.Состояние = Перечисления.СостоянияЭСФ.Отозван
		...
		ШиринаСтроки = Цел(СтрДлина(ПредставлениеСостояния)/1.3);
		Если ШиринаСтроки > 100 Тогда
			Элементы.ПредставлениеСостояния.Ширина = 100;			
		Иначе
			Элементы.ПредставлениеСостояния.Ширина = ШиринаСтроки;
		КонецЕсли;		
...
Процедура СформироватьПредставлениеОтзыва(Форма)
	...
	ИначеЕсли Количество > 1 Тогда
		...
		Если Количество = 2 ИЛИ Количество = 3 ИЛИ Количество = 4 Тогда
			Форма.ПредставлениеОтзыва = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Счет-фактура сторнирован %1 раза'"), Количество);			
		Иначе
			Форма.ПредставлениеОтзыва = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Счет-фактура сторнирован %1 раз'"), Количество);			
		КонецЕсли;
		Форма.Элементы.ПредставлениеОтзыва.ЦветТекста = ЭСФКлиентСервер.ЦветКрасный();
...
Процедура ЗаполнитьДопРеквизитыОднойТаблицыТоваровПоУчастникамСД(Знач ИмяТаблицы)
	...
	Для Каждого СтрокаТоварыПоУчастникамСД Из Объект[ИмяТаблицы] Цикл
		...
		Если МассивСтрокТовары.Количество() <> 0 Тогда
			...
			// Заполнить поле GTIN.
			СтрокаТоварыПоУчастникамСД.GTIN = СтрокаТовары.GTIN;
			
			// Заполнить поле Количественная ед.изм.
			СтрокаТоварыПоУчастникамСД.КоличественнаяЕдИзм = СтрокаТовары.КоличественнаяЕдИзмКод;
...
Процедура УправлениеФормой()
	...
	Если ВедетсяУчетПоТоварамОрганизацийНаВС Тогда
		ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыНайтиСоздатьОбновитьНомераГТД", "Заголовок", НСтр("ru='Заполнить источники происхождения'"));
	...
	Иначе
		ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыНайтиСоздатьОбновитьНомераГТД", "Заголовок", НСтр("ru='Найти, создать, обновить'"));
	КонецЕсли;
...
Функция СсылочныеРеквизитыДляСозданияСчетаФактурыЗаполнены()
	...
	Для Каждого СтрокаПоставщики Из Объект.Поставщики Цикл
		Если НЕ ЗначениеЗаполнено(СтрокаПоставщики.Поставщик) Тогда
			...
			ТекстСообщения = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Поле ""Поставщик ссылка"" не заполнено, в строке №%1, в разделе ""Раздел B. Поставщики"".'"),СтрокаПоставщики.НомерСтроки);
			Сообщить(ТекстСообщения);
		КонецЕсли;		
	КонецЦикла;
	...
	Для Каждого СтрокаПолучатели Из Объект.Получатели Цикл
		Если НЕ ЗначениеЗаполнено(СтрокаПолучатели.Получатель) Тогда
			РеквизитыЗаполнены = Ложь;
			ТекстСообщения = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Поле ""Получатель ссылка"" не заполнено, в строке №%1, в разделе ""Раздел C. Получатели"".'"),СтрокаПоставщики.НомерСтроки);
			Сообщить(ТекстСообщения);
		КонецЕсли;		
	КонецЦикла;
	...
	Для Каждого ТоварыСтрока Из Объект.Товары Цикл
		Если НЕ ЗначениеЗаполнено(ТоварыСтрока.Товар) Тогда
			РеквизитыЗаполнены = Ложь;
			ТекстСообщения = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Поле ""Товар ссылка"" не заполнено, в строке №%1, в разделе ""G. Товары"".'"),ТоварыСтрока.НомерСтроки);
			Сообщить(ТекстСообщения);
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого ТоварыСтрока Из Объект.Товары Цикл
		Если НЕ ЗначениеЗаполнено(ТоварыСтрока.ЕдиницаИзмерения) Тогда
			РеквизитыЗаполнены = Ложь;
			ТекстСообщения = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Поле ""Ед. изм. ссылка"" не заполнено, в строке №%1, в разделе ""G. Товары"".'"),ТоварыСтрока.НомерСтроки); 
			Сообщить(ТекстСообщения);
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого ТоварыСтрока Из Объект.Товары Цикл
		Если ЗначениеЗаполнено(ТоварыСтрока.СтавкаАкцизаЧисло) И НЕ ЗначениеЗаполнено(ТоварыСтрока.СтавкаАкциза) Тогда
			РеквизитыЗаполнены = Ложь;
			ТекстСообщения = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Поле ""Ставка акциза ссылка"" не заполнено, в строке №%1, в разделе ""G. Товары"".'"),ТоварыСтрока.НомерСтроки); 
			Сообщить(ТекстСообщения);
		КонецЕсли;
	КонецЦикла;	
	...
	Для Каждого ТоварыСтрока Из Объект.Товары Цикл
		Если НЕ ЗначениеЗаполнено(ТоварыСтрока.СтавкаНДС) Тогда
			РеквизитыЗаполнены = Ложь;
			ТекстСообщения = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Поле ""Ставка НДС ссылка"" не заполнено, в строке №%1, в разделе ""G. Товары"".'"),ТоварыСтрока.НомерСтроки); 
			Сообщить(ТекстСообщения);
		КонецЕсли;
	КонецЦикла;
...
Процедура НастроитьВерсиюЭСФ(Форма, ВерсияБланка)
	...
	СтруктураЗаголовковВерсия1.Вставить("ГруппаБанковскиеРеквизитыПоставщика", НСтр("ru='Банковские реквизиты поставщика'"));
	СтруктураЗаголовковВерсия1.Вставить("РазделE", НСтр("ru='E. Условия поставки'"));
	СтруктураЗаголовковВерсия1.Вставить("Декорация5", НСтр("ru='Раздел E. Условия поставки'"));
	
	СтруктураЗаголовковВерсия1.Вставить("РазделI", НСтр("ru='I. Доп. сведения'"));
	СтруктураЗаголовковВерсия1.Вставить("ДекорацияРазделДопСведения", НСтр("ru='Раздел I. Дополнительные сведения'"));
	
	СтруктураЗаголовковВерсия1.Вставить("РазделJ", НСтр("ru='J. Сведения по ЭЦП'"));
	СтруктураЗаголовковВерсия1.Вставить("ДекорацияРазделСведенияЭЦП", НСтр("ru='Раздел J. Сведения по ЭЦП'"));
	
	СтруктураЗаголовковВерсия1.Вставить("ТоварыНомерЗаявленияВРамкахТС", НСтр("ru='№ заявления в рамках ТС или Декларации на товары'"));
	СтруктураЗаголовковВерсия1.Вставить("ТоварыНомерГТД", НСтр("ru='Источник происхождения'"));
	
	
	//версия 2	
	СтруктураЗаголовковВерсия2 = Новый Структура;
	СтруктураЗаголовковВерсия2.Вставить("ГруппаБанковскиеРеквизитыПоставщика", НСтр("ru='B1. Банковские реквизиты поставщика'"));
	СтруктураЗаголовковВерсия2.Вставить("РазделE", НСтр("ru='E. Договор (контракт)'"));
	СтруктураЗаголовковВерсия2.Вставить("Декорация5", НСтр("ru='Раздел E. Договор (контракт)'"));
	
	СтруктураЗаголовковВерсия2.Вставить("РазделI", НСтр("ru='K. Доп. сведения'"));
	СтруктураЗаголовковВерсия2.Вставить("ДекорацияРазделДопСведения", НСтр("ru='Раздел K. Дополнительные сведения'"));
	
	СтруктураЗаголовковВерсия2.Вставить("РазделJ", НСтр("ru='L. Сведения по ЭЦП'"));
	СтруктураЗаголовковВерсия2.Вставить("ДекорацияРазделСведенияЭЦП", НСтр("ru='Раздел L. Сведения по ЭЦП'"));

	СтруктураЗаголовковВерсия2.Вставить("ТоварыНомерЗаявленияВРамкахТС", НСтр("ru='№ Декларации на товары, заявления в рамках ТС, СТ-1 или СТ-KZ'"));
	СтруктураЗаголовковВерсия2.Вставить("ТоварыНомерГТД", НСтр("ru='Источник происхождения'"));
...
Процедура ОбновитьПредставлениеКодаКлассификатора(ИмяМакета) Экспорт
	...
	Если ИмяМакета = "УсловияПоставки" Тогда
		...
		Иначе
			...
			Если ПустаяСтрока(НаименованиеСтроки) Тогда
				Элементы.ДекорацияРасшифровкаУсловияПоставки.Заголовок = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru ='строка с кодом %1 не найдена.'"), СокрЛП(Объект.ДоговорПоставкиУсловияПоставки));
			Иначе
	...
	Если ИмяМакета = "СпособыОтправления" Тогда
		...
		Если ПустаяСтрока(Объект.СпособОтправленияПоКлассификатору) Тогда
			Элементы.ДекорацияРасшифровкаСпособОтправления.Заголовок = НСтр("ru='<не указано>'");
		Иначе
			...
			Если ПустаяСтрока(НаименованиеСтроки) Тогда
				Элементы.ДекорацияРасшифровкаСпособОтправления.Заголовок = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru ='строка с кодом %1 не найдена.'"),СокрЛП(Объект.СпособОтправленияПоКлассификатору));				
			Иначе
...
&НаСервереБезКонтекста
Процедура ЗаполнитьДанныеПоКоличественнойЕдинице(СтрокаТабличнойЧасти, ТипПошлины)
	ЭСФСерверПереопределяемый.ЗаполнитьДанныеПоКоличественнойЕдинице(СтрокаТабличнойЧасти, ТипПошлины);
КонецПроцедуры
//-- НЕ ЭСФ
...
&НаКлиенте
Процедура ТоварыКоличественнаяЕдИзмПриИзменении(Элемент)
	
	// По ссылочному полю количественная ед.измерения получаем данные о коде и наименовании.
	ДанныеЕдиницыИзмерения = ЭСФВызовСервера.ПолучитьДанныеЕдиницыИзмерения(Элементы.Товары.ТекущиеДанные.КоличественнаяЕдИзм);	
		
	ЗаполнитьДанныеКоличественнойЕИ(ДанныеЕдиницыИзмерения);
