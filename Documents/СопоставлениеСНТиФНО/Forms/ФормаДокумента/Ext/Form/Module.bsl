...
Процедура УстановкаПредставленияДокумента()
	...
	Если Параметры.Ключ.Пустая() Тогда
		Представление = "";
	Иначе
		Представление = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru='%1 от %2'"),Объект.Номер,Формат(Объект.Дата, "ДФ = dd.MM.yyyy HH:mm:ss"));
	КонецЕсли;
...
Процедура ЗаполнитьРеквизитыТоваровСНТПоРеквизитамТоваровФНО(МассивСтрокФНО,МассивСтрокСНТ)
	...
	Для Каждого СтрокаФНО Из МассивСтрокФНО Цикл
		...
		Если ИндексыДубликата <> Неопределено Тогда
			...
			СообщениеОбОшибке = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(
				СообщениеОбОшибке,
