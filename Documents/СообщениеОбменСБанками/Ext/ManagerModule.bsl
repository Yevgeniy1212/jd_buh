...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Пока ВыборкаСообщенийОбмена.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, ВыборкаСообщенийОбмена.Ссылка, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));  
...
Функция СформироватьПечатнуюФормуЭД(СообщениеОбмена, ВидЭД, ДанныеФайла, ИдентификаторПечати, ДляПечати)
	...
	Если ВидЭД = Перечисления.ВидыЭДОбменСБанками.ПлатежноеПоручение Тогда
		...
		Если ДанныеЭДДляПечати.Свойство("ТипМакетаМХ") Тогда
			Если ДанныеЭДДляПечати.ТипМакетаМХ = "Единичный" Тогда
				ЗаполнитьТабличныйДокументПлатежноеПоручениеИсходящее_ЭДISO(ТабличныйДокумент, ДанныеЭДДляПечати, СообщениеОбмена);
			Иначе	
				ЗаполнитьТабличныйДокументСводноеПлатежноеПоручениеИсходящее_ЭДISO(ТабличныйДокумент, ДанныеЭДДляПечати, СообщениеОбмена);
			КонецЕсли;
		Иначе
...
Процедура ЗаполнитьТабличныйДокументПлатежноеПоручениеИсходящее_ЭДISO(ТабличныйДокумент, ДанныеПечати, ИД)
	...
	ОбластьМакета.Параметры.НаименованиеНомер = НСтр("ru = 'ПЛАТЕЖНОЕ ПОРУЧЕНИЕ №'") + " " + ДанныеПечати.НаименованиеНомер;
	ОбластьМакета.Параметры.ДатаДокумента = Формат(ДанныеПечати.ДатаДокумента, НСтр("ru='ДФ=''дд MMMM гггг'''"));
	ОбластьМакета.Параметры.СуммаПрописью = ЧислоПрописью(ДанныеПечати.СуммаЧислом, "Л=ru_; ДП=Ложь", "тенге, тенге, тенге, м, тиын, тиын, тиын, ж, 2");
	ОбластьЯчеек = ОбластьМакета.Область();
	ОбластьЯчеек.СоздатьФорматСтрок();
	ТабличныйДокумент.Вывести(ОбластьМакета);
...
Процедура ЗаполнитьТабличныйДокументСводноеПлатежноеПоручениеИсходящее_ЭДISO(ТабличныйДокумент, ДанныеПечати, ИД)
	
	НазваниеМакета = СтрШаблон("ЭД_СводноеПлатежноеПоручениеMX_%1", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	Макет = Документы.СообщениеОбменСБанками.ПолучитьМакет(НазваниеМакета);
	Макет.КодЯзыка = Метаданные.Языки.Русский.КодЯзыка;
	...
	ОбластьМакета.Параметры.НаименованиеНомер = НСтр("ru = 'Сводное платежное поручение №'") + " " + ДанныеПечати.НаименованиеНомер;
	ОбластьМакета.Параметры.ДатаДокумента = Формат(ДанныеПечати.ДатаДокумента, НСтр("ru='ДФ=''дд MMMM гггг'''"));
	ОбластьМакета.Параметры.СуммаПрописью = ЧислоПрописью(ДанныеПечати.СуммаЧислом, "Л=ru_; ДП=Ложь", "тенге, тенге, тенге, м, тиын, тиын, тиын, ж, 2");
	ОбластьЯчеек = ОбластьМакета.Область();
	ОбластьЯчеек.СоздатьФорматСтрок();
	...
	Для каждого Сотрудник Из ДанныеПечати.СписокСотрудников Цикл
		
		ОбластьМакетаРазделитель = Макет.ПолучитьОбласть("Разделитель");
		ТабличныйДокумент.Вывести(ОбластьМакетаРазделитель);
		
		ТабличныйДокумент.ВывестиГоризонтальныйРазделительСтраниц();
		ОбластьМакетаТранзакция = Макет.ПолучитьОбласть("Транзакция");
		
		ОбластьМакетаТранзакция.Параметры.Заполнить(Сотрудник);
		
		НомерПП = НомерПП + 1;
		ОбластьМакетаТранзакция.Параметры.НаименованиеНомер = НСтр("ru = 'Платежное поручение №'") + " " + Формат(НомерПП, "ЧГ=0");
		ОбластьМакетаТранзакция.Параметры.ДатаДокумента = Формат(ДанныеПечати.ДатаДокумента, НСтр("ru='ДФ=''дд MMMM гггг'''"));
		ОбластьМакетаТранзакция.Параметры.СуммаПрописью = ЧислоПрописью(ОбластьМакетаТранзакция.Параметры.СуммаПрописью, "Л=ru_; ДП=Ложь", "тенге, тенге, тенге, м, тиын, тиын, тиын, ж, 2");
		ОбластьЯчеек = ОбластьМакетаТранзакция.Область();
		ОбластьЯчеек.СоздатьФорматСтрок();
		ТабличныйДокумент.Вывести(ОбластьМакетаТранзакция);
