...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ПараметрыПроведения = Документы.ПеремещениеОСЗабаланс.ПодготовитьПараметрыПроведения(Ссылка, Отказ);
	...
	Если ПараметрыПроведения.ПеремещениеОССтоимостьОСБУ.Количество() <> 0  Тогда
		//И ФормироватьПроводкиБУ 	Тогда 
...
Процедура ПроверкаОСПоОрганизации(Отказ, ЭтотОбъект)
	...
	Если Выборка.Количество() > 0 Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект.Ссылка, Поле, "Объект"); 
