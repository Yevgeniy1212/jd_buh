...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
  ...
  // Включим предопределенные счета и их субсчета
	...
	// Дата: 03.07.2023
	//
	// Описание:  Добавление забалансовых счетов
	//
	//
	                                            
		
	Элементы.ОССчетУчетаБУ.СписокВыбора.Добавить(ПланыСчетов.Типовой.АрендованныеОсновныеСредства,ПланыСчетов.Типовой.АрендованныеОсновныеСредства.Код +  " (" +ПланыСчетов.Типовой.АрендованныеОсновныеСредства.Наименование + ")" );
	сч9040 = ПланыСчетов.Типовой.НайтиПоКоду("9040");
	Если ПланыСчетов.Типовой.НайтиПоКоду("9040") <> ПланыСчетов.Типовой.ПустаяСсылка() Тогда
		Элементы.ОССчетУчетаБУ.СписокВыбора.Добавить(ПланыСчетов.Типовой.НайтиПоКоду("9040"), сч9040.Код + " (" + сч9040.Наименование + ")");
	КонецЕсли;
	////////////////////////////////FINISH Кибернетика/////////////////////////////////////// 
...
Процедура ОСПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если Элементы.ОСРедактированиеВДиалоге.Пометка Тогда
		...
		ОткрытьФорму("Документ.ВводНачальныхОстатковОСЗабаланс.Форма.ФормаРедактированияСтроки", Новый Структура("СтруктураПараметров", СтруктураПараметров), ЭтаФорма);
