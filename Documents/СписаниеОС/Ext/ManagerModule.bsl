...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ДА3") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			Киб_ПечатьДА3(МассивОбъектов, ОбъектыПечати),
			//Евгений-,
			,
...
Функция ПечатьДА3(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Пока Док.Следующий() Цикл
		...
		Пока ВыборкаОС.Следующий() Цикл
			...
			ОбластьМакета.Параметры.ДатаВводаВЭксплуатацию = Формат(ДатаПринятнияКУчету, "Л=" + КодЯзыка + "; ДЛФ=DD");
		...
		ОбластьМакета.Параметры.Заполнить(Док);
		ОбластьМакета.Параметры.ДокументОснованиеДата 	= ?(ЗначениеЗаполнено(Док.ДокументОснованиеДата), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 года'", КодЯзыка), Формат(Док.ДокументОснованиеДата, "Л=" + КодЯзыка + "; ДФ=дд ММММ гггг")), НСтр("ru='«____» ________________20__года'"));
		...
		Если ЗначениеЗаполнено(Док.ДокументОснованиеВид) Тогда
			Если Док.ДокументОснованиеВид = "приказом" Тогда
				ОбластьМакета.Параметры.ДокументОснованиеВид = НСтр("ru='приказом'", КодЯзыка);
			ИначеЕсли Док.ДокументОснованиеВид = "постановлением" Тогда
				ОбластьМакета.Параметры.ДокументОснованиеВид = НСтр("ru='постановлением'", КодЯзыка);
			ИначеЕсли Док.ДокументОснованиеВид = "распоряжением" Тогда
				ОбластьМакета.Параметры.ДокументОснованиеВид = НСтр("ru='распоряжением'", КодЯзыка);
			КонецЕсли;	
		Иначе
			ОбластьМакета.Параметры.ДокументОснованиеВид = НСтр("ru='приказом (распоряжением)'", КодЯзыка);
		КонецЕсли;
		ТабДокумент.Вывести(ОбластьМакета);
		...
		Для Каждого СтрокаНаимОС ИЗ ТаблицаОС Цикл
			...
			ОбластьМакета.Параметры.ПроизвелаОсмотр = ?(ВыводитьНадпись, НСтр("ru='на основании ________________ произвела осмотр'", КодЯзыка) ,"");
...
//Евгений+
Функция Киб_ПечатьДА3(МассивОбъектов, ОбъектыПечати) Экспорт
	
	Счетчик = 0;
	
	ТабД = Новый ТабличныйДокумент;
	
	Пока Счетчик < МассивОбъектов.Количество() Цикл
		...
		Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.СписаниеОС.ОС4");
		
		
		//вспомогательные запросы для формирования печатной формы документа
		Запрос = Новый Запрос();
		Запрос.УстановитьПараметр("Ссылка", МассивОбъектов[Счетчик]);
		Запрос.Текст = "ВЫБРАТЬ
		|	СписаниеОС.Номер КАК НомерДок,
		|	СписаниеОС.Дата КАК ДатаДок,
		|	СписаниеОС.Организация КАК Организация,
		|	СписаниеОС.СтруктурноеПодразделение КАК СтруктурноеПодразделение,
		...
		|	СписаниеОС.ОС_СписаноНаОсновании КАК ОС_СписаноНаОсновании,
		|	СписаниеОС.ЗаключениеКомиссии КАК ЗаключениеКомиссии,
		|	СписаниеОС.СостояниеАктивов КАК СостояниеАктивов
		|ИЗ
		|	Документ.СписаниеОС КАК СписаниеОС
		|ГДЕ
		|	СписаниеОС.Ссылка В(&Ссылка)";
		...
		Пока Док.Следующий() Цикл
			...
			ОбластьМакета.Параметры.ОрганизацияРНН       = ОбщегоНазначенияБКВызовСервера.ОписаниеОрганизации(СведенияОбОрганизации, "БИН_ИИН,", Ложь, Док.ДатаДок, "ru");
			...
			Если НЕ (ПредседательКомиссии = Неопределено) Тогда			
				ДанныеПредседателя = ПроцедурыУправленияПерсоналомВызовСервера.ДанныеФизЛица(Док.Организация, ПредседательКомиссии.ФизЛицо, Док.ДатаДок);
				ОбластьМакета.Параметры.Должность                = "Председатель комиссии"+Символы.ПС+ДанныеПредседателя.Должность;
				ОбластьМакета.Параметры.РасшифровкаПодписи       = ДанныеПредседателя.Представление;
			КонецЕсли;
			...
			Пока ВыборкаОС.Следующий() Цикл
				...
				ОбластьМакета.Параметры.БалансоваяСтоимость = ВыборкаОС.ОстСтоимостьБУ;
				УправлениеВнеоборотнымиАктивамиСервер.ПолучитьДокументБухСостоянияОС(ВыборкаОС.ОС, Док.Организация, Перечисления.ВидыСостоянийОС.ПринятоКУчету, Док.ДатаДок, ДокументПринятияКУчетуБУ,  ДатаПринятнияКУчету);
				ОбластьМакета.Параметры.ДатаВводаВЭксплуатацию = ДатаПринятнияКУчету;
				Если ВыборкаОС.ДатаПоследнихПараметровАмортизации <> Null Тогда 
					ФактическийСрокИспользования = УправлениеВнеоборотнымиАктивамиСервер.ОпределитьФактическийСрокИспользования(ВыборкаОС.ДатаПоследнихПараметровАмортизации, Док.ДатаДок); 
				Иначе 
					ФактическийСрокИспользования = "";
			...
			Для Каждого ЧленКомиссии Из ДанныеЧленовКомиссии Цикл
				...
				ОбластьМакета.Параметры.ЗаголовокРазделаКомиссии = ?(ВыводитьЗаголовок, ЗаголовокРазделаКомиссии.ЧленыКомиссии, "");
				ОбластьМакета.Параметры.ОборотнаяСторона = ?(ВыводитьЗаголовок и ОборотнаяСторона, "Оборотная сторона формы ДА-3","");
				ОбластьМакета.Параметры.Должность          = ЧленКомиссии.Должность;
				ОбластьМакета.Параметры.РасшифровкаПодписи = ЧленКомиссии.Представление;
			...
			Если ДанныеЧленовКомиссии.Количество() < 1 Тогда
				...
				ОбластьМакета.Параметры.ЗаголовокРазделаКомиссии = ?(ВыводитьЗаголовок, ЗаголовокРазделаКомиссии.ЧленыКомиссии, "");
				ОбластьМакета.Параметры.ОборотнаяСторона = "";			
				ОбластьМакета.Параметры.Должность          = "";
				ОбластьМакета.Параметры.РасшифровкаПодписи = "";
			...
			Если МассивОбъектов[Счетчик].СолидарныеМол.Количество()>0 тогда
				СписокМОЛ = МассивОбъектов[Счетчик].СолидарныеМол.ВыгрузитьКолонку("ФизЛицо");
				ДанныеМОЛ = ПроцедурыУправленияПерсоналомСервер.ДанныеФизЛиц(Док.Организация, СписокМОЛ, Док.ДатаДок);
				Для каждого СтрокаМОЛ из ДанныеМОЛ цикл
					ОбластьМакета.Параметры.Должность          = Сокрлп(СтрокаМОЛ.Должность);
					ОбластьМакета.Параметры.РасшифровкаПодписи = СтрокаМОЛ.Представление;
					ОбластьМакета.Параметры.ЗаголовокРазделаКомиссии = "";
				...
				//ПустаяСтрока=Макет.ПолучитьОбласть("ПустаяСтрока");
				//ТабДокумент.Вывести(ПустаяСтрока);
				СолМолДата=Макет.ПолучитьОбласть("СолМолДата");
				СолМолДата.Параметры.СолДата = Формат(МассивОбъектов[Счетчик].СолидарноеМолДата, "ДФ=""дд ММММ гггг 'года'""");
				ТабДокумент.Вывести(СолМолДата);
			Конецесли;
			...
			ОбластьМакета.Параметры.Валюта			   = ВалютаПечати;
			ОбластьМакета.Параметры.РасшифровкаПодписи = МассивОбъектов[Счетчик].Подписант;
			ОбластьМакета.Параметры.Должность = МассивОбъектов[Счетчик].ДолжностьПодписанта;
			ОбластьМакета.Параметры.Дата = СтрЗаменить(Формат(Док.ДатаДок,"ДЛФ=DD"),"г.","года");
			ТабДокумент.Вывести(ОбластьМакета);
			
			УправлениеПечатью.ЗадатьОбластьПечатиДокумента(ТабДокумент, НомерСтрокиНачало, ОбъектыПечати, Док.Ссылка);
			
			ТабД.Вывести(ТабДокумент);
			ТабД.ВывестиГоризонтальныйРазделительСтраниц();
