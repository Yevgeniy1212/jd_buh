...
Процедура КТ_КонтрольСвязейСправочниковПередЗаписью(Источник, Отказ) Экспорт
	...
	Если Не Выб.Количество()=0 Тогда
		...
		Пока Выб.Следующий() Цикл
			Сообщить("В """+Выб.Ссылка+""" уже выбрано значение """+Выб.Значение+"""");
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбновлениеОтображенияОбновитьВерсии(Форма, Открытие=Ложь) Экспорт
	...
	Если Форма.ЭлементыФормы.ДействияФормы.Кнопки.Найти("МенюВыборВерсии")=Неопределено Тогда
		...
		Форма.ЭлементыФормы.ДействияФормы.Кнопки.Добавить("МенюВыборВерсии", ТипКнопкиКоманднойПанели.Подменю, "Выбор версии");
	...
	Кн = Форма.ЭлементыФормы.ДействияФормы.Кнопки.МенюВыборВерсии.Кнопки.Добавить("Версия0", ТипКнопкиКоманднойПанели.Действие, "Все версии бюджета", Форма.ДействиеКнопкиВерсии);
	КнОсн = Кн;
	...
	КонецЕсли;
	Если РольДоступна("КТ_АдминистраторМодуляЗакупок") И Не МдРеквизиты.Найти("ДоступЗакрыт")=Неопределено Тогда
...
Процедура ДействияФормыВыборВерсии(Форма, Кнопка) Экспорт
	...
	Форма.ЭлементыФормы.ДействияФормы.Кнопки.МенюВыборВерсии.Текст = "Версия: "+Кнопка.Текст;
...
Процедура ДействияФормыИзменитьДоступ(Форма, Кнопка) Экспорт
	...
	Мд = Метаданные.НайтиПоТипу(ТипЗнч(СписокФормы));
	ЭтоСправочник = Метаданные.Справочники.Содержит(Мд);
	Если Мд.Реквизиты.Найти("ВерсияБюджета")=Неопределено Или Мд.Реквизиты.Найти("ДоступЗакрыт")=Неопределено Или Не РольДоступна("КТ_АдминистраторМодуляЗакупок") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОбновлениеОтображенияСменитьСтатусы(Форма) Экспорт
	
	Если Не Форма.ЭлементыФормы.ДействияФормы.Кнопки.Найти("МенюСменыСтатуса")=Неопределено Тогда
		Пока Форма.ЭлементыФормы.ДействияФормы.Кнопки.МенюСменыСтатуса.Кнопки.Количество()>0 Цикл
			Форма.ЭлементыФормы.ДействияФормы.Кнопки.МенюСменыСтатуса.Кнопки.Удалить(0);
		КонецЦикла;
	КонецЕсли;
	//Если Не Форма.ЭлементыФормы.ДействияФормы.Кнопки.Найти("МенюСменыМесяца")=Неопределено Тогда
	//	Пока Форма.ЭлементыФормы.ДействияФормы.Кнопки.МенюСменыМесяца.Кнопки.Количество()>0 Цикл
	//		Форма.ЭлементыФормы.ДействияФормы.Кнопки.МенюСменыМесяца.Кнопки.Удалить(0);
	//	КонецЦикла;
	//КонецЕсли;
	
	Если РольДоступна("АдминистраторОМТО") И Форма.ДокументСписок.Отбор.ВерсияБюджета.Использование Тогда
		Если Форма.ЭлементыФормы.ДействияФормы.Кнопки.Найти("РазделительМенюСмены")=Неопределено Тогда
			Форма.ЭлементыФормы.ДействияФормы.Кнопки.Добавить("РазделительМенюСмены", ТипКнопкиКоманднойПанели.Разделитель);
		КонецЕсли;
		//Если Форма.ЭлементыФормы.ДействияФормы.Кнопки.Найти("МенюСменыМесяца")=Неопределено Тогда
		//	Форма.ЭлементыФормы.ДействияФормы.Кнопки.Добавить("МенюСменыМесяца", ТипКнопкиКоманднойПанели.Подменю, "Установить месяц закрытия");
		//КонецЕсли;
		Если Форма.ЭлементыФормы.ДействияФормы.Кнопки.Найти("МенюСменыСтатуса")=Неопределено Тогда
			Форма.ЭлементыФормы.ДействияФормы.Кнопки.Добавить("МенюСменыСтатуса", ТипКнопкиКоманднойПанели.Подменю, "Сменить статусы документов");
		КонецЕсли;
		
		ДействиеСменыСтатуса = Новый Действие("ДействияФормыСменитьСтатус");
		//ДействиеСменыМесяца = Новый Действие("КнопкаСменитьМесяц");
		Запрос = Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ Документ." + СтрЗаменить(Строка(Форма.ДокументСписок), "ДокументСписок.", "") + ".СтатусЗаявки ГДЕ ВерсияБюджета = &ВерсияБюджета");
		Запрос.УстановитьПараметр("ВерсияБюджета", Форма.ДокументСписок.Отбор.ВерсияБюджета.Значение);
		Выб = Запрос.Выполнить().Выбрать();
		Пока Выб.Следующий() Цикл
			Если Выб.СтатусЗаявки=Перечисления.КТ_СтатусыГодовыхЗаявок.Утверждено_ПЭО Тогда
				Кн = Форма.ЭлементыФормы.ДействияФормы.Кнопки.МенюСменыСтатуса.Кнопки.Добавить("Статус_3_4", ТипКнопкиКоманднойПанели.Действие, "Отправить заявки в процесс закупа", ДействиеСменыСтатуса);
				Кн.Подсказка = "";
				Кн.Пояснение = "";
			ИначеЕсли Выб.СтатусЗаявки=Перечисления.КТ_СтатусыГодовыхЗаявок.ВПроцессеЗакупа Тогда
				Кн = Форма.ЭлементыФормы.ДействияФормы.Кнопки.МенюСменыСтатуса.Кнопки.Добавить("Статус_4_1", ТипКнопкиКоманднойПанели.Действие, "Отправить заявки на корректировку", ДействиеСменыСтатуса);
				Кн.Подсказка = "";
				Кн.Пояснение = "";
				
				//Для Сч=1 По 12 Цикл
				//	Кн = Форма.ЭлементыФормы.ДействияФормы.Кнопки.МенюСменыМесяца.Кнопки.Добавить("Мес_"+Сч, ТипКнопкиКоманднойПанели.Действие, Формат(Дата(2, Сч, 1), "ДФ=ММММ"), ДействиеСменыМесяца);
				//	Кн.Подсказка = "";
				//	Кн.Пояснение = "";
				//КонецЦикла;
			КонецЕсли;
	...
	Иначе
		Если Не Форма.ЭлементыФормы.ДействияФормы.Кнопки.Найти("МенюСменыСтатуса")=Неопределено Тогда
			Форма.ЭлементыФормы.ДействияФормы.Кнопки.Удалить(Форма.ЭлементыФормы.ДействияФормы.Кнопки.МенюСменыСтатуса);
		КонецЕсли;
		//Если Не Форма.ЭлементыФормы.ДействияФормы.Кнопки.Найти("МенюСменыМесяца")=Неопределено Тогда
		//	Форма.ЭлементыФормы.ДействияФормы.Кнопки.Удалить(Форма.ЭлементыФормы.ДействияФормы.Кнопки.МенюСменыМесяца);
		//КонецЕсли;
	КонецЕсли;
...
Процедура КТ_КонтрольКорректностиВерсийБюджетаПередЗаписью(Источник, Отказ) Экспорт
	...
	Если Мд.Реквизиты.Найти("ДоступЗакрыт")<>Неопределено Тогда
		Если РольДоступна("КТ_АдминистраторМодуляЗакупок") Тогда
			Если Источник.ДоступЗакрыт И Источник.Ссылка.ДоступЗакрыт Тогда
				...
				Возврат;
			КонецЕсли;
		ИначеЕсли Не РольДоступна("ПользовательБюджетирования") Или Источник.ДоступЗакрыт Тогда
			Отказ = Истина;
			Сообщить("Доступ к """+Источник+""" на изменение закрыт!", СтатусСообщения.Внимание);
			Возврат;
		КонецЕсли;
	...
	Если Не Источник.Год=текВерсия.Год И Не ТипЗнч(Источник)=Тип("ДокументОбъект.РасчетАмортизацииПНbug") Тогда
...
Функция ПолучитьМесяца(СНулем=Ложь, Преф="", Нач=1) Экспорт
	Рез = Новый СписокЗначений;
	Если СНулем И Нач=1 Тогда
		Рез.Добавить(0, "(все месяца)");
	КонецЕсли;
	
	Для Сч=Нач По 12 Цикл
		Рез.Добавить(Сч, Формат(Дата(2, Сч, 2), "ДФ=ММММ")+?(Преф="", "", " "+Преф));
	КонецЦикла;
	Возврат Рез;
КонецФункции // ПолучитьМесяца()
...
//Отчет ПФА
Процедура ОткрытьПФАПроцентовка() Экспорт
	ОткрытьФорму("РегистрСведений.ОтчетПФА_Процентовка.Форма.НастройкаПроцентовки");
КонецПроцедуры

Процедура ОткрытьГруппировкуСтатей() Экспорт
	ОткрытьФорму("РегистрСведений.ОтчетПФА_СоставГруппировокСтатейЗатрат.Форма.ФормаДопНастроек");
КонецПроцедуры

Процедура ОткрытьГруппировкуПодразделений() Экспорт
	ОткрытьФорму("РегистрСведений.ОтчетПФА_СоставГруппировокПодразделений.Форма.НастройкаГруппировок");
КонецПроцедуры
...
Процедура ПоказатьФормуВводаПримечания(Элемент, СтандартнаяОбработка) Экспорт 
	ТекстПримечания = Элемент.Значение;
	Если ПоказатьФормуВводаПроизвольногоТекста(СтандартнаяОбработка, ТекстПримечания, "Введите текст примечания") Тогда
		Элемент.Значение = ТекстПримечания;
	КонецЕсли;
КонецПроцедуры

Функция ПоказатьФормуВводаПроизвольногоТекста(СтандартнаяОбработка, Текст, Заголовок, Длина=0, Многострочность = Истина) Экспорт
	СтандартнаяОбработка = Ложь;
	Если ВвестиСтроку(Текст, Заголовок, Длина, Многострочность) Тогда
		Возврат Истина;
	КонецЕсли;
	Возврат Ложь;
КонецФункции
