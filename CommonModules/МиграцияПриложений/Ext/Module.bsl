...
Процедура НачатьВыгрузку(АдресПриложения, Логин, Пароль, ВыгружатьНастройкиПользователей, ЗавершитьМиграциюАвтоматически, ДополнительныеСвойства) Экспорт
	...
	Попытка
		...
		НовыйУзел.Наименование = СтрШаблон(НСтр("ru = 'Миграция в %1 (%2)'", ОбщегоНазначения.КодОсновногоЯзыка()), Метаданные.Представление(), АдресПриложения);
...
Функция ВыгрузитьПользовательскиеДанные(УзелОбмена, НомерСообщения, ПоследнийОбъектМетаданных, ПараметрыВыгрузки)
	...
	Для Каждого ОбъектМетаданных Из ПорядокВыборкиМетаданных(ПоследнийОбъектМетаданных) Цикл
		...
		РегистрацияПримечания(НСтр("ru = 'Статистика выгрузки'", ОбщегоНазначения.КодОсновногоЯзыка()), Комментарий);
	...
	Для Каждого ПользовательИБ Из ПользователиИнформационнойБазы.ПолучитьПользователей() Цикл
		...
		Пока Истина Цикл
			...
			Попытка
				...
				КонецЕсли;
			Исключение
				ШаблонОшибки = НСтр("ru = 'Выгрузка настройки %1 (пользователь: %2, ключ объекта: %3, ключ настроек: %4 ) пропущена по причине: %5'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ЗагрузитьПользовательскиеДанные(ПараметрыЗагрузки)
	...
	Для Каждого СтрокаДанных Из ПараметрыЗагрузки.ТаблицаДанных Цикл
		...
		РегистрацияПримечания(НСтр("ru = 'Статистика загрузки'", ОбщегоНазначения.КодОсновногоЯзыка()), Комментарий);
