...
Функция ПроверитьВозможностьИспользованияКриптопровайдера() Экспорт
	СистемнаяИнформация = Новый СистемнаяИнформация;
	ТекстВерсииОС = СистемнаяИнформация.ВерсияОС + СистемнаяИнформация.ТипПлатформы;
	ВерсияОСПоддерживается = ?(СтрНайти(ВРег(ТекстВерсииОС), ВРег("Windows")) > 0, Истина, Ложь);
	
	ВерсияПлатформы = ОбщегоНазначенияКлиентСервер.ВерсияКонфигурацииБезНомераСборки(СистемнаяИнформация.ВерсияПриложения);
	РазницаВерсий = ОбщегоНазначенияКлиентСервер.СравнитьВерсииБезНомераСборки(ВерсияПлатформы, "8.3.22");
	
	Поддерживается = РазницаВерсий < 0 ИЛИ ВерсияОСПоддерживается; 
...
Функция ВерноУказанУчетныйНомер(Знач УчетныйНомерДляПроверки) Экспорт
	...
	//Учетный номер валютного договора должен быть в формате X/XXX/XXXX/XXXXXXXXXX, где X - цифра, первый фасет должен быть 1, в последней фасет должен содержать от 1 до 20 цифр
	ФасетУчетныйНомер = "[1][/][0-9][0-9][0-9][/][0-9][0-9][0-9][0-9][/]";
