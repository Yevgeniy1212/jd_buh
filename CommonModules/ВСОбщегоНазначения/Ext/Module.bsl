...
Процедура УстановитьЗаголовокФормыДокумента(СтрокаВидаОперации = "", ДокументСсылка, ФормаДокумента) Экспорт
	...
	Если ДокументСсылка.Пустая() Тогда  
		ЗаголовокТекст = СтрШаблон(НСтр("ru='%1 Новый'"), Заголовок);
	Иначе
		Если ДокументСсылка.Проведен Тогда
			ЗаголовокТекст = СтрШаблон(НСтр("ru='%1 Проведен'"), Заголовок);
		ИначеЕсли ДокументСсылка.Метаданные().Проведение = Метаданные.СвойстваОбъектов.Проведение.Разрешить Тогда
			ЗаголовокТекст = СтрШаблон(НСтр("ru='%1 Не проведен'"), Заголовок);
		Иначе
			ЗаголовокТекст = СтрШаблон(НСтр("ru='%1 Записан'"), Заголовок);
		КонецЕсли;  				
	КонецЕсли;
	
	ФормаДокумента.Заголовок = ЗаголовокТекст;
