...
//
Функция ФорматСумм(Сумма, Валюта = Неопределено, ЧН = "", ЧРГ = "", ЧРД = "", ВалютаПрописью = Ложь, КодЯзыка="ru_RU") Экспорт
	...
	Если ЗначениеЗаполнено(Валюта) Тогда
		Если ВалютаПрописью и НЕ СокрЛП(Валюта.ПараметрыПрописиНаРусском) = "" Тогда 
			...
			ЗначимаяЧасть = ЧислоПрописью(Цел(Сумма),"Л=" + КодЯзыка + "; НП = Истина; ДП = Ложь", НовыеПараметрыПрописи);
			ЧисловаяЧасть = ЧислоПрописью(Цел(Сумма),"Л=" + КодЯзыка + "; НП = Ложь; ДП = Ложь", ",,,,,,,,0");
...
//
Функция СформироватьСуммуПрописью(Сумма, Валюта, КодЯзыка=Неопределено) Экспорт

	Если Валюта.ПараметрыПрописиНаРусском = "" Тогда
		Возврат ФорматСумм(Сумма);
	Иначе
		Возврат ЧислоПрописью(Сумма, ?(КодЯзыка <> Неопределено,"L="+КодЯзыка,"L=ru_RU"), Валюта.ПараметрыПрописиНаРусском);
	КонецЕсли;
...
Функция ПолучитьПаспортныеДанныеСтрокой(ФизЛицо, ПериодСреза = Неопределено) Экспорт
	...
	СтруктураПаспортныхДанных = РегистрыСведений.ПаспортныеДанныеФизЛиц.ПолучитьПоследнее(ПериодСреза, Новый Структура("ФизЛицо", ФизЛицо));
	...
	Если ЗначениеЗаполнено(Серия) Тогда
		Результат.Добавить(СтрШаблон(НСтр("ru = 'Серия: %1'"), Серия));
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Номер) Или ЗначениеЗаполнено(ДатаВыдачи) Или ЗначениеЗаполнено(КемВыдан) Тогда
		Результат.Добавить(СтрШаблон(НСтр("ru = '№ %1, Выдан: %2 года, %3'"), Номер, Формат(ДатаВыдачи,"ДФ='дд ММММ гггг'"), КемВыдан));
	КонецЕсли;
...
// Возвращает страну гражданства физ.лица
Функция ГражданствоФизЛица(ФизЛицо, ДатаПериода = '20200101')
	
	Результат = Справочники.КлассификаторСтранМира.Казахстан;
...
// Возвращает полные ФИО физ.лица
Функция ФИОФизЛица(ФизЛицо, ДатаПериода = '20200101')
	...
	|ИЗ
	|	РегистрСведений.ФИОФизЛиц.СрезПоследних(&Дата, ФизЛицо = &ФизЛицо) КАК ФИОФизЛиц
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|ВЫБРАТЬ
	|	1,
	...
	Если Выборка.Следующий() Тогда
		ФИО = Новый Массив;
		Если Не ПустаяСтрока(Выборка.Фамилия) Тогда
			ФИО.Добавить(Выборка.Фамилия);
		КонецЕсли;
		Если Не ПустаяСтрока(Выборка.Имя) Тогда
			ФИО.Добавить(Выборка.Имя);
		КонецЕсли;
		Если Не ПустаяСтрока(Выборка.Отчество) Тогда
			ФИО.Добавить(Выборка.Отчество);
		КонецЕсли;
		Результат = СтрСоединить(ФИО, " ");
	КонецЕсли;
...
Функция СведенияОЮрФизЛице(ЮрФизЛицо, ДатаПериода = '20200101', ДляФизЛицаТолькоИнициалы = Истина, БанковскийСчет = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(ЮрФизЛицо) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
		Сведения.ПолноеНаименование = ФИОФизЛица(ЮрФизЛицо, ?(ДатаПериода = NULL, ТекущаяДата(), ДатаПериода));
		Сведения.СтранаРезидентства = ГражданствоФизЛица(ЮрФизЛицо, ?(ДатаПериода = NULL, ТекущаяДата(), ДатаПериода));
		Сведения.СтранаРезидентстваКод2 = Сведения.СтранаРезидентства.КодАльфа2;
		Сведения.СтранаРезидентстваУчастникТаможенногоСоюза = Сведения.СтранаРезидентства.УчастникТаможенногоСоюза;
		Сведения.КБе = ?(Сведения.СтранаРезидентства = Справочники.КлассификаторСтранМира.Казахстан, "1", "2") + "9";
		Сведения.БИН_ИИН = ЮрФизЛицо.ИдентификационныйКодЛичности;
		Сведения.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо;
		Возврат Сведения

	ИначеЕсли ТипЗнч(ЮрФизЛицо) = Тип("СправочникСсылка.КартСчета") Тогда
		Если ЮрФизЛицо.НеЯвляетсяВладельцемСчета Тогда
			Возврат СведенияОЮрФизЛице(ЮрФизЛицо.ФизЛицоВладелецСчета, ?(ДатаПериода = NULL, ТекущаяДата(), ДатаПериода), ДляФизЛицаТолькоИнициалы, БанковскийСчет);
		Иначе
			Возврат СведенияОЮрФизЛице(ЮрФизЛицо.Владелец, ?(ДатаПериода = NULL, ТекущаяДата(), ДатаПериода), ДляФизЛицаТолькоИнициалы, БанковскийСчет);
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(ЮрФизЛицо) = Тип("СправочникСсылка.Организации")  Тогда
		...
		ИмяРеквизитаСтранаРезидентства = "СтранаРегистрации";
		...
		ИмяРеквизитаФизЛица = "ИндивидуальныйПредприниматель";
	...
	ИначеЕсли ТипЗнч(ЮрФизЛицо) = Тип("СправочникСсылка.Контрагенты") Тогда
		...
		ИмяРеквизитаСтранаРезидентства = "СтранаРезидентства";
	...
	Если Данные <> Неопределено Тогда
		...
		Если Данные.ЮрФизЛицо = ПредопределенноеЗначение("Перечисление.ЮрФизЛицо.ЮрЛицо") Тогда
			Сведения.СтранаРезидентства = Запрос.Параметры["ПарЮрФизЛицо"][ИмяРеквизитаСтранаРезидентства];
			Если Не ЗначениеЗаполнено(Сведения.СтранаРезидентства) Тогда
				Сведения.СтранаРезидентства = Справочники.КлассификаторСтранМира.Казахстан;
			КонецЕсли;
		Иначе
			Сведения.СтранаРезидентства = ГражданствоФизЛица(Запрос.Параметры["ПарЮрФизЛицо"][ИмяРеквизитаФизЛица], ?(ДатаПериода = NULL, ТекущаяДата(), ДатаПериода));
		КонецЕсли;
		Сведения.СтранаРезидентстваКод2 = Сведения.СтранаРезидентства.КодАльфа2;
		...
		Если РезультатЗапроса.Колонки.Найти("ДатаСвидетельстваПоНДС") <> Неопределено Тогда
			...
			Сведения.Вставить("СвидетельствоПоНДС"     , СтрШаблон(НСтр("ru = ' серия %1, № %2, от %3'"),
				Данные.СерияСвидетельстваПоНДС,
				Данные.НомерСвидетельстваПоНДС,
				Формат(Данные.ДатаСвидетельстваПоНДС, "ДФ=dd.MM.yyyy")));
		КонецЕсли;
...
Функция ОписаниеОрганизации(СписокСведений, Список = "", СПрефиксом = Истина, Дата = Неопределено, КодЯзыка = "ru") Экспорт
	...
	СоответствиеПараметров.Вставить("РНН",            	  "ru = "" РНН ""; kk = "" СТН """);
	СоответствиеПараметров.Вставить("БИН_ИИН",            "ru = "" БИН / ИИН ""; kk = "" БСН / ЖСН """);

	СоответствиеПараметров.Вставить("ЮридическийАдрес",   " ");
	СоответствиеПараметров.Вставить("Телефоны",           "ru = "" тел.: ""; kk = "" тел.: """);
	СоответствиеПараметров.Вставить("НомерСчета",         "ru = """"; kk = """"");
	СоответствиеПараметров.Вставить("Банк",               "ru = "" в банке ""; kk = "" банктегі """);
	СоответствиеПараметров.Вставить("БИК",                "ru = "" БИК ""; kk = "" БСК """);
	СоответствиеПараметров.Вставить("КоррСчет",           "ru = "" к/с ""; kk = "" к/ш """);
	СоответствиеПараметров.Вставить("КодПоОКПО",          "ru = "" Код по ОКПО ""; kk = "" ҚҰЖЖ бойынша коды """);
	СоответствиеПараметров.Вставить("НомерРегистрацииВСтранеРезидентства", "ru = "" ИНН/УНП ""; kk = "" ИНН/УНП """); // Учетные данные контрагентов из РФ и РБ
	...
	Для Счетчик = 1 по ЧислоПараметров Цикл
		...
		Если ПозЗапятой > 0  Тогда
			...
			Попытка
				...
				Если ИмяПараметра = "Банк" И КодЯзыка = "kk" Тогда 
			...
			Исключение
				...
				ОбщегоНазначения.СообщитьПользователю(СтрШаблон(НСтр("ru = 'Не удалось определить значение параметра организации: %1'"), ИмяПараметра));
