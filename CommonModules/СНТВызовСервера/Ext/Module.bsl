...
Функция ВыполнитьЗапросНаИзменениеСтатусов(Знач Действие, Знач ТекстЗапроса, Знач ПрофильИСЭСФ, ИдентификаторСессии = Неопределено, Знач СоответствиеПодписейСНТ = Неопределено, ДанныеКлючаЭЦП = Неопределено) Экспорт
	
	Возврат СНТСерверПовтИсп.ОбработкаОбменСНТ().ВыполнитьЗапросНаИзменениеСтатусовСНТ(Действие, ТекстЗапроса, ПрофильИСЭСФ, ИдентификаторСессии, СоответствиеПодписейСНТ, ДанныеКлючаЭЦП);
	
КонецФункции

Функция ВыполнитьЗапросНаИзменениеСтатусовВФоне(Знач ПараметрыВыгрузки, Знач АдресХранилища) Экспорт
	
	ДанныеИзмененияСтатусов = ВыполнитьЗапросНаИзменениеСтатусов(ПараметрыВыгрузки.Действие, ПараметрыВыгрузки.ТекстЗапроса, ПараметрыВыгрузки.ДанныеПрофиляИСЭСФ,, ПараметрыВыгрузки.СоответствиеПодписейСНТ, ПараметрыВыгрузки.ДанныеКлючаЭЦП);
	...
	Если СоответствиеДляОбновленияАктуальныхОснований.Количество() <> МассивСНТ.Количество() Тогда
		...
		Если ОтсутствуетАктуальноеОснование.Количество() <> 0 Тогда		
			...
			ТекстСообщения = ТекстСообщения + ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр(
	            ...
	            |%1
	            |Так как эти СНТ не отражены в учете.'"), СокрЛП(СписокСНТ));
		...
		Если НедопустимоеСостояние.Количество() <> 0 Тогда
			...
			ТекстСообщения = ТекстСообщения + ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр(
				...
				|Возможно обновление СНТ только в состоянии ""%2"", ""%3"".'"),СокрЛП(СписокСНТ),Перечисления.СостоянияСНТ.Сформирован,Перечисления.СостоянияСНТ.ОтклоненСервером);
		КонецЕсли;
