...
Функция ПакетыСтраниц(НосительМакетов) Экспорт
	...
	// Локализация_РК (для получения макета Описатель на текущем языке пользователя)
	Результат.Колонки.Добавить("КодЯзыка",                     Новый ОписаниеТипов("Строка"));
	...
	// Локализация_РК (для получения макета Описатель на текущем языке пользователя) 
	ТабличныйДокумент.КодЯзыка = ТекущийЯзык().КодЯзыка;
	Для НомерСтроки = 3 По ТабличныйДокумент.ВысотаТаблицы Цикл
		...
		// Локализация_РК (для получения макета Описатель на текущем языке пользователя)	
		СтрокаТаблицы.КодЯзыка                 = ТекущийЯзык().КодЯзыка;
	КонецЦикла;
