...
Функция НовыйМассивОтзываемыхЭСФ(Знач МассивСчетовФактур) Экспорт
	...
	Если МассивОтзываемыхЭСФ.Количество() <> МассивСчетовФактур.Количество() Тогда
		...
		Если ОтсутствуетАктуальныйЭСФ.Количество() <> 0 Тогда
			...
			ТекстСообщения = ТекстСообщения + ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр(
				"ru = 'Невозможно отозвать актуальные электронные счета-фактуры для счетов-фактур:
                 |%1
                 |Так как счета-фактуры не имеют актуальных электронных счетов-фактур.'"), СокрЛП(СписокСчетовФактур));
		...
		Если НедопустимоеСостояниеЭСФ.Количество() <> 0 Тогда
			...
			ТекстСообщения = ТекстСообщения + ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр(
				"ru = 'Невозможно отозвать актуальные электронные счета-фактуры для счетов-фактур:
                 ...
                 |Возможен отзыв электронных счетов фактур только в состоянии ""%2"", ""%3"".'"), СокрЛП(СписокСчетовФактур), Перечисления.СостоянияЭСФ.ПринятСервером, Перечисления.СостоянияЭСФ.ДоставленПолучателю);
...
Функция СоздатьИОтправитьКоллекциюInvoice(КоллекцияСгруппированныхЭСФ, ДанныеПрофилейИСЭСФ, ДополнительныеПараметры, ВерсияИСЭСФ) Экспорт
	...
	Для Каждого СгруппированныеЭСФ Из КоллекцияСгруппированныхЭСФ Цикл
		...
		Если КоллекцияПодписей.Количество() = 0 Тогда
			...
			ЭСФКлиентСерверПереопределяемый.СообщитьПользователю(ТекстОшибки);
			// Не помещаем в очередь отправки, так как сведения для подписи мы не получили
			ЭСФВызовСервера.ОчиститьОчередьОтправкиЭСФ(КоллекцияСгруппированныхЭСФ);
			Возврат Неопределено;
		КонецЕсли;
...
Функция НовоеСоответствиеДляОбновленияАктуальныхЭСФ(Знач МассивСчетовФактур) Экспорт
	...
	Если СоответствиеДляОбновленияАктуальныхЭСФ.Количество() <> МассивСчетовФактур.Количество() Тогда
		...
		Если НедопустимоеСостояниеЭСФ.Количество() <> 0 Тогда
			...
			|Возможно обновление электронных счетов фактур только в состоянии ""%2"", ""%3"".'"),СокрЛП(СписокСчетовФактур), Перечисления.СостоянияЭСФ.Сформирован, Перечисления.СостоянияЭСФ.ОтклоненСервером);
...
Процедура ОбновитьАктуальныеЭСФ(Знач СоответствиеДляОбновленияАктуальныхЭСФ) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'ЭСФВызовСервера.ОбновитьАктуальныеЭСФ'", ОбщегоНазначения.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ПолучитьДополнительныеСведенияКриптопровайдера() Экспорт
	...
	Возврат ЭСФКлиентСервер.ПолучитьДополнительныеСведенияКриптопровайдера();
	
КонецФункции

//См. ЭСФСервер.ПолучитьСохраненныеПараметрыСоглашения
Функция ПолучитьСохраненныеПараметрыСоглашения() Экспорт
	Возврат ЭСФСервер.ПолучитьСохраненныеПараметрыСоглашения();	
КонецФункции
...
// Подготавливает текст гиперссылки передаваемого элемента для списка оповещений ИС ЭСФ.
//
// Параметры:
//  ПользовательИСЭСФ - СправочникСсылка.СчетФактураПолученный - устанавливает отбор оповещений по пользователю ИС ЭСФ
//  СокращенныВид - Булево - устанавливает отбор оповещений по пользователю ИС ЭСФ
//
// Возвращаемое значение:
//  Строка - текст заголовка с информацией о количестве оповещений
//
Функция ПолучитьТекстГиперссылкиОповещений(ПользовательИСЭСФ = Неопределено, СокращенныВид = Истина) Экспорт

	Запрос = Новый Запрос;
	ВыбранПользовательИСЭСФ = ЗначениеЗаполнено(ПользовательИСЭСФ);
	Запрос.УстановитьПараметр(
		"ПользовательОтправки", 
		?(ВыбранПользовательИСЭСФ, ПользовательИСЭСФ, Справочники.ПользователиИСЭСФ.ПустаяСсылка())
	);
	
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ОповещенияИСЭСФ.ИДСообщения) КАК КоличествоНепрочитанных
		|ИЗ
		|	РегистрСведений.ОповещенияИСЭСФ КАК ОповещенияИСЭСФ
		|ГДЕ
		|	НЕ ОповещенияИСЭСФ.Прочитано
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	Если ЗначениеЗаполнено(ПользовательИСЭСФ) Тогда
		...
		|	И ОповещенияИСЭСФ.ПользовательИСЭСФ = &ПользовательОтправки";
	...
	ЕстьЗаписи = ВыборкаОбщееКоличество.КоличествоНепрочитанных <> 0;
	ЕстьЗаписиПользователя = ВыборкаКоличествоПоПользователю.Следующий();
	
	Если ЕстьЗаписи Тогда
		
		КоличествоНепрочитанныхПоПользователю = ?(ЕстьЗаписиПользователя, ВыборкаКоличествоПоПользователю.КоличествоНепрочитанных, 0);
		
		Если ЗначениеЗаполнено(ПользовательИСЭСФ) Тогда
			ТекстГиперссылки = ?(СокращенныВид,
				ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Оповещения ИС ЭСФ (+%1 всего, +%2 для пользователя)'"),ВыборкаОбщееКоличество.КоличествоНепрочитанных,КоличествоНепрочитанныхПоПользователю),
				ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Оповещения ИС ЭСФ (непрочитанных %1, из них для выбранного пользователя %2)'"),ВыборкаОбщееКоличество.КоличествоНепрочитанных,КоличествоНепрочитанныхПоПользователю)
			);
		Иначе
			ТекстГиперссылки = ?(СокращенныВид,
				ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Оповещения ИС ЭСФ (+%1)'"),ВыборкаОбщееКоличество.КоличествоНепрочитанных),
				ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Оповещения ИС ЭСФ (непрочитанных %1)'"),ВыборкаОбщееКоличество.КоличествоНепрочитанных)
			);
		КонецЕсли;

		Заголовок = ТекстГиперссылки;
	Иначе
		ТекстГиперссылки = НСтр("ru = 'Оповещения ИС ЭСФ'");
		Заголовок = ТекстГиперссылки;
	КонецЕсли;
...
// Вызывает процедуру ПолучитьНовыеУведомления для фоновых заданий
Функция ПолучитьНовыеУведомления(Знач ПараметрыВыгрузки, Знач АдресХранилища = Неопределено) Экспорт
	
	Возврат ЭСФСервер.ПолучитьНовыеУведомления(ПараметрыВыгрузки);
