...
Функция ПространствоИменАсинхронногоОбмена(ВерсияФормата) Экспорт
	...
	ИначеЕсли ВерсияФормата = "2.05" ИЛИ ВерсияФормата = "2.06" Тогда
...
Процедура СоздатьЭДПакетаAsync(Конверт, СвойстваСертификатов = Неопределено) Экспорт
	...
	Попытка
		...
		Для каждого Строка Из Конверт.Сообщения Цикл
			...
			Если ВидЭД = Перечисления.ВидыЭДОбменСБанками.ПлатежноеПоручение И РеквизитыНастройкиОбмена.ВерсияФормата <> "2.06" Тогда
				ВидЭДПакета = "10"
			...
			ИначеЕсли ВидЭД = Перечисления.ВидыЭДОбменСБанками.ПоручениеНаПереводВалюты Тогда
				ВидЭДПакета = "30" 
			ИначеЕсли РеквизитыНастройкиОбмена.ВерсияФормата = "2.06" Тогда
				ВидЭДПакета = "51"
			КонецЕсли;
...
Процедура СформироватьПлатежноеПоручение(
	...
	ИначеЕсли (ПрограммаБанка = Перечисления.ПрограммыБанка.АсинхронныйОбмен
		...
		Если РеквизитыНастройкиОбмена.ВерсияФормата = "2.05" Тогда
			СформироватьПлатежноеПоручениеAsync(СсылкаНаОбъект, ДеревоДокумента, АдресФайлаВоВременномХранилище);
		ИначеЕсли РеквизитыНастройкиОбмена.ВерсияФормата = "2.06" Тогда
			СформироватьПлатежноеПоручениеAsyncISO(СсылкаНаОбъект, ДеревоДокумента, АдресФайлаВоВременномХранилище);
		КонецЕсли; 
...
Функция СоздатьНастройкуОбменаИзФайла(НастройкиЗаполнения, Организация,
	...
	Если ЗаполненыНаименования Тогда 	
		Объект.Наименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1 - %2", Строка(НаименованиеОрганизации),Строка(НаименованиеБанка));
	Иначе		
	...
	Попытка
		...
		Если ВРег(ЭД.Тип().Имя) = ВРег("anyType") Тогда // не указан тип элемента в явном виде
			...
			ИначеЕсли ЭД.Свойства().Получить("CstmrCdtTrfInitn") <> Неопределено Тогда
				Фабрика = ОбменСБанкамиСлужебныйПовтИсп.ФабрикаAsyncXDTO("2.06");
				ЭД = Фабрика.ПрочитатьXML(ОбъектXML);
				ПрочитатьПлатежноеПоручениеAsyncISO(ЭД, ДеревоРазбора, НовыйЭД);
...
Процедура ПрочитатьОперациюВыписки(ЭД, Operation, НовыйЭД)
	...
	Если Payer <> Неопределено Тогда
		...
		CtryOfRes = ЗначениеСвойстваXDTO(Payer, "CtryOfRes");
		Если CtryOfRes <> Неопределено Тогда
			РеквизитыПлатежа.Добавить(Payer.CtryOfRes, "ПлательщикКодСтраныРегистрации");
		КонецЕсли;
		
		UltmtPayer = ЗначениеСвойстваXDTO(Payer, "UltmtPayer");
		Если UltmtPayer <> Неопределено Тогда
			РеквизитыПлатежа.Добавить(ЗначениеСвойстваXDTO(UltmtPayer, "Name"), "ФактическийПлательщикНаименование");
			РеквизитыПлатежа.Добавить(ЗначениеСвойстваXDTO(UltmtPayer, "INN"), "ФактическийПлательщикИНН");
		КонецЕсли;
	...
	Если Payee <> Неопределено Тогда
		...
		CtryOfRes = ЗначениеСвойстваXDTO(Payee, "CtryOfRes");
		Если CtryOfRes <> Неопределено Тогда
			РеквизитыПлатежа.Добавить(Payee.CtryOfRes, "ПолучательКодСтраныРегистрации");
		КонецЕсли;
...
Процедура СформироватьПлатежноеПоручениеAsyncISO(СсылкаНаДокумент, ДеревоДанных, АдресФайлаВоВременномХранилище)
	
	ВерсияФормата = "2.06";

	Фабрика = ОбменСБанкамиСлужебныйПовтИсп.ФабрикаAsyncXDTO(ВерсияФормата);

	ПространствоИмен = "urn:iso:std:iso:20022:tech:xsd:pain.001.001.11";

	ТекстОшибки = "";
	
	CstmrCdtTrfInitn = ОбъектТипаCML(Фабрика, "Document", ПространствоИмен);
	СообщениеЗаголовок = ОбъектТипаCML(Фабрика, "CustomerCreditTransferInitiationV11", ПространствоИмен);
	GrpHdr = ОбъектТипаCML(Фабрика, "GroupHeader95", ПространствоИмен);
	ИННПлательщика = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПлательщика.ИНН");
	ЗаполнитьСвойствоXDTO(GrpHdr, "MsgId", "MIDORG" + ИННПлательщика + "N0001", Истина, ТекстОшибки);//MsgId - Идентификатор сообщения, обязательный
	Дата = ТекущаяДатаСеанса();
	ЗаполнитьСвойствоXDTO(GrpHdr, "CreDtTm", ТекущаяДатаСеанса(), Истина, ТекстОшибки);//CreDtTm - Дата и время создания сообщения, обязательный
	ПереданСписокСотрудников = ЗначениеРеквизитаВДереве(ДеревоДанных, "ПереданСписокСотрудников");
	СписокСотрудников = ЗначениеРеквизитаВДереве(ДеревоДанных, "СписокСотрудников");
	КоличествоОтдельныхПлатежей = Строка(?(ПереданСписокСотрудников, СписокСотрудников, 1));
	ЗаполнитьСвойствоXDTO(GrpHdr, "NbOfTxs", КоличествоОтдельныхПлатежей, Истина, ТекстОшибки);//NbOfTxs - Количество отдельных платежей, включенных в сообщение, обязательный
	Сумма = ЗначениеРеквизитаВДереве(ДеревоДанных, "Сумма");
	ЗаполнитьСвойствоXDTO(GrpHdr, "CtrlSum", Сумма, Истина, ТекстОшибки);//CtrlSum - Итоговая сумма отдельных сумм, включенных в группу информации платежа, рассчитанная независимо от валют

	InitgPty = ОбъектТипаCML(Фабрика, "PartyIdentification135", ПространствоИмен);//InitgPty - Инициатор платежа. Может быть как плательщик, так и сторона, инициирующая платежь от его имени. Обязательный
	ЗаполнитьПлательщика(InitgPty, ДеревоДанных, Фабрика, ПространствоИмен, ТекстОшибки);
	
	ВидОперации = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПлатежа.ВидОплаты");
	ВалютаДокумента = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПлатежа.ВалютаДокумента");
	
	Если ВидОперации = "41" Тогда //ПеречислениеЗаработнойПлаты
		
		ЗаполнитьИнформациюПлатежаЗаработнойПлаты(СообщениеЗаголовок, ВалютаДокумента, ДеревоДанных, Фабрика, ПространствоИмен, ТекстОшибки);
		
	ИначеЕсли ВидОперации = "42" ИЛИ ВидОперации = "43" Тогда //ПеречислениеСоциальныхОтчислений/ПеречислениеПенсионныхВзносов
		
		ЗаполнитьИнформациюПлатежаПоВзносамОтчислениямЕП(СообщениеЗаголовок, ВалютаДокумента, ДеревоДанных, Фабрика, ПространствоИмен, ТекстОшибки);
		
	ИначеЕсли ВидОперации = "18" Тогда //ПеречислениеПоИсполнительнымЛистам
		
		ЗаполнитьИнформациюПлатежаИсполнительногоЛиста(СообщениеЗаголовок, ВалютаДокумента, ДеревоДанных, Фабрика, ПространствоИмен, ТекстОшибки);
		
	Иначе //Для налоговых и прочих платежей (посмотреть кроме налоговых кто ещё сюда зайдёт!!!!!!!!!!!!!!)
		
		ЗаполнитьИнформациюПлатежаПоНалоговымИПрочимВыплатам(СообщениеЗаголовок, ВалютаДокумента, ДеревоДанных, Фабрика, ПространствоИмен, ТекстОшибки);
		
	КонецЕсли;

	ЗаполнитьСвойствоXDTO(GrpHdr, "InitgPty", InitgPty, Истина, ТекстОшибки);
	ЗаполнитьСвойствоXDTO(СообщениеЗаголовок, "GrpHdr", GrpHdr, Истина, ТекстОшибки);

	ЗаполнитьСвойствоXDTO(CstmrCdtTrfInitn, "CstmrCdtTrfInitn", СообщениеЗаголовок, Истина, ТекстОшибки);

	CstmrCdtTrfInitn.Проверить();

	Если ЗначениеЗаполнено(ТекстОшибки) Тогда
		ВызватьИсключение ТекстОшибки;
	Иначе
		ДвоичныеДанные = ДвоичныеДанныеИзXDTO(Фабрика, CstmrCdtTrfInitn);
		АдресФайлаВоВременномХранилище = ПоместитьВоВременноеХранилище(ДвоичныеДанные);
	КонецЕсли;
...
Процедура ПроверитьРеквизитыОрганизации(ДеревоДанных, Ошибки)
	...
	ИННОрганизации = СокрЛП(ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПлательщика.ИНН", , НайденнаяСтрокаРеквизита));
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ИННОрганизации)
	...
	КБеОрганизации = СокрЛП(ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПлательщика.КБЕ", , НайденнаяСтрокаРеквизита));
	Если НЕ ЗначениеЗаполнено(КБеОрганизации) Тогда
		ТекстОшибки = НСтр("ru = 'Не заполнено значение КБе организации.'");
		ЭлектронноеВзаимодействиеСлужебный.ДобавитьОшибкуПроверкиДереваДанных(
		Ошибки, НайденнаяСтрокаРеквизита, ТекстОшибки);
	КонецЕсли;
	
	СтранаРегистрацииПлательщика = СокрЛП(ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПлательщика.ПлательщикКодСтраныРегистрации", , НайденнаяСтрокаРеквизита));
	Если НЕ ЗначениеЗаполнено(СтранаРегистрацииПлательщика) Тогда
		ТекстОшибки = НСтр("ru = 'Не заполнено значение код страны организации.'");
		ЭлектронноеВзаимодействиеСлужебный.ДобавитьОшибкуПроверкиДереваДанных(
		Ошибки, НайденнаяСтрокаРеквизита, ТекстОшибки);
	КонецЕсли;
...
Процедура ПроверитьРеквизитыКонтрагента(ДеревоДанных, Ошибки)
	...
	ИННКонтрагента = СокрЛП(ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПолучателя.ИНН", , НайденнаяСтрокаРеквизита));
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ИННКонтрагента)
	...
	КБеКонтрагента = СокрЛП(ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПолучателя.КБЕ", , НайденнаяСтрокаРеквизита));
	Если НЕ ЗначениеЗаполнено(КБеКонтрагента) Тогда
		ТекстОшибки = НСтр("ru = 'Не заполнено значение КБе контрагента.'");
		ЭлектронноеВзаимодействиеСлужебный.ДобавитьОшибкуПроверкиДереваДанных(
		Ошибки, НайденнаяСтрокаРеквизита, ТекстОшибки);
	КонецЕсли;
	
	СтранаРегистрацииБенефициара = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПолучателя.ПолучательКодСтраныРегистрации");
	Если НЕ ЗначениеЗаполнено(СтранаРегистрацииБенефициара) Тогда
		ТекстОшибки = НСтр("ru = 'Не заполнено значение код страны контрагента.'");
		ЭлектронноеВзаимодействиеСлужебный.ДобавитьОшибкуПроверкиДереваДанных(
		Ошибки, НайденнаяСтрокаРеквизита, ТекстОшибки);
	КонецЕсли;
...
Функция БанкСбербанк(ДеревоДанных, Путь, ТекстОшибки)
	...
	Иначе
		ДопОшибка = СтрШаблон(НСтр("ru = 'Некорректные данные в поле ""Город"" банка %1'"), НаименованиеБанка);
...
Процедура ПрочитатьПлатежноеПоручениеAsyncISO(ЭД, ДеревоРазбора, НовыйЭД)
	
	НовыйЭД.ВидЭД = Перечисления.ВидыЭДОбменСБанками.ПлатежноеПоручение;
	
	ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ДатаДокумента", ЭД.CstmrCdtTrfInitn.GrpHdr.CreDtTm);
	ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "СуммаЧислом", ЭД.CstmrCdtTrfInitn.GrpHdr.CtrlSum);
	ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ИдентификаторДокумента", ЭД.CstmrCdtTrfInitn.GrpHdr.MsgId);
	
	ИнформацияОПлатеже = ЭД.CstmrCdtTrfInitn.PmtInf;
	
	РеквизитыПоПолучателю = ЭД.CstmrCdtTrfInitn.PmtInf[0].CdtTrfTxInf[0];
	
	ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "НаименованиеНомер", РеквизитыПоПолучателю.RmtInf.Strd[0].RfrdDocInf[0].Nb);

	РеквизитыPrvtId = ?(РеквизитыПоПолучателю.Cdtr.Id.PrvtId <> Неопределено, Истина, Ложь);//Перечисление заработной платы
	РеквизитыOrgId = ?(РеквизитыПоПолучателю.Cdtr.Id.OrgId <> Неопределено, Истина, Ложь);//Перечисление взносов и отчислений соц. страхования, Перечисление пенсионных взносов и другие.
	
	Если РеквизитыPrvtId ИЛИ (РеквизитыOrgId И ЭД.CstmrCdtTrfInitn.GrpHdr.NbOfTxs > "1") Тогда
		
		ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ТипМакетаМХ", "Сводный");//Тип платёжного поручения, служебное, для определения в дальнейшем макета
		ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "КоличествоПП", ЭД.CstmrCdtTrfInitn.GrpHdr.NbOfTxs); 
		
		СписокСотрудников = ПолучитьПустуюТЗПлатёжногоПоручения();
		
		Для каждого Платёж Из ИнформацияОПлатеже Цикл
			
			БИКПлательщика = Платёж.DbtrAgt.FinInstnId.BICFI;
			ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "БИКБанкаПлательщика", БИКПлательщика);

			СсылкаБанкПлательщика = ЭлектронноеВзаимодействие.НайтиСсылку("Банки", БИКПлательщика);//поиск наименования банка-ссылка по БИК, т.к.в хмл файл не записываем наименование банка
			БанкПлательщика = ?(НЕ ЗначениеЗаполнено(СсылкаБанкПлательщика), "", СсылкаБанкПлательщика.Наименование);
			ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "БанкПлательщика", БанкПлательщика);
			
			БИКПолучателя = Платёж.CdtTrfTxInf[0].CdtrAgt.FinInstnId.BICFI;
			ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "БИКБанкаПолучателя", БИКПолучателя);
			
			СсылкаБанкПолучателя = ЭлектронноеВзаимодействие.НайтиСсылку("Банки", БИКПолучателя);//поиск наименования банка-ссылка по БИК, т.к.в хмл файл не записываем наименование банка
			БанкПолучателя = ?(НЕ ЗначениеЗаполнено(СсылкаБанкПолучателя), "", СсылкаБанкПолучателя.Наименование);
			ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "БанкПолучателя", БанкПолучателя);
			
			// В исполнтельных листах контрагент может быть юр./физ. лицо
			ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ИИК", ?(РеквизитыOrgId ИЛИ (РеквизитыPrvtId И Платёж.CdtTrfTxInf[0].Cdtr.Id.OrgId = Неопределено), Платёж.CdtTrfTxInf[0].CdtrAcct.Id.IBAN, Платёж.CdtTrfTxInf[0].CdtrAgtAcct.Id.IBAN));
			
			РеквизитыПлательщикаРукводитель = Платёж.Dbtr.Id.OrgId.Othr.Получить(3).Id;
			ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ФИОРуководителя", РеквизитыПлательщикаРукводитель);
			РеквизитыПлательщикаГлавБух = Платёж.Dbtr.Id.OrgId.Othr.Получить(4).Id;
			ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ФИОГлавногоБухгалтера", РеквизитыПлательщикаГлавБух); 
			ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ДатаВалютирования", Формат(Платёж.CdtTrfTxInf[0].RmtInf.Strd[0].RfrdDocInf[0].RltdDt, НСтр("ru='ДФ=''дд.MM.гггг'''")));
			
			СтрокаСотрудника = СписокСотрудников.Добавить();
			
			СтрокаСотрудника.Отправитель = Платёж.Dbtr.Nm;
			РеквизитыПлательщикаИНН = Платёж.Dbtr.Id.OrgId.Othr.Получить(0).Id;
			СтрокаСотрудника.ОтправительБИН = РеквизитыПлательщикаИНН;
			РеквизитыПлательщикаКБЕ = Платёж.Dbtr.Id.OrgId.Othr.Получить(1).Id + Платёж.Dbtr.Id.OrgId.Othr.Получить(2).Id;
			СтрокаСотрудника.ПлательщикКБЕ = РеквизитыПлательщикаКБЕ;
			СтрокаСотрудника.НомерСчетаПлательщика = Платёж.DbtrAcct.Id.IBAN;
			СтрокаСотрудника.БанкПлательщика = БанкПлательщика;
			СтрокаСотрудника.БИКБанкаПлательщика = Платёж.DbtrAgt.FinInstnId.BICFI;
			СтрокаСотрудника.ФИОРуководителя = РеквизитыПлательщикаРукводитель;
			СтрокаСотрудника.ФИОГлавногоБухгалтера = РеквизитыПлательщикаГлавБух;
			РеквизитыПлательщикаРезиденство = Платёж.Dbtr.Id.OrgId.Othr.Получить(1).Id;
			Если РеквизитыПлательщикаРезиденство <> "1" Тогда
				СтрокаСотрудника.СтранаРезидентстваОтправителя = Платёж.Dbtr.CtryOfRes;
			КонецЕсли;
						
			Если Платёж.UltmtDbtr <> Неопределено Тогда
				СтрокаСотрудника.Плательщик = Платёж.UltmtDbtr.Nm;
				Если Платёж.UltmtDbtr.Id.OrgId <> Неопределено Тогда
					РеквизитыФактическогоПлательщикаИНН = Платёж.UltmtDbtr.Id.OrgId.Othr.Получить(0).Id;
				ИначеЕсли Платёж.UltmtDbtr.Id.PrvtId <> Неопределено Тогда
					РеквизитыФактическогоПлательщикаИНН = Платёж.UltmtDbtr.Id.PrvtId.Othr.Получить(0).Id;
				КонецЕсли;
				СтрокаСотрудника.ПлательщикБИН = РеквизитыФактическогоПлательщикаИНН;
			КонецЕсли;
			
			СтрокаСотрудника.Получатель = Платёж.CdtTrfTxInf[0].Cdtr.Nm;
			
			Если РеквизитыOrgId Тогда 
				РеквизитыПолучателяИНН = РеквизитыПоПолучателю.Cdtr.Id.OrgId.Othr.Получить(0).Id;
				РеквизитыПолучателяКБЕ = РеквизитыПоПолучателю.Cdtr.Id.OrgId.Othr.Получить(1).Id + РеквизитыПоПолучателю.Cdtr.Id.OrgId.Othr.Получить(2).Id;
				РеквизитыПолучателяРезиденство = РеквизитыПоПолучателю.Cdtr.Id.OrgId.Othr.Получить(1).Id;
				Если Платёж.CdtTrfTxInf[0].UltmtCdtr <> Неопределено Тогда
					СтрокаСотрудника.КонечныйПолучатель = Платёж.CdtTrfTxInf[0].UltmtCdtr.Nm;
					СтрокаСотрудника.КонечныйПолучательБИН = Платёж.CdtTrfTxInf[0].UltmtCdtr.Id.PrvtId.Othr[0].Id;
				КонецЕсли;
				
				Если Платёж.CdtTrfTxInf[0].RmtInf.Strd[0].TaxRmt <> Неопределено Тогда
					ПериодГод = Платёж.CdtTrfTxInf[0].RmtInf.Strd[0].TaxRmt.Rcrd[0].Prd.Yr;
					ПериодМесяц = Прав(Платёж.CdtTrfTxInf[0].RmtInf.Strd[0].TaxRmt.Rcrd[0].Prd.Tp, 2);//отсекли от значение буквы ММ, оставили только месяц
					СтрокаСотрудника.Период = Формат(ПериодМесяц+"."+ПериодГод, НСтр("ru='ДФ=''MM.гггг'''"));
				КонецЕсли;
			КонецЕсли;
			
			Если РеквизитыPrvtId И Платёж.CdtTrfTxInf[0].Cdtr.Id.OrgId = Неопределено Тогда
				РеквизитыПолучателяИНН = Платёж.CdtTrfTxInf[0].Cdtr.Id.PrvtId.Othr.Получить(0).Id;
				РеквизитыПолучателяРезиденство = Платёж.CdtTrfTxInf[0].Cdtr.Id.PrvtId.Othr.Получить(1).Id;
				Если Платёж.CdtTrfTxInf[0].Cdtr.Id.PrvtId.Othr.Количество()>2 И Платёж.CdtTrfTxInf[0].Cdtr.Id.PrvtId.Othr.Получить(2) <> Неопределено Тогда 
					РеквизитыПолучателяКБЕ = Платёж.CdtTrfTxInf[0].Cdtr.Id.PrvtId.Othr.Получить(1).Id + Платёж.CdtTrfTxInf[0].Cdtr.Id.PrvtId.Othr.Получить(2).Id;//исполнительные листы
				Иначе	
					РеквизитыПолучателяКБЕ = ?(РеквизитыПолучателяРезиденство <> "1", "2", "1") + "9";//зарплата
				КонецЕсли;
			КонецЕсли;
			
			Если Платёж.CdtTrfTxInf[0].Cdtr.Id.OrgId <> Неопределено Тогда //в исполнительных листах могут быть и физ. и юр. лица
				РеквизитыПолучателяИНН = Платёж.CdtTrfTxInf[0].Cdtr.Id.OrgId.Othr.Получить(0).Id;
				РеквизитыПолучателяКБЕ = Платёж.CdtTrfTxInf[0].Cdtr.Id.OrgId.Othr.Получить(1).Id + Платёж.CdtTrfTxInf[0].Cdtr.Id.OrgId.Othr.Получить(2).Id;
				РеквизитыПолучателяРезиденство = Платёж.CdtTrfTxInf[0].Cdtr.Id.OrgId.Othr.Получить(1).Id;
			КонецЕсли;
			
			СтрокаСотрудника.ПолучательБИН = РеквизитыПолучателяИНН;
			СтрокаСотрудника.ПолучательКБЕ = РеквизитыПолучателяКБЕ;
			СтрокаСотрудника.НомерСчетаПолучателя = Платёж.CdtTrfTxInf[0].CdtrAcct.Id.IBAN;
			СтрокаСотрудника.БанкПолучателя = БанкПолучателя;
			СтрокаСотрудника.БИКБанкаПолучателя = БИКПолучателя;
			Если РеквизитыПолучателяРезиденство <> "1" Тогда
				СтрокаСотрудника.СтранаРезидентстваПолучателя = Платёж.CdtTrfTxInf[0].Cdtr.CtryOfRes;
			КонецЕсли;
			СтрокаСотрудника.ИИК = Платёж.CdtTrfTxInf[0].CdtrAcct.Id.IBAN;
			СтрокаСотрудника.СуммаПрописью = Платёж.CdtTrfTxInf[0].Amt.InstdAmt.__content;
			СтрокаСотрудника.СуммаЧислом = Платёж.CdtTrfTxInf[0].Amt.InstdAmt.__content;
			СписокXDTO = РеквизитыПоПолучателю.RmtInf.Strd[0].AddtlRmtInf;
			НазначениеПлатежа = "";
			Для каждого Строка Из СписокXDTO Цикл
				НазначениеПлатежа = НазначениеПлатежа + Строка;
			КонецЦикла;
			СтрокаСотрудника.НазначениеПлатежа = НазначениеПлатежа;
			СтрокаСотрудника.КодНазначенияПлатежа = Платёж.CdtTrfTxInf[0].Purp.Cd;
			СтрокаСотрудника.ДатаВалютирования = Формат(Платёж.CdtTrfTxInf[0].RmtInf.Strd[0].RfrdDocInf[0].RltdDt, НСтр("ru='ДФ=''дд.MM.гггг'''"));
			
		КонецЦикла;
		
		ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "СписокСотрудников", СписокСотрудников);
		
	Иначе 
		
		ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ТипМакетаМХ", "Единичный");//Тип платёжного поручения, служебное, для определения в дальнейшем макета

		Если ИнформацияОПлатеже[0].UltmtDbtr <> Неопределено Тогда
			РеквизитыФактическогоПлательщикаИНН = ИнформацияОПлатеже[0].UltmtDbtr.Id.OrgId.Othr.Получить(0).Id;
			ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ФактическийПлательщик", ИнформацияОПлатеже[0].UltmtDbtr.Nm);
			ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ФактическийПлательщикБИН", РеквизитыФактическогоПлательщикаИНН);
		КонецЕсли;
		
		ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "Плательщик", ИнформацияОПлатеже[0].Dbtr.Nm);
		РеквизитыПлательщикаИНН = ИнформацияОПлатеже[0].Dbtr.Id.OrgId.Othr.Получить(0).Id;
		ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ПлательщикБИН", РеквизитыПлательщикаИНН);
		РеквизитыПлательщикаКБЕ = ИнформацияОПлатеже[0].Dbtr.Id.OrgId.Othr.Получить(1).Id + ИнформацияОПлатеже[0].Dbtr.Id.OrgId.Othr.Получить(2).Id;
		ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ПлательщикКБЕ", РеквизитыПлательщикаКБЕ);
		ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "НомерСчетаПлательщика", ИнформацияОПлатеже[0].DbtrAcct.Id.IBAN);
		БИКПлательщика = ИнформацияОПлатеже[0].DbtrAgt.FinInstnId.BICFI;
		ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "БИКБанкаПлательщика", БИКПлательщика);

		СсылкаБанкПлательщика = ЭлектронноеВзаимодействие.НайтиСсылку("Банки", БИКПлательщика);//поиск наименования банка-ссылка по БИК, т.к.в хмл файл не записываем наименование банка
		БанкПлательщика = ?(НЕ ЗначениеЗаполнено(СсылкаБанкПлательщика), "", СсылкаБанкПлательщика.Наименование);
		ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "БанкПлательщика", БанкПлательщика);
		РеквизитыПлательщикаРукводитель = ИнформацияОПлатеже[0].Dbtr.Id.OrgId.Othr.Получить(3).Id;
		ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ФИОРуководителя", РеквизитыПлательщикаРукводитель);
		РеквизитыПлательщикаГлавБух = ИнформацияОПлатеже[0].Dbtr.Id.OrgId.Othr.Получить(4).Id;
		ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ФИОГлавногоБухгалтера", РеквизитыПлательщикаГлавБух);
		РеквизитыПлательщикаРезиденство = ИнформацияОПлатеже[0].Dbtr.Id.OrgId.Othr.Получить(1).Id;
		Если РеквизитыПлательщикаРезиденство <> "1" Тогда
			ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "СтранаРезидентстваПлательщика", ИнформацияОПлатеже[0].Dbtr.CtryOfRes);
		КонецЕсли;
		
		РеквизитыПоПолучателю = ИнформацияОПлатеже[0].CdtTrfTxInf[0];
		ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "Получатель", РеквизитыПоПолучателю.Cdtr.Nm);
		РеквизитыПолучателяКБЕ = РеквизитыПоПолучателю.Cdtr.Id.OrgId.Othr.Получить(1).Id + РеквизитыПоПолучателю.Cdtr.Id.OrgId.Othr.Получить(2).Id;
		РеквизитыПолучателяРезиденство = РеквизитыПоПолучателю.Cdtr.Id.OrgId.Othr.Получить(1).Id;
		РеквизитыПолучателяИНН = РеквизитыПоПолучателю.Cdtr.Id.OrgId.Othr.Получить(0).Id;
		ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ПолучательБИН",РеквизитыПолучателяИНН);
		ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ПолучательКБЕ", РеквизитыПолучателяКБЕ);
		ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "НомерСчетаПолучателя", РеквизитыПоПолучателю.CdtrAcct.Id.IBAN);
		БИКПолучателя = РеквизитыПоПолучателю.CdtrAgt.FinInstnId.BICFI;
		ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "БИКБанкаПолучателя", БИКПолучателя);
		...
		ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "БанкПолучателя", БанкПолучателя);
		Если РеквизитыПолучателяРезиденство <> "1" Тогда
			ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "СтранаРезидентстваПолучателя", РеквизитыПоПолучателю.Cdtr.CtryOfRes);
		КонецЕсли;
		...
		Если РеквизитыПоПолучателю.RmtInf.Strd[0].TaxRmt <> Неопределено Тогда //Представление информации перевода платежа, связанных с налогообложением
			ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "КодБюджетнойКлассификации", РеквизитыПоПолучателю.RmtInf.Strd[0].TaxRmt.Rcrd[0].Ctgy);
		КонецЕсли;

		ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "КодНазначенияПлатежа", РеквизитыПоПолучателю.Purp.Cd);
		ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ДатаВалютирования",Формат(РеквизитыПоПолучателю.RmtInf.Strd[0].RfrdDocInf[0].RltdDt, НСтр("ru='ДФ=''дд.MM.гггг'''")));
		
	КонецЕсли;
	
КонецПроцедуры

Функция ПолучитьПустуюТЗПлатёжногоПоручения()

	СписокСотрудников = Новый ТаблицаЗначений;
	
	СписокСотрудников.Колонки.Добавить("Плательщик");//фактический плательщик
	СписокСотрудников.Колонки.Добавить("ПлательщикБИН");//БИН фактического плательщика
	СписокСотрудников.Колонки.Добавить("Отправитель");
	СписокСотрудников.Колонки.Добавить("ОтправительБИН");
	СписокСотрудников.Колонки.Добавить("ПлательщикКБЕ");
	СписокСотрудников.Колонки.Добавить("НомерСчетаПлательщика");
	СписокСотрудников.Колонки.Добавить("БИКБанкаПлательщика");
	СписокСотрудников.Колонки.Добавить("ФИОРуководителя");
	СписокСотрудников.Колонки.Добавить("ФИОГлавногоБухгалтера");
	СписокСотрудников.Колонки.Добавить("БанкПлательщика");
	СписокСотрудников.Колонки.Добавить("СтранаРезидентстваОтправителя");
	
	СписокСотрудников.Колонки.Добавить("Получатель");
	СписокСотрудников.Колонки.Добавить("ПолучательБИН");
	СписокСотрудников.Колонки.Добавить("ПолучательКБЕ");
	СписокСотрудников.Колонки.Добавить("НомерСчетаПолучателя");
	СписокСотрудников.Колонки.Добавить("БИКБанкаПолучателя");
	СписокСотрудников.Колонки.Добавить("СтранаРезидентстваПолучателя");
	СписокСотрудников.Колонки.Добавить("ИИК");
	СписокСотрудников.Колонки.Добавить("КонечныйПолучатель");
	СписокСотрудников.Колонки.Добавить("КонечныйПолучательБИН");
	СписокСотрудников.Колонки.Добавить("СуммаПрописью");
	СписокСотрудников.Колонки.Добавить("СуммаЧислом");
	СписокСотрудников.Колонки.Добавить("Период");
	СписокСотрудников.Колонки.Добавить("БанкПолучателя");
	СписокСотрудников.Колонки.Добавить("НазначениеПлатежа");
	СписокСотрудников.Колонки.Добавить("КодНазначенияПлатежа");
	СписокСотрудников.Колонки.Добавить("ДатаВалютирования");
...
Процедура ОбработатьОшибкиФормированияЭД(Ошибки, ВидЭД, СсылкаНаОбъект = Неопределено)
	...
	Если ЗначениеЗаполнено(Ошибки) Тогда
		...
		Иначе
			...
			ТекстЗаголовка = СтрШаблон(ШаблонЗаголовка, ВидЭД);
		КонецЕсли;
...
#Область ЗаполнениеЧастейXDTO 
//PartyIdentification135
Процедура ЗаполнитьПлательщика(Элемент, ДеревоДанных, Фабрика, ПространствоИмен, ТекстОшибки)
	
	Организация = ЗначениеРеквизитаВДереве(ДеревоДанных, "Организация");
	СтруктурноеПодразделениеОтправитель = ЗначениеРеквизитаВДереве(ДеревоДанных, "СтруктурноеПодразделениеОтправитель");
	Дата = ЗначениеРеквизитаВДереве(ДеревоДанных, "Дата");
	
	ОтветственныеЛица = ОбменСБанкамиПереопределяемый.ПолучитьОтветственныеЛицаОрганизации(Организация, СтруктурноеПодразделениеОтправитель, КонецДня(Дата));
	ДобавитьЗначениеВДерево(ДеревоДанных, "Руководители", ?(НЕ ЗначениеЗаполнено(ОтветственныеЛица.Руководитель), "ФИО ответственного лица организации", ОтветственныеЛица.Руководитель));
	ДобавитьЗначениеВДерево(ДеревоДанных, "ГлавныйБухгалтер", ?(НЕ ЗначениеЗаполнено(ОтветственныеЛица.ГлавныйБухгалтер), "Не предусмотрено", ОтветственныеЛица.ГлавныйБухгалтер));

	ПлательщикНаименование = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПлательщика.Наименование");
	ЗаполнитьСвойствоXDTO(Элемент, "Nm", ПлательщикНаименование, Истина, ТекстОшибки);//Имя плательщика платежа
	Id = ОбъектТипаCML(Фабрика, "Party38Choice", ПространствоИмен);
	OrgId = ОбъектТипаCML(Фабрика, "OrganisationIdentification29", ПространствоИмен);//Структура идентификации организации
	
	Othr = НовыйЭлементСпискаXDTO(OrgId.Othr, Фабрика);
	ИННПлательщика = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПлательщика.ИНН");
	ЗаполнитьСвойствоXDTO(Othr, "Id", ИННПлательщика, Истина, ТекстОшибки);
	SchmeNm = ОбъектТипаCML(Фабрика, "OrganisationIdentificationSchemeName1Choice", ПространствоИмен); 
	ЗаполнитьСвойствоXDTO(SchmeNm, "Cd", "COID", Истина, ТекстОшибки);//"COID" указывает на то, что в Id БИН/ИИН инициатора платежа
	ЗаполнитьСвойствоXDTO(Othr, "SchmeNm", SchmeNm, Истина, ТекстОшибки);
	
	Othr = НовыйЭлементСпискаXDTO(OrgId.Othr, Фабрика);
	СтранаРегистрацииПлательщика = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПлательщика.ПлательщикКодСтраныРегистрации");
	ЗаполнитьСвойствоXDTO(Othr, "Id", ?(СтранаРегистрацииПлательщика = "KZ", "1", "2"), Истина, ТекстОшибки); // Платёж производится резидентом/нерезидентом
	SchmeNm = ОбъектТипаCML(Фабрика, "OrganisationIdentificationSchemeName1Choice", ПространствоИмен);
	ЗаполнитьСвойствоXDTO(SchmeNm, "Prtry", "IRS", Истина, ТекстОшибки); //"IRS" указывает на то, что в Id признак резиденства (1 - резидент РК)
	ЗаполнитьСвойствоXDTO(Othr, "SchmeNm", SchmeNm, Истина, ТекстОшибки);
	
	Если СтранаРегистрацииПлательщика <> "KZ" Тогда
		ЗаполнитьСвойствоXDTO(Элемент, "CtryOfRes", СтранаРегистрацииПлательщика, Истина, ТекстОшибки);//CtryOfRes - код страны плательщика в формате Альфа-2. Устанавливается, если платёж производится от нерезидентом
	КонецЕсли;
	...
	КБЕПлательщика = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПлательщика.КБЕ");
	Othr = НовыйЭлементСпискаXDTO(OrgId.Othr, Фабрика);
	ЗаполнитьСвойствоXDTO(Othr, "Id", Прав(КБЕПлательщика, 1), Истина, ТекстОшибки); //Cектор экономики из КБе, вторая - сектор экономики
	SchmeNm = ОбъектТипаCML(Фабрика, "OrganisationIdentificationSchemeName1Choice", ПространствоИмен);
	ЗаполнитьСвойствоXDTO(SchmeNm, "Prtry", "SECO", Истина, ТекстОшибки);//"SECO" указывает на то, что в Id сектор экономики
	ЗаполнитьСвойствоXDTO(Othr, "SchmeNm", SchmeNm, Истина, ТекстОшибки);
	
	РуководительПлательщика = ЗначениеРеквизитаВДереве(ДеревоДанных, "Руководители");
	Othr = НовыйЭлементСпискаXDTO(OrgId.Othr, Фабрика);
	ЗаполнитьСвойствоXDTO(Othr, "Id", РуководительПлательщика, Истина, ТекстОшибки);
	SchmeNm = ОбъектТипаCML(Фабрика, "OrganisationIdentificationSchemeName1Choice", ПространствоИмен);
	ЗаполнитьСвойствоXDTO(SchmeNm, "Prtry", "CHIEF", Истина, ТекстОшибки);//"CHIEF" указывает на то, что в Id руководитель организации
	ЗаполнитьСвойствоXDTO(Othr, "SchmeNm", SchmeNm, Истина, ТекстОшибки);
	
	ГлавныйБухгалтерПлательщика = ЗначениеРеквизитаВДереве(ДеревоДанных, "ГлавныйБухгалтер");
	Othr = НовыйЭлементСпискаXDTO(OrgId.Othr, Фабрика);
	ЗаполнитьСвойствоXDTO(Othr, "Id", ГлавныйБухгалтерПлательщика, Истина, ТекстОшибки);
	SchmeNm = ОбъектТипаCML(Фабрика, "OrganisationIdentificationSchemeName1Choice", ПространствоИмен);
	ЗаполнитьСвойствоXDTO(SchmeNm, "Prtry", "MAINBK", Истина, ТекстОшибки);//"MAINBK" указывает на то, что в Id главный бухгалтер организации
	ЗаполнитьСвойствоXDTO(Othr, "SchmeNm", SchmeNm, Истина, ТекстОшибки);
	
	ЗаполнитьСвойствоXDTO(Id, "OrgId", OrgId, Истина, ТекстОшибки);
	ЗаполнитьСвойствоXDTO(Элемент, "Id", Id, Истина, ТекстОшибки); 
	
КонецПроцедуры

// PmtInf - для документов пенсионных/социальных выплат
Процедура ЗаполнитьИнформациюПлатежаПоВзносамОтчислениямЕП(СообщениеЗаголовок, ВалютаДокумента, ДеревоДанных, Фабрика, ПространствоИмен, ТекстОшибки)
	...
	Если ПереданСписокСотрудников И ТаблицаСотрудников.Количество() > 0 Тогда
		...
		Для Каждого Сотрудник Из ТаблицаСотрудников Цикл
			...
			Месяц = Месяц(Сотрудник.ПериодПлатежа);
			
			PmtInf = НовыйЭлементСпискаXDTO(СообщениеЗаголовок.PmtInf, Фабрика); 
			
			ЗаполнитьСвойствоXDTO(PmtInf, "PmtInfId", "PIDORG" + ИННПлательщика + "N" + НомерПлатежа, Истина, ТекстОшибки);//PmtInfId - Идентификатор группы платежей, объединённых одинаковыми характеристиками, обязательный
			
			ЗаполнитьСвойствоXDTO(PmtInf, "PmtMtd", "TRF", Истина, ТекстОшибки);//PmtMtd - Метод оплаты (по умолчанию "TRF", также может быть "CHK" или "TRA"), обязательный
			
			ReqdExctnDt = ОбъектТипаCML(Фабрика, "DateAndDateTime2Choice", ПространствоИмен);
			ЗаполнитьСвойствоXDTO(ReqdExctnDt, "Dt", Дата, Истина, ТекстОшибки);//ReqdExctnDt.Dt - Дата, на которую банк отправителя денег запрашивает платежную систему обработать платеж
			ЗаполнитьСвойствоXDTO(PmtInf, "ReqdExctnDt", ReqdExctnDt, Истина, ТекстОшибки);
			
			ФактическийПлательщик = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПлательщика.ФактическийПлательщик");
			Если ЗначениеЗаполнено(ФактическийПлательщик) Тогда 
				//Заполняется,если конечный отправитель денег отличается от отправителя денег - Dbtr
				UltmtDbtr = ОбъектТипаCML(Фабрика, "PartyIdentification135", ПространствоИмен);// UltmtDbtr - конечный отправитель денег
				ЗаполнитьСвойствоXDTO(UltmtDbtr, "Nm", ФактическийПлательщик.НаименованиеПолное, Истина, ТекстОшибки);//Имя фактического(конечного) плательщика платежа
				
				Id = ОбъектТипаCML(Фабрика, "Party38Choice", ПространствоИмен);
				OrgId = ОбъектТипаCML(Фабрика, "OrganisationIdentification29", ПространствоИмен);//Структура идентификации организации
				
				Othr = НовыйЭлементСпискаXDTO(OrgId.Othr, Фабрика);
				ЗаполнитьСвойствоXDTO(Othr, "Id", ФактическийПлательщик.ИдентификационныйНомер, Истина, ТекстОшибки);
				SchmeNm = ОбъектТипаCML(Фабрика, "OrganisationIdentificationSchemeName1Choice", ПространствоИмен); 
				ЗаполнитьСвойствоXDTO(SchmeNm, "Cd", "COID", Истина, ТекстОшибки);//"COID" указывает на то, что в Id БИН/ИИН конечного отправителя денег
				ЗаполнитьСвойствоXDTO(Othr, "SchmeNm", SchmeNm, Истина, ТекстОшибки);
				
				ЗаполнитьСвойствоXDTO(Id, "OrgId", OrgId, Истина, ТекстОшибки);
				ЗаполнитьСвойствоXDTO(UltmtDbtr, "Id", Id, Истина, ТекстОшибки);
				
				ЗаполнитьСвойствоXDTO(PmtInf, "UltmtDbtr", UltmtDbtr, Истина, ТекстОшибки);
			КонецЕсли;
				
			Dbtr = ОбъектТипаCML(Фабрика, "PartyIdentification135", ПространствоИмен);//Dbtr идентифицирует плательщика
			ЗаполнитьПлательщика(Dbtr, ДеревоДанных, Фабрика, ПространствоИмен, ТекстОшибки);
			ЗаполнитьСвойствоXDTO(PmtInf, "Dbtr", Dbtr, Истина, ТекстОшибки);
			
			DbtrAcct = ОбъектТипаCML(Фабрика, "CashAccount40", ПространствоИмен);//DbtrAcct - Счёт отправителя, в схеме xsd не обязателен к заполненению, однако, судя по примерам нацбанка, предполагается заполнять 
			Id = ОбъектТипаCML(Фабрика, "AccountIdentification4Choice", ПространствоИмен);
			ПлательщикРасчСчет = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПлательщика.РасчСчет");//IBAN: KZCCБББХХХХХХХХХХХХХ
			Попытка
				ЗаполнитьСвойствоXDTO(Id, "IBAN", ПлательщикРасчСчет, Истина, ТекстОшибки);
			Исключение
				ВызватьИсключение("Счет плательщика указан в формате, отличном от IBAN!");
			КонецПопытки;
			ЗаполнитьСвойствоXDTO(DbtrAcct, "Id", Id, Истина, ТекстОшибки);
			ЗаполнитьСвойствоXDTO(PmtInf, "DbtrAcct", DbtrAcct, Истина, ТекстОшибки);
			
			DbtrAgt = ОбъектТипаCML(Фабрика, "BranchAndFinancialInstitutionIdentification6", ПространствоИмен);//DbtrAgt - Банк отправителя денег
			FinInstnId = ОбъектТипаCML(Фабрика, "FinancialInstitutionIdentification18", ПространствоИмен);
			ПлательщикБанкБИК = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПлательщика.Банк.БИК");
			ЗаполнитьСвойствоXDTO(FinInstnId, "BICFI", СокрЛП(ПлательщикБанкБИК), Истина, ТекстОшибки);
			ЗаполнитьСвойствоXDTO(DbtrAgt, "FinInstnId", FinInstnId, Истина, ТекстОшибки);
			//DEBT - Все расходы по сделке несет Плательщик
			//CRED - Все расходы по сделке несет Получатель средств
			//SHAR - В контексте перевода денежных средств по инициативе Плательщика означает, что расходы по сделке на стороне отправителя несет Плательщик, 
			//		а расходы по сделке на стороне получателя несет Получатель средств. В контексте прямого дебета означает, что расходы по сделке на стороне отправителя 
			//		несет Получатель средств, а расходы по сделке на стороне получателя несет Плательщик
			//SLEV - Плата взимается в соответствии с правилами, согласованными на уровне обслуживания и (или) схеме
			ЗаполнитьСвойствоXDTO(PmtInf, "ChrgBr", "DEBT", , ТекстОшибки);
			ЗаполнитьСвойствоXDTO(PmtInf, "DbtrAgt", DbtrAgt, Истина, ТекстОшибки);
			
			CdtTrfTxInf = НовыйЭлементСпискаXDTO(PmtInf.CdtTrfTxInf, Фабрика);//CdtTrfTxInf - Информация платежа кредитового перевода, обязательный
			PmtId = ОбъектТипаCML(Фабрика, "PaymentIdentification6", ПространствоИмен);//PmtId - Идентификатор платежа, обязательный
			
			ЗаполнитьСвойствоXDTO(PmtId, "EndToEndId", "E2EIDORG" + ИННПлательщика + "N" + НомерПлатежа, Истина, ТекстОшибки);//EndToEndId - Сквозной идентификатор единичного платежа, обязательный
			
			ЗаполнитьСвойствоXDTO(CdtTrfTxInf, "PmtId", PmtId, Истина, ТекстОшибки);
			
			PmtTpInf = ОбъектТипаCML(Фабрика, "PaymentTypeInformation26", ПространствоИмен);//PmtTpInf - Информация деталей
			ЗаполнитьСвойствоXDTO(PmtTpInf, "InstrPrty", "NORM", , ТекстОшибки);//Приоритет указания - срочность выполнения, не обязательный: либо "NORM", либо "HIGH"
			LclInstrm = ОбъектТипаCML(Фабрика, "LocalInstrument2Choice", ПространствоИмен);//Имеет либо свойством "Prtry"(вид платежа), либо свойством "Cd" (заполняется по внешнему справочнику кодов)
			ЗаполнитьСвойствоXDTO(LclInstrm, "Prtry", "01", , ТекстОшибки); //01 - платёжное поручение
			ЗаполнитьСвойствоXDTO(PmtTpInf, "LclInstrm", LclInstrm, Истина, ТекстОшибки);
			
			CtgyPurp = ОбъектТипаCML(Фабрика, "CategoryPurpose1Choice", ПространствоИмен);//CtgyPurp - категория назначения, обязательна при осуществлении пенсионных платежей и социальный отчислений
			ЗаполнитьСвойствоXDTO(CtgyPurp, "Prtry", Сотрудник.ВидНалога, Истина, ТекстОшибки);
			ЗаполнитьСвойствоXDTO(PmtTpInf, "CtgyPurp", CtgyPurp, Истина, ТекстОшибки);
			
			ЗаполнитьСвойствоXDTO(CdtTrfTxInf, "PmtTpInf", PmtTpInf, Истина, ТекстОшибки);
			
			Amt = ОбъектТипаCML(Фабрика, "AmountType4Choice", ПространствоИмен);//Amt - Сумма, обязательный. Атрибут "Ccy" отражает валюту перевода.
			InstdAmt = ОбъектТипаCML(Фабрика, "ActiveOrHistoricCurrencyAndAmount", ПространствоИмен);
			ЗаполнитьСвойствоXDTO(InstdAmt, "Ccy", Строка(ВалютаДокумента), Истина, ТекстОшибки);
			ЗаполнитьСвойствоXDTO(InstdAmt, "__content", Сотрудник.Сумма, Истина, ТекстОшибки);
			ЗаполнитьСвойствоXDTO(Amt, "InstdAmt", InstdAmt, Истина, ТекстОшибки);
			ЗаполнитьСвойствоXDTO(CdtTrfTxInf, "Amt", Amt, Истина, ТекстОшибки);
			
			CdtrAgt = ОбъектТипаCML(Фабрика, "BranchAndFinancialInstitutionIdentification6", ПространствоИмен);//CdtrAgt - Банк получателя
			FinInstnId = ОбъектТипаCML(Фабрика, "FinancialInstitutionIdentification18", ПространствоИмен);
			ПолучательБанкБИК = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПолучателя.Банк.БИК");
			ЗаполнитьСвойствоXDTO(FinInstnId, "BICFI", СокрЛП(ПолучательБанкБИК), Истина, ТекстОшибки);
			ЗаполнитьСвойствоXDTO(CdtrAgt, "FinInstnId", FinInstnId, Истина, ТекстОшибки);
			ЗаполнитьСвойствоXDTO(CdtTrfTxInf, "CdtrAgt", CdtrAgt, Истина, ТекстОшибки);
			
			#Область Cdtr
			
			Cdtr = ОбъектТипаCML(Фабрика, "PartyIdentification135", ПространствоИмен);//Cdtr - Бенефициар. В случае с перечислениями пенсионных и социальных отчислений данные соответствующего налогового органа
			ПолучательНаименование = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПолучателя.Наименование");
			ЗаполнитьСвойствоXDTO(Cdtr, "Nm", ПолучательНаименование, Истина, ТекстОшибки);//Имя получателя платежа
			
			Id = ОбъектТипаCML(Фабрика, "Party38Choice", ПространствоИмен);
			OrgId = ОбъектТипаCML(Фабрика, "OrganisationIdentification29", ПространствоИмен);
			
			Othr = НовыйЭлементСпискаXDTO(OrgId.Othr, Фабрика);
			ПолучательИНН = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПолучателя.ИНН");
			ЗаполнитьСвойствоXDTO(Othr, "Id", ПолучательИНН, Истина, ТекстОшибки);
			SchmeNm = ОбъектТипаCML(Фабрика, "OrganisationIdentificationSchemeName1Choice", ПространствоИмен); 
			ЗаполнитьСвойствоXDTO(SchmeNm, "Cd", "COID", Истина, ТекстОшибки);//"COID" указывает на то, что в Id БИН/ИИН получателя
			ЗаполнитьСвойствоXDTO(Othr, "SchmeNm", SchmeNm, Истина, ТекстОшибки);
			
			Othr = НовыйЭлементСпискаXDTO(OrgId.Othr, Фабрика);
			СтранаРегистрацииБенефициара = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПолучателя.ПолучательКодСтраныРегистрации");
			ЗаполнитьСвойствоXDTO(Othr, "Id", ?(СтранаРегистрацииБенефициара = "KZ", "1", "2"), Истина, ТекстОшибки); // Платёж производится с резидентом/нерезидентом
			SchmeNm = ОбъектТипаCML(Фабрика, "OrganisationIdentificationSchemeName1Choice", ПространствоИмен);
			ЗаполнитьСвойствоXDTO(SchmeNm, "Prtry", "IRS", Истина, ТекстОшибки); //"IRS" указывает на то, что в Id признак резиденства (1 - резидент РК)
			ЗаполнитьСвойствоXDTO(Othr, "SchmeNm", SchmeNm, Истина, ТекстОшибки);
			...
			ПолучательКБЕ = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПолучателя.КБЕ");
			Othr = НовыйЭлементСпискаXDTO(OrgId.Othr, Фабрика);
			ЗаполнитьСвойствоXDTO(Othr, "Id", Прав(ПолучательКБЕ, 1), Истина, ТекстОшибки);//Cектор экономики из КБе, вторая - сектор экономики
			SchmeNm = ОбъектТипаCML(Фабрика, "OrganisationIdentificationSchemeName1Choice", ПространствоИмен);
			ЗаполнитьСвойствоXDTO(SchmeNm, "Prtry", "SECO", Истина, ТекстОшибки); //"SECO" указывает на то, что в Id сектор экономики
			ЗаполнитьСвойствоXDTO(Othr, "SchmeNm", SchmeNm, Истина, ТекстОшибки);
			
			Если СтранаРегистрацииБенефициара <> "KZ" Тогда
				ЗаполнитьСвойствоXDTO(Cdtr, "CtryOfRes", СтранаРегистрацииБенефициара, Истина, ТекстОшибки);//CtryOfRes - код страны бенефициара в формате Альфа-2. Устанавливается, если платёж производится с нерезидентом
			КонецЕсли;
			
			ЗаполнитьСвойствоXDTO(Id, "OrgId", OrgId, Истина, ТекстОшибки);
			ЗаполнитьСвойствоXDTO(Cdtr, "Id", Id, Истина, ТекстОшибки);
			ЗаполнитьСвойствоXDTO(CdtTrfTxInf, "Cdtr", Cdtr, Истина, ТекстОшибки);
			
			#КонецОбласти
			
			CdtrAcct = ОбъектТипаCML(Фабрика, "CashAccount40", ПространствоИмен); //CdtrAcct - Счет бенефициара. Также заполняется форматом IBAN, если это возможно, иначе вызывается исключение.
			Id = ОбъектТипаCML(Фабрика, "AccountIdentification4Choice", ПространствоИмен);
			ПолучательРасчСчет = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПолучателя.РасчСчет");//IBAN: KZCCБББХХХХХХХХХХХХХ
			Попытка
				ЗаполнитьСвойствоXDTO(Id, "IBAN", ПолучательРасчСчет, Истина, ТекстОшибки);
			Исключение
				ВызватьИсключение("Счет бенефициара указан в формате, отличном от IBAN!");
			КонецПопытки;
			Tp = ОбъектТипаCML(Фабрика, "CashAccountType2Choice", ПространствоИмен);
			ЗаполнитьСвойствоXDTO(Tp, "Prtry", "Tran", Истина, ТекстОшибки);
			ЗаполнитьСвойствоXDTO(CdtrAcct, "Tp", Tp, Истина, ТекстОшибки);
			ЗаполнитьСвойствоXDTO(CdtrAcct, "Id", Id, Истина, ТекстОшибки);
			ЗаполнитьСвойствоXDTO(CdtTrfTxInf, "CdtrAcct", CdtrAcct, Истина, ТекстОшибки);
			
			UltmtCdtr = ОбъектТипаCML(Фабрика, "PartyIdentification135", ПространствоИмен);//UltmtCdtr - конечный бенефициар. Фактический бенефициар – участник перед которой у отправителя денег имеются денежные обязательства
			РеквизитыПолучателяНаименование = Сотрудник.Фамилия +" "+ Сотрудник.Имя +" "+ Сотрудник.Отчество;
			ЗаполнитьСвойствоXDTO(UltmtCdtr, "Nm", РеквизитыПолучателяНаименование, Истина, ТекстОшибки);
			Id = ОбъектТипаCML(Фабрика, "Party38Choice", ПространствоИмен);
			PrvtId = ОбъектТипаCML(Фабрика, "PersonIdentification13", ПространствоИмен);//PrvtId - Для идентификации физических лиц
			
			Othr = НовыйЭлементСпискаXDTO(PrvtId.Othr, Фабрика);
			ЗаполнитьСвойствоXDTO(Othr, "Id", Сотрудник.ИНН, Истина, ТекстОшибки);
			SchmeNm = ОбъектТипаCML(Фабрика, "PersonIdentificationSchemeName1Choice", ПространствоИмен);
			ЗаполнитьСвойствоXDTO(SchmeNm, "Cd", "NIDN", Истина, ТекстОшибки); //"NIDN" используется, когда в свойстве "Id" ИНН физического лица
			ЗаполнитьСвойствоXDTO(Othr, "SchmeNm", SchmeNm, Истина, ТекстОшибки);
			ЗаполнитьСвойствоXDTO(Id, "PrvtId", PrvtId, Истина, ТекстОшибки);
			ЗаполнитьСвойствоXDTO(UltmtCdtr, "Id", Id, Истина, ТекстОшибки);
			ЗаполнитьСвойствоXDTO(CdtTrfTxInf, "UltmtCdtr", UltmtCdtr, Истина, ТекстОшибки);
		
			Purp = ОбъектТипаCML(Фабрика, "Purpose2Choice", ПространствоИмен);
			КодНазначенияПлатежа = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПлатежа.КодНазначенияПлатежа");
			Попытка
				ЗаполнитьСвойствоXDTO(Purp, "Cd", КодНазначенияПлатежа, Истина, ТекстОшибки);
			Исключение
				ВызватьИсключение("Не указан код назначения платежа (КНП)!");
			КонецПопытки;
			ЗаполнитьСвойствоXDTO(CdtTrfTxInf, "Purp", Purp, Истина, ТекстОшибки);
			
			RmtInf = ОбъектТипаCML(Фабрика, "RemittanceInformation21", ПространствоИмен);//RmtInf - Информация перевода в формате RemittanceInformation11, не обязательно
			Strd = НовыйЭлементСпискаXDTO(RmtInf.Strd, Фабрика);
			RfrdDocInf = НовыйЭлементСпискаXDTO(Strd.RfrdDocInf, Фабрика); 
			НомерДокумента = ЗначениеРеквизитаВДереве(ДеревоДанных, "Номер");
			ЗаполнитьСвойствоXDTO(RfrdDocInf, "Nb", НомерДокумента, , ТекстОшибки);//Nb - Уникальный и однозначный идентификатор ссылочного документа
			Дата = ЗначениеРеквизитаВДереве(ДеревоДанных, "Дата");
			ЗаполнитьСвойствоXDTO(RfrdDocInf, "RltdDt", Дата, , ТекстОшибки);//RltdDt - Дата, связанная с ссылочным документом
			TaxRmt = ОбъектТипаCML(Фабрика, "TaxData1", ПространствоИмен);//TaxRmt - Представление информации перевода платежа, осуществляемого для целей, связанных с налогообложением
			Rcrd = НовыйЭлементСпискаXDTO(TaxRmt.Rcrd, Фабрика);
			Prd = ОбъектТипаCML(Фабрика, "TaxPeriod3", ПространствоИмен);//Rcrd.Prd информацию о периоде, к которому принадлежат отчисления в документе
			ЗаполнитьСвойствоXDTO(Prd, "Yr", Год(Сотрудник.ПериодПлатежа) , Истина, ТекстОшибки);
			ЗаполнитьСвойствоXDTO(Prd, "Tp", "MM" + ?(Месяц < 10, "0" + Месяц, Месяц), Истина, ТекстОшибки);
			
			НазначениеПлатежа = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПлатежа.НазначениеПлатежа");
			
			МассивСтрок = РазбитьСтрокуНаМассив(НазначениеПлатежа, 140);
			Для каждого Строка Из МассивСтрок Цикл
				AddtlRmtInf = НовыйЭлементСпискаXDTO(Strd.AddtlRmtInf, Фабрика, Строка);//AddtlRmtInf - Дополнительная информация в свободной текстовой форме
			КонецЦикла;
			
			ЗаполнитьСвойствоXDTO(Rcrd, "Prd", Prd, Истина, ТекстОшибки);
			ЗаполнитьСвойствоXDTO(Strd, "TaxRmt", TaxRmt, Истина, ТекстОшибки);
			ЗаполнитьСвойствоXDTO(CdtTrfTxInf, "RmtInf", RmtInf, Истина, ТекстОшибки);
		КонецЦикла;
	КонецЕсли;
	
КонецПроцедуры

// PmtInf - для документов налоговых и прочих платежей
Процедура ЗаполнитьИнформациюПлатежаПоНалоговымИПрочимВыплатам(СообщениеЗаголовок, ВалютаДокумента, ДеревоДанных, Фабрика, ПространствоИмен, ТекстОшибки)
	...
	PmtInf = НовыйЭлементСпискаXDTO(СообщениеЗаголовок.PmtInf, Фабрика);
	ЗаполнитьСвойствоXDTO(PmtInf, "PmtInfId", "PIDORG" + ИННПлательщика + "N0001", Истина, ТекстОшибки);//PmtInfId - Идентификатор группы платежей, объединённых одинаковыми характеристиками, обязательный
	ЗаполнитьСвойствоXDTO(PmtInf, "PmtMtd", "TRF", Истина, ТекстОшибки);//PmtMtd - Метод оплаты (по умолчанию "TRF", также может быть "CHK" или "TRA"), обязательный
	...
	CdtTrfTxInf = НовыйЭлементСпискаXDTO(PmtInf.CdtTrfTxInf, Фабрика);//CdtTrfTxInf - Информация платежа кредитового перевода, обязательный
	PmtId = ОбъектТипаCML(Фабрика, "PaymentIdentification6", ПространствоИмен);//PmtId - Идентификатор платежа, обязательный
	ЗаполнитьСвойствоXDTO(PmtId, "EndToEndId", "E2EIDORG" + ИННПлательщика + "N0001", Истина, ТекстОшибки);//EndToEndId - Сквозной идентификатор, обязательный
	ЗаполнитьСвойствоXDTO(CdtTrfTxInf, "PmtId", PmtId, Истина, ТекстОшибки);
	
	PmtTpInf = ОбъектТипаCML(Фабрика, "PaymentTypeInformation26", ПространствоИмен);//PmtTpInf - Информация деталей
	ЗаполнитьСвойствоXDTO(PmtTpInf, "InstrPrty", "NORM", , ТекстОшибки);//Срочность выполнения, не обязательный: либо "NORM", либо "HIGH"
	LclInstrm = ОбъектТипаCML(Фабрика, "LocalInstrument2Choice", ПространствоИмен);//Имеет либо свойством "Prtry"(вид платежа), либо свойством "Cd" (заполняется по внешнему справочнику кодов)
	ПеречислениеНалога = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПлатежа.ПеречислениеНалога");
	Если ПеречислениеНалога Тогда
		ЗаполнитьСвойствоXDTO(LclInstrm, "Prtry", "08", , ТекстОшибки);//08 - свободное платёжное поручение 
	иначе
		ЗаполнитьСвойствоXDTO(LclInstrm, "Prtry", "01", , ТекстОшибки);//01 - платёжное поручение 
	КонецЕсли;
	ЗаполнитьСвойствоXDTO(PmtTpInf, "LclInstrm", LclInstrm, Истина, ТекстОшибки);
	...
	InstdAmt = ОбъектТипаCML(Фабрика, "ActiveOrHistoricCurrencyAndAmount", ПространствоИмен);
	ЗаполнитьСвойствоXDTO(InstdAmt, "Ccy", Строка(ВалютаДокумента), Истина, ТекстОшибки);
	ЗаполнитьСвойствоXDTO(InstdAmt, "__content", Сумма, Истина, ТекстОшибки);
	ЗаполнитьСвойствоXDTO(Amt, "InstdAmt", InstdAmt, Истина, ТекстОшибки);
	ЗаполнитьСвойствоXDTO(CdtTrfTxInf, "Amt", Amt, Истина, ТекстОшибки);
	
	CdtrAgt = ОбъектТипаCML(Фабрика, "BranchAndFinancialInstitutionIdentification6", ПространствоИмен);//Банк получателя
	FinInstnId = ОбъектТипаCML(Фабрика, "FinancialInstitutionIdentification18", ПространствоИмен);
	ПолучательБанкБИК = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПолучателя.Банк.БИК");
	ЗаполнитьСвойствоXDTO(FinInstnId, "BICFI", СокрЛП(ПолучательБанкБИК), Истина, ТекстОшибки);
	ЗаполнитьСвойствоXDTO(CdtrAgt, "FinInstnId", FinInstnId, Истина, ТекстОшибки);
	...
	Cdtr = ОбъектТипаCML(Фабрика, "PartyIdentification135", ПространствоИмен);//Cdtr - Бенефициар
	ПолучательНаименование = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПолучателя.Наименование");
	ЗаполнитьСвойствоXDTO(Cdtr, "Nm", ПолучательНаименование, Истина, ТекстОшибки);//Имя получателя платежа
	Id = ОбъектТипаCML(Фабрика, "Party38Choice", ПространствоИмен);
	OrgId = ОбъектТипаCML(Фабрика, "OrganisationIdentification29", ПространствоИмен);
	...
	Если ПеречислениеНалога Тогда
		...
		Rcrd = НовыйЭлементСпискаXDTO(TaxRmt.Rcrd, Фабрика);
		КодБК = ЗначениеРеквизитаВДереве(ДеревоДанных, "ПлатежиВБюджет.ПоказательКБК");
		ЗаполнитьСвойствоXDTO(Rcrd, "Ctgy", КодБК, Истина, ТекстОшибки);// Rcrd.Ctgy - Налоговый код, опубликованный налоговым органом
		ЗаполнитьСвойствоXDTO(Strd, "TaxRmt", TaxRmt, Истина, ТекстОшибки);
	КонецЕсли;
...
// PmtInf - для документов заработной платы
Процедура ЗаполнитьИнформациюПлатежаЗаработнойПлаты(СообщениеЗаголовок, ВалютаДокумента, ДеревоДанных, Фабрика, ПространствоИмен, ТекстОшибки);
	...
	Если ПереданСписокСотрудников И ТаблицаСотрудников.Количество() > 0 Тогда
		...
		Для Каждого Сотрудник Из ТаблицаСотрудников Цикл
			...
			Dbtr = ОбъектТипаCML(Фабрика, "PartyIdentification135", ПространствоИмен);//Dbtr идентифицирует фактического плательщика
			ЗаполнитьПлательщика(Dbtr, ДеревоДанных, Фабрика, ПространствоИмен, ТекстОшибки);
			ЗаполнитьСвойствоXDTO(PmtInf, "Dbtr", Dbtr, Истина, ТекстОшибки);
			...
			CdtTrfTxInf = НовыйЭлементСпискаXDTO(PmtInf.CdtTrfTxInf, Фабрика);//CdtTrfTxInf - Информация платежа кредитового перевода, обязательный
			PmtId = ОбъектТипаCML(Фабрика, "PaymentIdentification6", ПространствоИмен);//PmtId - Идентификатор платежа, обязательный
			ЗаполнитьСвойствоXDTO(PmtId, "EndToEndId", "E2EIDORG" + ИННПлательщика + "N" + НомерПлатежа, Истина, ТекстОшибки);//EndToEndId - Сквозной идентификатор, обязательный
			ЗаполнитьСвойствоXDTO(CdtTrfTxInf, "PmtId", PmtId, Истина, ТекстОшибки);
			
			PmtTpInf = ОбъектТипаCML(Фабрика, "PaymentTypeInformation26", ПространствоИмен);//PmtTpInf - Информация деталей
			ЗаполнитьСвойствоXDTO(PmtTpInf, "InstrPrty", "NORM", , ТекстОшибки);//Приоритет указания - срочность выполнения, не обязательный: либо "NORM", либо "HIGH"
			LclInstrm = ОбъектТипаCML(Фабрика, "LocalInstrument2Choice", ПространствоИмен);//Имеет либо свойством "Prtry", либо свойством Cd (оба принимают в качестве значения строку от 1 до 35 символов)
			ЗаполнитьСвойствоXDTO(LclInstrm, "Prtry", "01", , ТекстОшибки);//01 - платёжное поручение
			ЗаполнитьСвойствоXDTO(PmtTpInf, "LclInstrm", LclInstrm, Истина, ТекстОшибки);
			ЗаполнитьСвойствоXDTO(CdtTrfTxInf, "PmtTpInf", PmtTpInf, Истина, ТекстОшибки);
			...
			CdtrAgtAcct = ОбъектТипаCML(Фабрика, "CashAccount40", ПространствоИмен); // CdtrAgtAcct - Счёт банка Бенефициара, по которому будет сделана кредитовая запись в результате платёжной операции.
			Id = ОбъектТипаCML(Фабрика, "AccountIdentification4Choice", ПространствоИмен);
			ПолучательРасчСчет = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПолучателя.РасчСчет");//IBAN: KZCCБББХХХХХХХХХХХХХ
			Попытка
				ЗаполнитьСвойствоXDTO(Id, "IBAN", ПолучательРасчСчет, Истина, ТекстОшибки);
			Исключение
				ВызватьИсключение("Счет бенефициара указан в формате, отличном от IBAN!");
			КонецПопытки;
			ЗаполнитьСвойствоXDTO(CdtrAgtAcct, "Id", Id, Истина, ТекстОшибки);
			ЗаполнитьСвойствоXDTO(CdtTrfTxInf, "CdtrAgtAcct", CdtrAgtAcct, Истина, ТекстОшибки);
			
			#Область Cdtr
			
			Cdtr = ОбъектТипаCML(Фабрика, "PartyIdentification135", ПространствоИмен);//Cdtr - Бенефициар. В случае с перечислениями пенсионных и социальных отчислений в данном элементе указываются данные соответствующего налогового органа
			РеквизитыПолучателяНаименование = Сотрудник.Фамилия +" "+ Сотрудник.Имя +" "+ Сотрудник.Отчество;
			ЗаполнитьСвойствоXDTO(Cdtr, "Nm", РеквизитыПолучателяНаименование, Истина, ТекстОшибки);//Имя получателя платежа
			Id = ОбъектТипаCML(Фабрика, "Party38Choice", ПространствоИмен);
			PrvtId = ОбъектТипаCML(Фабрика, "PersonIdentification13", ПространствоИмен);//PrvtId - Для идентификации физических лиц
			...
			Othr = НовыйЭлементСпискаXDTO(PrvtId.Othr, Фабрика);
			ЗаполнитьСвойствоXDTO(Othr, "Id", ?(Сотрудник.КодСтраныРегистрации= "KZ", "1", "2"), Истина, ТекстОшибки); 
			SchmeNm = ОбъектТипаCML(Фабрика, "PersonIdentificationSchemeName1Choice", ПространствоИмен);
			ЗаполнитьСвойствоXDTO(SchmeNm, "Prtry", "IRS", Истина, ТекстОшибки);//"IRS" указывает на то, что в Id признак резиденства (1 - резидент РК)
			ЗаполнитьСвойствоXDTO(Othr, "SchmeNm", SchmeNm, Истина, ТекстОшибки);
			...
			Если Сотрудник.КодСтраныРегистрации <> "KZ" Тогда
				ЗаполнитьСвойствоXDTO(Cdtr, "CtryOfRes", Сотрудник.КодСтраныРегистрации, Истина, ТекстОшибки);//CtryOfRes - код страны бенефициара в формате Альфа-2. Устанавливается, если платёж производится с нерезидентом
			КонецЕсли;
			...
			CdtrAcct = ОбъектТипаCML(Фабрика, "CashAccount40", ПространствоИмен); //CdtrAcct - Счет бенефициара.
			Id = ОбъектТипаCML(Фабрика, "AccountIdentification4Choice", ПространствоИмен);
			Попытка
				ЗаполнитьСвойствоXDTO(Id, "IBAN", Сотрудник.РасчСчет, Истина, ТекстОшибки);//IBAN: KZCCБББХХХХХХХХХХХХХ
			Исключение
				ВызватьИсключение("Счет бенефициара указан в формате, отличном от IBAN!");
			КонецПопытки;
			ЗаполнитьСвойствоXDTO(CdtrAcct, "Id", Id, Истина, ТекстОшибки);
...
// PmtInf - для документов исполнительных листов
Процедура ЗаполнитьИнформациюПлатежаИсполнительногоЛиста(СообщениеЗаголовок, ВалютаДокумента, ДеревоДанных, Фабрика, ПространствоИмен, ТекстОшибки);
	...
	Если ПереданСписокСотрудников И ТаблицаСотрудников.Количество() > 0 Тогда
		...
		Для Каждого Сотрудник Из ТаблицаСотрудников Цикл
			...
			Если ЗначениеЗаполнено(Сотрудник.ФактическийПлательщик) Тогда // Фактический плательщик в исполнтельных листах является физ.лицо от которого делают отправку денег
				UltmtDbtr = ОбъектТипаCML(Фабрика, "PartyIdentification135", ПространствоИмен);// UltmtDbtr - конечный отправитель денег
				ЗаполнитьСвойствоXDTO(UltmtDbtr, "Nm", Строка(Сотрудник.ФактическийПлательщик), Истина, ТекстОшибки);//Имя фактического(конечного) плательщика платежа
				
				Id = ОбъектТипаCML(Фабрика, "Party38Choice", ПространствоИмен);
				PrvtId = ОбъектТипаCML(Фабрика, "PersonIdentification13", ПространствоИмен);// PrvtId - Для идентификации физических лиц
				
				Othr = НовыйЭлементСпискаXDTO(PrvtId.Othr, Фабрика);
				ЗаполнитьСвойствоXDTO(Othr, "Id", Сотрудник.ФактическийПлательщикИНН, Истина, ТекстОшибки);
				SchmeNm = ОбъектТипаCML(Фабрика, "PersonIdentificationSchemeName1Choice", ПространствоИмен); 
				ЗаполнитьСвойствоXDTO(SchmeNm, "Cd", "NIDN", Истина, ТекстОшибки);//"NIDN" указывает на то, что в Id БИН/ИИН конечного отправителя денег обозначает ИНН физического лица
				ЗаполнитьСвойствоXDTO(Othr, "SchmeNm", SchmeNm, Истина, ТекстОшибки);
			...
			ТипКонтрагента = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПолучателя.ТипКонтрагента");
			...
			Если ТипКонтрагента = "ЮрЛицо" Тогда
				...
				ЗаполнитьСвойствоXDTO(Othr, "Id", ПолучательИНН, Истина, ТекстОшибки);
				SchmeNm = ОбъектТипаCML(Фабрика, "OrganisationIdentificationSchemeName1Choice", ПространствоИмен); 
				ЗаполнитьСвойствоXDTO(SchmeNm, "Cd", "COID", Истина, ТекстОшибки);//"COID" указывает на то, что в Id БИН/ИИН юр.получателя
				ЗаполнитьСвойствоXDTO(Othr, "SchmeNm", SchmeNm, Истина, ТекстОшибки);
...
Функция РазбитьСтрокуНаМассив(ИсходнаяСтрока, Длина) Экспорт
	...
	КоличествоСтрок = Цел(СтрДлина(ИсходнаяСтрока) / Длина) + 1;
	
	Для n = 0 По КоличествоСтрок - 1 Цикл
		Начало = n * Длина;
		Конец = Начало + Длина;
		Если Начало < СтрДлина(ИсходнаяСтрока) Тогда
			СтрокаЧасть = Сред(ИсходнаяСтрока, Начало + 1, Длина);
			Результат.Добавить(СтрокаЧасть);
		КонецЕсли;
	КонецЦикла;
