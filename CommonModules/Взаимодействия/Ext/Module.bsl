...
Функция СоздатьПисьмо(Сообщение, УчетнаяЗапись, ОтправитьСразу = Истина) Экспорт
	...
	Исключение
		...
		ШаблонТекстаСообщения = НСтр("ru = 'Не удалось сформировать письмо по причине:
			|%1'", ОбщегоНазначения.КодОсновногоЯзыка()); // строка записывается в ИБ
...
Процедура ПриПоискеЗаменыСсылок(ПарыЗамен, НеобработанныеЗначенияОригиналов) Экспорт
	...
	Для каждого НеобработанныйДубль Из НеобработанныеЗначенияОригиналов Цикл
		...
		Исключение
			ОтменитьТранзакцию();
			ЗаписьЖурналаРегистрации(НСтр("ru='Поиск и замена ссылок'", ОбщегоНазначения.КодОсновногоЯзыка()), // строка записывается в ИБ
...
Функция ТекстЗапросаНаПолучениеИнформацииОКонтактах(ВключатьEmail)
	...
	// БК3. В метод ФрагментЗапросаИнформацияОКонтактах
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, ",&ПолеАдресЭлектроннойПочты",
		ФрагментЗапросаИнформацияОКонтактах(ВключатьEmail, "Пользователи", Истина));
...
Функция СформироватьТекстЗапросаДляПоискаПоEmail(ПоискПоСписку, ИтогиПоEmail = Ложь)
	...
	Для каждого ОписаниеКонтакта Из ВзаимодействияКлиентСервер.ОписанияКонтактов() Цикл
		...
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
			...
			|	" + ?(ОписаниеКонтакта.ЕстьВладелец," ТаблицаКонтактнойИнформации.Объект.Владелец.Наименование","""""") + ",
			|	ТаблицаКонтактнойИнформации.Объект.Наименование
			|ИЗ
			|	РегистрСведений.КонтактнаяИнформация КАК ТаблицаКонтактнойИнформации
			|ГДЕ
			|	ТаблицаКонтактнойИнформации.Объект ССЫЛКА Справочник." + ОписаниеКонтакта.Имя + "
			|	И ВЫРАЗИТЬ(ТаблицаКонтактнойИнформации.Представление КАК СТРОКА(100)) = &Адрес
			|   И (НЕ ТаблицаКонтактнойИнформации.Объект.ПометкаУдаления)
			|	И ТаблицаКонтактнойИнформации.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты)
			|  "+?(ОписаниеКонтакта.Иерархический," И (НЕ ТаблицаКонтактнойИнформации.Объект.ЭтоГруппа)","");
...
Функция НайтиКонтактыСАдресами(СтрокаПоиска) Экспорт
	...
	|	И (Таблица.Наименование ПОДОБНО &ВведеннаяСтрока СПЕЦСИМВОЛ ""~""
	|			ИЛИ ПОДСТРОКА(ТаблицаКонтактнаяИнформация.Представление, 1, 1000) ПОДОБНО &ВведеннаяСтрока СПЕЦСИМВОЛ ""~"")";
	...
	Для каждого ОписаниеКонтакта Из ОписанияКонтактов Цикл
		...
		Для Каждого Поле Из СписокПолей Цикл
			...
			Если ПолеСтроковогоТипа Тогда
				УсловиеПолейВводаПоСтроке = УсловиеПолейВводаПоСтроке + " ИЛИ СправочникКонтакт." + Поле.Имя + " ПОДОБНО &ВведеннаяСтрока СПЕЦСИМВОЛ ""~""";
			КонецЕсли;
		КонецЦикла;
		...
		|			И (ПОДСТРОКА(ТаблицаКонтактнаяИнформация.Представление, 1, 1000) ПОДОБНО &ВведеннаяСтрока СПЕЦСИМВОЛ ""~"" 
		|" + УсловиеПолейВводаПоСтроке + ") 
		|";
...
Функция КонтактыПоДоменномуАдресу(Знач ИмяДомена)
	...
	Для каждого ОписаниеКонтакта Из ВзаимодействияКлиентСервер.ОписанияКонтактов() Цикл
		...
		"ВЫБРАТЬ РАЗЛИЧНЫЕ РАЗРЕШЕННЫЕ
			...
			|ГДЕ
			|	&УсловиеТипКонтакта
			|	И ТаблицаКонтактнаяИнформация.Представление ПОДОБНО &СтрокаПоиска СПЕЦСИМВОЛ ""~""
			|	И (ТаблицаКонтактнаяИнформация.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты))";
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПолеНаименованиеВладельца", 
			?(ОписаниеКонтакта.ЕстьВладелец, "ТаблицаКонтактнаяИнформация.Объект.Владелец.Наименование ", """"""));
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПолеНаименование", 
			"ТаблицаКонтактнаяИнформация.Объект.Наименование");
...
Функция ПолучитьТекстЗапросаПоискКонтактовПоСтрокеСEmail()
	...
	Для каждого ОписаниеКонтакта Из ВзаимодействияКлиентСервер.ОписанияКонтактов() Цикл
		...
		|ИЗ
		|	Справочник." +ОписаниеКонтакта.Имя +" КАК ТаблицаСправочника
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КонтактнаяИнформация КАК ТаблицаКонтактнаяИнформация
		|		ПО (ТаблицаКонтактнаяИнформация.Объект = ТаблицаСправочника.Ссылка)
		|			И (ТаблицаКонтактнаяИнформация.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты))
		|ГДЕ
		|	ТаблицаСправочника.Ссылка В(&МассивСсылок) "+?(ОписаниеКонтакта.Иерархический," И (НЕ ТаблицаСправочника.ЭтоГруппа)","")+"
		|	И (НЕ ТаблицаСправочника.ПометкаУдаления)";
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ШаблонСообщенияОбОшибке = НСтр("ru = 'Не удалось применить правило обработки писем ""%1"" для почты ""%2"" по причине: 
			                                |%3
			                                |Требуется исправить правило.'", ОбщегоНазначения.КодОсновногоЯзыка()); // строка записывается в ИБ
...
Функция НайтиКонтактыПоНачалуНаименования(Знач СтрокаПоиска) Экспорт
	...
	Для каждого ОписаниеКонтакта Из ВзаимодействияКлиентСервер.ОписанияКонтактов() Цикл
		...
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
			...
			//|			И (ТаблицаКонтактнаяИнформация.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты))
			|ГДЕ
			|	ТаблицаСправочника.Наименование ПОДОБНО &Наименование  СПЕЦСИМВОЛ ""~"" "+?(ОписаниеКонтакта.Иерархический, " И (НЕ ТаблицаСправочника.ЭтоГруппа)","")+"
			|	И (НЕ ТаблицаСправочника.ПометкаУдаления)";
