...
Процедура ВывестиИнформациюОбОбработанныхЭД(ТекстЗаголовка, КолСформированных, КолУтвержденных, КолПодписанных, КолПодготовленных, КолОтправленных = 0) Экспорт
	...
	Если КолОтправленных > 0 Тогда
		Текст.Вставить(0, СтрШаблон(НСтр("ru = 'отправлено пакетов: (%1)'"), КолОтправленных));
	КонецЕсли;
	
	Если КолПодготовленных > 0 Тогда
		Текст.Вставить(0, СтрШаблон(НСтр("ru = 'подготовлено к отправке пакетов: (%1)'"), КолПодготовленных));
	КонецЕсли;
	
	Если КолПодписанных > 0 Тогда
		Текст.Вставить(0, СтрШаблон(НСтр("ru = 'подписано: (%1)'"), КолПодписанных));
	КонецЕсли;
	
	Если КолУтвержденных > 0 Тогда
		Текст.Вставить(0, СтрШаблон(НСтр("ru = 'утверждено: (%1)'"), КолУтвержденных));
	КонецЕсли;
	
	Если КолСформированных > 0 Тогда
		Текст.Вставить(0, СтрШаблон(НСтр("ru = 'сформировано: (%1)'"), КолСформированных));
	КонецЕсли;
	
	Если Текст.Количество() Тогда
		Текст[0] = ВРег(Лев(Текст[0], 1)) + Сред(Текст[0], 2);
		Текст = СтрСоединить(Текст, ", ");
	Иначе
