...
Функция СтатусСопоставлениеСНТсФНО_ИБ(Знач СтатусСопСНТсФНО) Экспорт
	...
	ИначеЕсли ВРегСтатусСНТ = "CANCELED_BY_FNO_REVOKE" Тогда
		СтатусСопСНТсФНО = ПредопределенноеЗначение("Перечисление.СтатусыСопоставленийСНТиФНО.ОтзывМетодомЗамены");
...
Функция ТекстКомментарияСозданПоДаннымСНТ(СсылкаСНТ) Экспорт
	...
	ТекстКомментария = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(НСтр("ru = 'Создан по данным документа ""%1"".'"),КраткоеПредставлениеСНТ);
