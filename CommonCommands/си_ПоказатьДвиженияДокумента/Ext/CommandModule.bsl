...
Функция ПолучитьПараметрыДокумента(Ссылка)
	...
	ЕстьРучнаяКорректировка	= общ_ПереопределяемыеПроцедурыБКСервер.ЕстьРеквизитДокумента("РучнаяКорректировка", МетаданныеДокумента);
...
Процедура ПослеЗакрытияЗапроса(Результат, Параметры) Экспорт 
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Иначе
			...
			Сообщение.Текст = НСтр("ru = 'Не удалось провести документ'");
			Сообщение.Сообщить();
