...
Функция ПраздничныеДниПроизводственногоКалендаря(КодПроизводственногоКалендаря, НомерГода) Экспорт
	...
	Если КодПроизводственногоКалендаря = "РК" Тогда	
		...
		Если НомерГода >= 2022 Тогда
			// Законом РК № 143-VII ЗРК от 29 сентября 2022 года вновь утвержден национальный праздник День Республики
			ДобавитьПраздничныйДень(ПраздничныеДни, 25, 10, НомерГода);	
		КонецЕсли;
		ДобавитьПраздничныйДень(ПраздничныеДни, 16, 12, НомерГода);
		Если НомерГода < 2022 Тогда
			// Законом РК № 143-VII ЗРК от 29 сентября 2022 года 17.12 исключен из праздничных дней
		...
		Если НомерГода >= 2006 Тогда
			...
			ИначеЕсли НомерГода = 2025 Тогда
				ДобавитьПраздничныйДень(ПраздничныеДни,  6,  6, НомерГода, Ложь);
			КонецЕсли;
		КонецЕсли;
		...
		Если НомерГода >= 2012 И НомерГода < 2022 Тогда
