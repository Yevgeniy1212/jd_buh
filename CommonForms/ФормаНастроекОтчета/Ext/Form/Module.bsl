...
Процедура ОбновитьОтборы(Строки = Неопределено, ЭлементыНастроек = Неопределено)
	...
	Для Каждого ЭлементНастройки Из ЭлементыНастроек Цикл
		...
		Если ЭлементНастройки.Представление = "###ОтборПоОрганизации###" Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ИзменитьИспользованиеПредопределенныхПараметровВывода(Использование = Истина)
	...
	// Изменения БК 3.0
	СвойствоЭлементаОформления = Отчет.КомпоновщикНастроек.Настройки.УсловноеОформление;
	// --- БК 3.0
	...
	Для Каждого Строка Из Строки Цикл 
		...
		// Изменения БК 3.0
		Если ЭлементНастройки = Неопределено Тогда
			ЭлементНастройки = ЭлементНастройки(СвойствоЭлементаОформления, Строка);
			Если ЭлементНастройки = Неопределено Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
