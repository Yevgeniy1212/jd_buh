...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = НСтр("ru = 'Панель регистров налогового учета'");
...
Функция НачатьЗамер(Событие, Комментарий = Неопределено)
	...
	Если Событие = "СписокОтчетов.Открытие" Или Событие = "ПанельОтчетов.Открытие" Тогда
		Замер.Имя = Событие;
		Комментарий = Комментарий + ?(ПустаяСтрока(Комментарий), "", "; ") + НСтр("ru = 'Из панели отчетов:'") + " " + ПараметрыКлиента.ПутьКПодсистеме;
	Иначе
		...
		КонецЕсли;
		Комментарий = Комментарий + ?(ПустаяСтрока(Комментарий), "", "; ") + ПараметрыКлиента.ПутьКПодсистеме;
		Комментарий = Комментарий + ?(ПустаяСтрока(Комментарий), "", "; ") + НСтр("ru = 'Подсказки:'") + " " + Строка(ПоказыватьПодсказки);
	КонецЕсли;
...
Функция ДобавитьЭлементыВариантаОтчета(ПараметрыЗаполнения, Вариант, ВГруппу, УровеньВложенности = 0)
	...
	Если ЗначениеЗаполнено(Вариант.Автор) Тогда
		Надпись.Подсказка = СокрЛ(Надпись.Подсказка) + Символы.ПС + СтрШаблон(НСтр("ru = 'Автор: %1'"), СокрЛП(Строка(Вариант.Автор)));
	КонецЕсли;
