...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РежимРасшифровки = Параметры.Свойство("Расшифровка") И Параметры.Расшифровка <> Неопределено
		Или Параметры.Свойство("ПредставлениеВарианта") И Параметры.ПредставлениеВарианта = НСтр("ru = 'Расшифровка';kk = 'Мағынасын ашу'");
	...
	ВозможностьИзмененияРежимаОтчета = Истина;
...
Процедура НачатьЗамер()
	...
	Если ВыполнятьЗамеры Тогда
		Комментарий = НСтр("ru = 'Непосредственно:'") + " " + Строка(Непосредственно);
...
&НаСервере
Процедура ЗаполнитьПользовательскиеНастройкиОтчета()
...
Процедура ЗагрузитьРезультатФормированияОтчета()
	...
	СкрытьВспомогательныеПараметрыОтчета(ОтчетТабличныйДокумент);
...
Функция ДополнительныеДанныеТекстаЗапросаПоОбъектуПоиска(Объект)
	...
	Для Каждого Поле Из Поля Цикл
		...
		Реквизит = Объект.Реквизиты.Найти(Поле.Имя);
		Данные = "" + ?(Реквизит = Неопределено, Поле.Имя, Реквизит.Представление()) + ": "" + " + Поле.Имя;
...
Функция ПредставлениеУниверсальногоПоискаСсылки(Данные, ПолеПоиска, Условие, СтрокаПоиска)
	...
	Если ЗначениеЗаполнено(Данные.ДополнительныеДанные) Тогда 
		...
		ПредставлениеЗначения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"%1 (%2)", Данные.Представление, Данные.ДополнительныеДанные);
...
Процедура УстановитьВидимостьДоступность(ПриСохраненииВарианта = Ложь)
	...
	Элементы.РежимВариантаОтчетаБК.Видимость = ВозможностьИзмененияРежимаОтчета;
...
Функция ПолучитьРежимВыполненияОтчета()
	...
	Если Параметры.КлючВарианта = "ОтчетОРеализацииТоваровУслуг" Тогда
		Если Параметры.Свойство("ОткрытИзРассылки") И Параметры.ОткрытИзРассылки Тогда
			Возврат Истина
		КонецЕсли;
	КонецЕсли;
	...
	Если СписокОтчетов.НайтиПоЗначению(ОтчетМетаданные.Имя) <> Неопределено Тогда
		...
		ВозможностьИзмененияРежимаОтчета = Ложь;
	    Возврат Истина;
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Процедура СкрытьВспомогательныеПараметрыОтчета(ТабличныйДокумент)
	
	ЯчейкаПараметры = ТабличныйДокумент.НайтиТекст(НСтр("ru = 'Параметры:'"));
	Если ЯчейкаПараметры <> Неопределено И ЯчейкаПараметры.Текст = НСтр("ru = 'Параметры:'") Тогда
		
		ЯчейкаПараметрыВерх = ЯчейкаПараметры.Верх;
		ЯчейкаПараметрыЛево = ЯчейкаПараметры.Лево;
		
		СкрываемыеПараметры = Новый Массив;
		СкрываемыеПараметры.Добавить(НСтр("ru = 'Период рассылки'"));
		
		КоличествоПараметров = 1;
		Пока КоличествоПараметров < ТабличныйДокумент.ВысотаТаблицы Цикл
			Если Не ПустаяСтрока(ТабличныйДокумент.Область(ЯчейкаПараметрыВерх + КоличествоПараметров, ЯчейкаПараметрыЛево).Текст) Тогда
				Прервать;
			КонецЕсли;
			Если ПустаяСтрока(ТабличныйДокумент.Область(ЯчейкаПараметрыВерх + КоличествоПараметров, ЯчейкаПараметрыЛево + 1).Текст) Тогда
				Прервать;
			КонецЕсли;
			КоличествоПараметров = КоличествоПараметров + 1;
		КонецЦикла;
		
		ВыводимыеПараметры = Новый Массив;
		Для НомерСтроки = 1 По КоличествоПараметров Цикл
			ВывестиПараметр = Истина;
			ТекстПараметра = ТабличныйДокумент.Область(ЯчейкаПараметрыВерх + НомерСтроки - 1, ЯчейкаПараметрыЛево + 1).Текст;
			Для Каждого СкрываемыйПараметр Из СкрываемыеПараметры Цикл
				Если СтрНачинаетсяС(ТекстПараметра, СкрываемыйПараметр + ":") Тогда
					ВывестиПараметр = Ложь;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			Если ВывестиПараметр Тогда
				ВыводимыеПараметры.Добавить(ТекстПараметра);
			КонецЕсли;
		КонецЦикла;
		
		КоличествоВыводимыхПараметров = ВыводимыеПараметры.Количество();
		Если КоличествоВыводимыхПараметров < КоличествоПараметров Тогда
			Если КоличествоВыводимыхПараметров = 0
			   И ПустаяСтрока(ТабличныйДокумент.Область(ЯчейкаПараметрыВерх - 1, ЯчейкаПараметрыЛево).Текст)
			   И ПустаяСтрока(ТабличныйДокумент.Область(ЯчейкаПараметрыВерх - 1, ЯчейкаПараметрыЛево + 1).Текст) Тогда
				ЯчейкаПараметрыВерх = ЯчейкаПараметрыВерх - 1;
				КоличествоПараметров = КоличествоПараметров + 1;
			Иначе
				Для НомерСтроки = 1 По КоличествоВыводимыхПараметров Цикл
					ТабличныйДокумент.Область(ЯчейкаПараметрыВерх + НомерСтроки - 1, ЯчейкаПараметрыЛево + 1).Текст = ВыводимыеПараметры[НомерСтроки - 1];
				КонецЦикла;
			КонецЕсли;
			СкрываемыеСтроки = ТабличныйДокумент.Область(ЯчейкаПараметрыВерх + КоличествоВыводимыхПараметров, , ЯчейкаПараметрыВерх + КоличествоПараметров - 1);
			СкрываемыеСтроки.ВысотаСтроки = 1;
			СкрываемыеСтроки.Очистить();
			СкрываемыеСтроки.Разгруппировать();
		КонецЕсли;
		
	КонецЕсли;
	
	ЯчейкаОтбор = ТабличныйДокумент.НайтиТекст(НСтр("ru = 'Отбор:'"));
	Если ЯчейкаОтбор <> Неопределено И ЯчейкаОтбор.Текст = НСтр("ru = 'Отбор:'") Тогда
		
		ЯчейкаОтборВерх = ЯчейкаОтбор.Верх;
		ЯчейкаОтборЛево = ЯчейкаОтбор.Лево;
		
		СкрываемыеОтборы = Новый Массив;
		СкрываемыеОтборы.Добавить(НСтр("ru = ' ИЛИ ';kk = ' НЕМЕСЕ '") +"###ОтборПоОрганизации###");
		СкрываемыеОтборы.Добавить(НСтр("ru = ' И ';kk = ' және '") + "###ОтборПоОрганизации###");
		СкрываемыеОтборы.Добавить("###ОтборПоОрганизации###");
		
		ТекстОтбор = ТабличныйДокумент.Область(ЯчейкаОтборВерх, ЯчейкаОтборЛево + 1).Текст;
		ТекстОтбор = СтрЗаменить(ТекстОтбор, Символы.ПС, " ");
		Для Каждого СкрываемыйОтбор Из СкрываемыеОтборы Цикл
			Если СтрНайти(ТекстОтбор, СкрываемыйОтбор) Тогда
				ТекстОтбор = СтрЗаменить(ТекстОтбор, СкрываемыйОтбор, "");
			КонецЕсли;
		КонецЦикла;
		
		Если ПустаяСтрока(ТекстОтбор) Тогда
			СкрываемыеСтроки = ТабличныйДокумент.Область(ЯчейкаОтборВерх, , ЯчейкаОтборВерх);
			СкрываемыеСтроки.ВысотаСтроки = 1;
			СкрываемыеСтроки.Очистить();
			СкрываемыеСтроки.Разгруппировать();
		Иначе
			ТабличныйДокумент.Область(ЯчейкаОтборВерх, ЯчейкаОтборЛево + 1).Текст = СтрЗаменить(СтрЗаменить(СокрЛП(ТекстОтбор), Символы.Таб, " "), "  ", " ");
		КонецЕсли;
